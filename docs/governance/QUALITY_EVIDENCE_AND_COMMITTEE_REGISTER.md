...
| Decision Intelligence | **Conditional — server/RLS gates established; library closure remains open** | `npm run check:decision-intelligence-rls` verifies the 19 models reached by Decision services for tenant UUID, ENABLE/FORCE and policy USING/WITH CHECK; API check, `verify:decision-intelligence-http` and `verify:hr-http` pass. The Decision verifier proves 401/403, capability/company/tenant and revoked-session isolation, alert/evidence/feedback/policy lifecycle, replay/audit non-duplication, and stable mismatch receipts: HTTP `409`, `IDEMPOTENCY_MISMATCH`, `do-not-retry`. | Complete Decision browser/a11y/budget evidence and the registered UI-adapter migration before closure. |
...
| Browser E2E and accessibility coverage | Pass at Reports closure | `npm run test:e2e --workspace @baseer-erp/web`: `97 passed` + `1 intentional skip`; includes authenticated Operations/HR/Finance/Administration/Reports journeys on desktop/mobile, AR/EN, RTL/LTR and route-specific Axe checks | Continue route-specific coverage for Decision Intelligence and Command Center; manual visual review remains required before production. |
