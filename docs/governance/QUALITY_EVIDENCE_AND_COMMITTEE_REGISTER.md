...
| 2026-08-22 | Quality foundation — automated accessibility audit | **Pass — local/source verification** | [ADR-QUALITY-001](ADR-QUALITY-001-AXE_PLAYWRIGHT_ACCESSIBILITY.md): `@axe-core/playwright@4.13.0` checks WCAG A/AA on the Arabic and English sign-in page; desktop and mobile Playwright runs pass without exemptions. | Expand route-specific audit coverage when a primitive or workflow enters an active scope; manual keyboard, reader and visual review remain required. |
| 2026-08-22 | Security foundation — Fastify Helmet and authentication throttling | **Conditional pass — local/source verification** | [ADR-SEC-001](ADR-SEC-001-FASTIFY_HELMET_BASELINE.md) and [ADR-SEC-002](ADR-SEC-002-AUTH_THROTTLING.md): security headers apply to API JSON responses; sign-in, owner activation and refresh have IP plus hashed-identity 15-minute limits. `verify:auth-throttling` proves identity and IP 429 responses with `Retry-After: 900`; deployable dependency audit is clean. | CSP/HSTS/trusted proxy require deployment evidence. In-memory throttling is limited to one API replica; shared atomic storage and cross-replica proof are mandatory before horizontal scaling. Prisma P1 remains in build/migrate and production remains blocked. |
