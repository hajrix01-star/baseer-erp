...
| Command Center | **Current local security baseline — read-only calendar** | `check:command-center-rls` verifies the two calendar read models (`FinanceOperationalDay`, `FinanceDailyFinancialSummary`) for tenant UUID, RLS ENABLE/FORCE and USING/WITH CHECK; `verify:command-center-http` proves 401/403, `finance.daily_sales.read`, company/tenant isolation, revoked-session rejection and live RLS. | Re-run after any Command Center endpoint/model/capability change; financial writes and operational-calendar management are outside this read-only scope. |
...
| Browser E2E and accessibility coverage | Pass at Decision Intelligence closure | `npm run test:e2e --workspace @baseer-erp/web`: `103 passed` + `1 intentional skip`; includes authenticated Operations/HR/Finance/Administration/Reports/Decision journeys on desktop/mobile, AR/EN, RTL/LTR and route-specific Axe checks | Continue route-specific coverage for Command Center; manual visual review remains required before production. |
