...
11. **Noorix-compatible sign-in:** BASEER accepts either a full email or a 3–64 character short username. The server resolves the latter to `<username>@<tenant-code>.baseer.local`, uses that same resolved identity for rate limiting, and the administration UI displays the short form. HTTP verification covers both forms.
