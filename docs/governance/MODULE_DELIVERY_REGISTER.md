...
| --- | --- | --- | --- |
| Baseer UI Platform Enablement | **Conditional — Phase 0 evidence passes in working tree** | Governance, monitor model, staged plan, CSS-budget recovery, complete-session HR E2E fixture and local-database guard are recorded. | Establish a clean revision, then run dependency review and one non-financial BaseerCombobox only. |
