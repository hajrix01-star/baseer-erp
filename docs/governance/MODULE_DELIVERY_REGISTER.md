...
| Field | Value |
| --- | --- |
| Active work | **Command Center — library-migration closure** is the next module in the approved centralized sequence. Operations, HR, Finance, Administration, Reports and Decision Intelligence migrations are closed locally. UI Platform Enablement remains an ongoing cross-cutting authority, not a competing business scope. |
| Open owner acceptance | No closed module above retains library-migration acceptance. Production, Noorix, formal reporting and decision-policy acceptance keep their independent gates. |
| Next decision gate | جرد مركز القيادة من الـmanifest، وتصنيف كل قراءة وKPI وإجراء، ثم تحويل المستهلكين الحاليين عبر المحولات المركزية مع source/as-of وHTTP/RLS/browser evidence وإغلاق خطة التحول بالكامل. |
...
| Command Center | **Active — library-migration closure** | Existing command-center consumers remain available for the active migration. | Complete inventory, KPI/action ownership, scoped query, HTTP/RLS/browser and budget closure. |
