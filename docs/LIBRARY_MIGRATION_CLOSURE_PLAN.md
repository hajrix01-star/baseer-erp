...
| القسم | التبويبات/المسارات التي تدخل الجرد |
|---|---|
| المبيعات اليومية | تسجيل الدخول/التفعيل، إغلاق اليوم/الإجازة، سجل الإغلاقات وتفاصيلها |
| المشتريات والمصروفات | الإدخال المجمع، المورد/الفئة/الخزينة، النقدي والآجل، السداد، التعديل والعكس |
| الالتزامات المتكررة والقروض | profiles، الدفعات الفردية والمجمعة، الفواتير، القروض والسداد |
| الخزائن والبنوك | تعريف الخزائن، التحويل، الجرد/المطابقة، النشاط، العكس والترتيب |
| الحسابات | دليل الحسابات، حركة الحساب، الأرصدة والملخص الخادمي المؤرخ |
| إعدادات المالية والبيانات الرئيسية | الفترة والتهيئة، الموردون، الفئات وتصنيفاتها |
| سجل الفواتير المالي | register، مرشحات الخادم، pagination، drill-down وsnapshot |

**بوابة إغلاق المالية:** كل نموذج حي يمر عبر عقد Baseer المركزي، وكل تاريخ حي
عبر `BaseerDatePicker`، وكل اختيار بعيد عبر `BaseerCombobox` بعزل الشركة/
الجلسة، وكل سجل cursor عبر `BaseerDataGrid` في وضع الخادم. تبقى الجداول
الصغيرة ذات الحد الخادمي `DataTable` خفيفاً مقصوداً وليست legacy. لا يفرض
`TanStack Query` على command قصير أو قراءة محلية غير مشتركة؛ يستخدم فقط عند
وجود cache/invalidation مشترك موثق. ولا ينشأ Chart مالي لمجرد إكمال المكتبة؛
يحتاج read-model وmetric contract وsnapshot خادميين مستقلين.

التحقق المالي لا يكتفي بالواجهة: Decimal strings بلا `Number()` في أي حساب أو
payload مالي، وHTTP 401/403/409 وidempotency/reversal، وRLS الفعلي، وإغلاق
الفترة وسباقها، وتوازن القيود، ثم Playwright عربي/إنجليزي وسطح المكتب/الجوال
