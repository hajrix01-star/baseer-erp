...
const table = "MarketingCampaign";
const block = schema.match(new RegExp(`model ${table} \\{([\\s\\S]*?)^\\}`, "m"))?.[1] ?? "";
if (!/^\s*tenantId\s+String\s+@db\.Uuid\s*$/m.test(block) || !/^\s*companyId\s+String\s+@db\.Uuid\s*$/m.test(block)) throw new Error("MarketingCampaign must be tenant and company scoped.");
for (const required of [
  `ALTER TABLE "${table}" ENABLE ROW LEVEL SECURITY;`,
  `ALTER TABLE "${table}" FORCE ROW LEVEL SECURITY;`,
  `CREATE POLICY "${table}_tenant_isolation"`,
]) if (!migrations.includes(required)) throw new Error(`MarketingCampaign is missing Gate A1 RLS control: ${required}`);
...
if (/\b(fetch|axios|https?\.request|googleapis)\b/i.test(`${service}\n${controller}`)) throw new Error("Marketing Gate A1 must not include provider egress or provider SDK usage.");
...
console.log("Marketing & Reputation Gate A1 guard passed: company scope, RLS, idempotency/audit, no provider egress, honest readiness, central RHF/Zod forms.");
