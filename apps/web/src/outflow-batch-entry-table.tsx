...
export function OutflowBatchEntryTable<Row extends OutflowBatchEntryRow>({ language, text, ariaLabel, rows, categories, suppliers, vaults, vatEnabled, vatRateBasisPoints, allowedKinds, maxInvoiceDate, onChange, onSupplierChange, remoteSupplierSearch, remoteCategorySearch, renderSupplierAction, onRemove }: { language: "ar" | "en"; text: FinanceText; ariaLabel: string; rows: readonly Row[]; categories: readonly Category[]; suppliers: readonly Supplier[]; vaults: readonly Vault[]; vatEnabled: boolean; vatRateBasisPoints: number; allowedKinds: readonly Row["kind"][]; maxInvoiceDate?: string; onChange: <K extends keyof Row>(id: string, key: K, value: Row[K]) => void; onSupplierChange: (id: string, supplierId: string) => void; remoteSupplierSearch?: (query: string) => Promise<readonly { id: string; label: string; isFavorite?: boolean }[]>; remoteCategorySearch?: (kind: Row["kind"], query: string) => Promise<readonly { id: string; label: string }[]>; renderSupplierAction?: (supplier: Supplier) => ReactNode; onRemove: (id: string) => void }) {
