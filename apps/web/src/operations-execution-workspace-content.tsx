...
type OperationsOverview = {
  inventory: unknown[];
  requests: Array<{ status: "PENDING_RECEIPT" | "PARTIALLY_RECEIVED" | "RECEIVED" | "CANCELLED" | "REVERSED" }>;
...
export function OperationsExecutionWorkspaceContent({ language }: { language: Language }) {
  ...
  useEffect(() => {
    ...
    let active = true;
    void api<OperationsOverview>(current, "/operations/execution-workspace")
  ...
  const openRequests = overview?.requests.filter((request) => request.status === "PENDING_RECEIPT" || request.status === "PARTIALLY_RECEIVED").length ?? 0;
  return <section className="daily-sales-workspace">
    ...
    {loading ? <BaseerCard aria-busy="true"><p>{t.loading}</p></BaseerCard> : error ? <BaseerCard><p className="daily-sales-message error">{error}</p></BaseerCard> : <div className="baseer-card-grid"><BaseerCard><strong>{t.pending}</strong><p dir="ltr">{formatCount(openRequests, language)}</p></BaseerCard><BaseerCard><strong>{t.custody}</strong><p>{overview?.custody.representativeName ?? t.noRepresentative}</p><p dir="ltr">{formatMoney(overview?.custody.balance, "SAR", language)}</p></BaseerCard><BaseerCard><strong>{t.inventory}</strong><p dir="ltr">{formatCount(overview?.inventory.length ?? 0, language)}</p></BaseerCard></div>}
  </section>;
}
