import { lazy, Suspense, useCallback, useState, type ComponentType, type ReactNode, type RefObject } from "react";
...
function LoginFields({ language, local, domain, onLocal, onDomain, localError, domainError, disabled = false }: { language: "ar" | "en"; local: string; domain: string; onLocal: (value: string) => void; onDomain: (value: string) => void; localError?: string; domainError?: string; disabled?: boolean }) { const text = administrationText(language);
  return <><label>{text.domain}<span className="administration-login-domain"><b>@</b><input disabled={disabled} aria-invalid={Boolean(domainError)} value={domain} onChange={(event) => onDomain(event.target.value.replace(/^@/, ""))} placeholder="hajrix.com" inputMode="email" /></span>{domainError ? <small role="alert">{domainError}</small> : null}</label><label>{text.loginName}<input disabled={disabled} aria-invalid={Boolean(localError)} value={local} onChange={(event) => onLocal(event.target.value)} placeholder="ahmed" autoComplete="username" />{localError ? <small role="alert">{localError}</small> : null}</label></>;
}
...
function CreateUserDialog({ language, session, overview, onDone, onError, onClose }: { language: "ar" | "en"; session: ActiveSession; overview: AdministrationOverview; onDone: () => Promise<void>; onError: (error: unknown) => void; onClose: () => void }) { const text = administrationText(language); const companies = activeCompanies(overview);
  ...
  const dialogRef = useDialogFocusTrap({ open: true, saving: busy, onClose }); const save = async (next: CreateUserForm) => { setBusy(true); try { await createAdministrationUser(session, { login: composeLogin(next.loginLocal, next.loginDomain), nameAr: next.nameAr, nameEn: next.nameEn, password: next.password, preferredLanguage: next.preferredLanguage, companyIds: next.companyIds, roleId: next.roleId, avatarKind: next.avatarKind }); await onDone(); onClose(); } catch (error) { onError(error); } finally { setBusy(false); } };
  return <Dialog language={language} title={text.createEmployee} busy={busy} dialogRef={dialogRef} onClose={onClose}><BaseerValidatedFormField<CreateUserForm> id="administration-create-user" className="administration-dialog-form" values={values} schemaFactory={schemaFactory} onValid={(next) => void save(next)} errorSummaryLabel={text.checkRequiredFields}>{({ errors }) => <><AvatarPicker language={language} value={values.avatarKind} onChange={(avatarKind) => setValues((current) => ({ ...current, avatarKind }))} previewName={values.nameAr || text.users} /><LoginFields language={language} local={values.loginLocal} domain={values.loginDomain} onLocal={(loginLocal) => setValues((current) => ({ ...current, loginLocal }))} onDomain={(loginDomain) => setValues((current) => ({ ...current, loginDomain }))} localError={errors.loginLocal?.message} domainError={errors.loginDomain?.message} disabled={busy} /><label>{text.arabicName}<input aria-invalid={Boolean(errors.nameAr)} value={values.nameAr} onChange={(event) => setValues((current) => ({ ...current, nameAr: event.target.value }))} />{errors.nameAr ? <small role="alert">{errors.nameAr.message}</small> : null}</label><label>{text.englishName}<input aria-invalid={Boolean(errors.nameEn)} value={values.nameEn} onChange={(event) => setValues((current) => ({ ...current, nameEn: event.target.value }))} />{errors.nameEn ? <small role="alert">{errors.nameEn.message}</small> : null}</label><label>{text.preferredLanguage}<select value={values.preferredLanguage} onChange={(event) => setValues((current) => ({ ...current, preferredLanguage: event.target.value as CreateUserForm["preferredLanguage"] }))}><option value="ar">{text.arabic}</option><option value="en">{text.english}</option></select></label><PasswordField language={language} label={text.password} value={values.password} onChange={(password) => setValues((current) => ({ ...current, password }))} autoComplete="new-password" />{errors.password ? <small role="alert">{errors.password.message}</small> : null}<label>{text.role}<select aria-invalid={Boolean(errors.roleId)} value={values.roleId} onChange={(event) => setValues((current) => ({ ...current, roleId: event.target.value }))}><option value="">—</option>{overview.roles.map((role) => <option key={role.id} value={role.id}>{displayName(language, role)}</option>)}</select>{errors.roleId ? <small role="alert">{errors.roleId.message}</small> : null}</label><CompanyCheckboxes language={language} overview={overview} value={values.companyIds} onChange={(companyIds) => setValues((current) => ({ ...current, companyIds }))} disabled={busy} />{errors.companyIds ? <small role="alert">{errors.companyIds.message}</small> : null}<footer><button className="daily-sales-primary" disabled={busy}>{busy ? text.saving : text.saveEmployee}</button></footer></>}</BaseerValidatedFormField></Dialog>;
}
...
function ManageUserDialog({ language, session, user, overview, owner, onDone, onError, onClose }: { language: "ar" | "en"; session: ActiveSession; user: User; overview: AdministrationOverview; owner: boolean; onDone: () => Promise<void>; onError: (error: unknown) => void; onClose: () => void }) { const text = administrationText(language); const initialLogin = splitLogin(user.login);
  ...
  return <Dialog language={language} title={displayName(language, user)} busy={busy} dialogRef={dialogRef} onClose={onClose}><BaseerValidatedFormField<ManageUserForm> id="administration-manage-user" className="administration-dialog-form" values={values} schemaFactory={schemaFactory} onValid={(next) => void saveAll(next)} errorSummaryLabel={text.checkRequiredFields}>{({ errors }) => <><LoginFields language={language} local={values.loginLocal} domain={values.loginDomain} onLocal={(loginLocal) => setValues((current) => ({ ...current, loginLocal }))} onDomain={(loginDomain) => setValues((current) => ({ ...current, loginDomain }))} localError={errors.loginLocal?.message} domainError={errors.loginDomain?.message} disabled={!owner || busy} /><label>{text.arabicName}<input aria-invalid={Boolean(errors.nameAr)} value={values.nameAr} disabled={!owner || busy} onChange={(event) => setValues((current) => ({ ...current, nameAr: event.target.value }))} />{errors.nameAr ? <small role="alert">{errors.nameAr.message}</small> : null}</label><label>{text.englishName}<input aria-invalid={Boolean(errors.nameEn)} value={values.nameEn} disabled={!owner || busy} onChange={(event) => setValues((current) => ({ ...current, nameEn: event.target.value }))} />{errors.nameEn ? <small role="alert">{errors.nameEn.message}</small> : null}</label><label>{text.role}<select aria-invalid={Boolean(errors.roleId)} value={values.roleId} disabled={!owner || busy} onChange={(event) => setValues((current) => ({ ...current, roleId: event.target.value }))}>{overview.roles.map((role) => <option key={role.id} value={role.id}>{displayName(language, role)} · {role.permissionCodes.length} {text.rolePermissionCount}</option>)}</select>{errors.roleId ? <small role="alert">{errors.roleId.message}</small> : null}</label><CompanyCheckboxes language={language} overview={overview} value={values.companyIds} onChange={(companyIds) => setValues((current) => ({ ...current, companyIds }))} disabled={!owner || busy} />{errors.companyIds ? <small role="alert">{errors.companyIds.message}</small> : null}{owner && <section className="administration-sensitive-actions"><PasswordField language={language} label={text.newPassword} value={values.password} onChange={(password) => setValues((current) => ({ ...current, password }))} autoComplete="new-password" />{errors.password ? <small role="alert">{errors.password.message}</small> : null}<button className="daily-sales-secondary" disabled={busy} type="button" onClick={() => void run(() => updateAdministrationUserStatus(session, user.id, user.status === "ACTIVE" ? "DISABLED" : "ACTIVE", administrationAuditReason))}>{user.status === "ACTIVE" ? text.disableUser : text.enableUser}</button></section>}<footer>{owner && <button className="daily-sales-primary" disabled={busy || !canSave}>{busy ? text.saving : text.saveChanges}</button>}</footer></>}</BaseerValidatedFormField></Dialog>;
}
