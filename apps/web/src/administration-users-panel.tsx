...
export function AdministrationUsersPanel({ session, overview, owner, onDone, onError }: Props) {
  ...
  return <section className="administration-section administration-users-section">
    <header className="administration-section-heading"><div><h3>المستخدمون</h3></div>{owner && <button className="daily-sales-primary" type="button" onClick={() => setMode("create")}>+ إضافة موظف</button>}</header>
