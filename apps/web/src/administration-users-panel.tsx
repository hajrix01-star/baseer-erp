...
function CreateUserDialog({ language, session, overview, onDone, onError, onClose }: { language: "ar" | "en"; session: ActiveSession; overview: AdministrationOverview; onDone: () => Promise<void>; onError: (error: unknown) => void; onClose: () => void }) { const text = administrationText(language);
  ...
  return <Dialog language={language} title={text.createEmployee} busy={busy} dialogRef={dialogRef} onClose={onClose}><form className="administration-dialog-form" onSubmit={(event) => void save(event)}><AvatarPicker language={language} value={avatarKind} onChange={setAvatarKind} previewName={nameAr || text.users} /><LoginFields language={language} local={loginLocal} domain={loginDomain} onLocal={setLoginLocal} onDomain={setLoginDomain} disabled={busy} /><label>{text.arabicName}<input required value={nameAr} onChange={(event) => setNameAr(event.target.value)} /></label><label>{text.englishName}<input required value={nameEn} onChange={(event) => setNameEn(event.target.value)} /></label><label>{text.preferredLanguage}<select value={preferredLanguage} onChange={(event) => setPreferredLanguage(event.target.value as "ar" | "en")}><option value="ar">{text.arabic}</option><option value="en">{text.english}</option></select></label><PasswordField language={language} label={text.password} value={password} onChange={setPassword} autoComplete="new-password" required /><label>{text.role}<select value={roleId} onChange={(event) => setRoleId(event.target.value)}>{overview.roles.map((role) => <option key={role.id} value={role.id}>{displayName(language, role)}</option>)}</select></label><CompanyCheckboxes language={language} overview={overview} value={companyIds} onChange={setCompanyIds} disabled={busy} /><footer><button className="daily-sales-primary" disabled={busy || !companyIds.length}>{busy ? text.saving : text.saveEmployee}</button></footer></form></Dialog>;
}
