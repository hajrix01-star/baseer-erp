...
type Movement = { id: string; journalEntryId: string; businessDate: string; sourceType: string; sourceReference: string; displayLabelAr: string; displayLabelEn: string; displayReference: string; description: string | null; debitAmount: string; creditAmount: string };
...
export function FinanceAccountsWorkspace({ language }: { language: Language }) {
  ...
  const balance = (debit: string, credit: string) => {
    ...
    return value === 0 ? formatMoney("0") : `${formatMoney(String(Math.abs(value)))} · ${value >= 0 ? labels.debitBalance : labels.creditBalance}`;
  };
  ...
  const movementColumns: DataTableColumn<Movement>[] = [
    ...
    { id: "source", header: labels.source, cell: (item) => language === "ar" ? item.displayLabelAr : item.displayLabelEn },
  ...
  return <section className="daily-sales-workspace finance-setup-workspace" aria-label={labels.title}>
    ...
    {!receipt ? <BaseerCard><p>{text.loading}</p></BaseerCard> : <><BaseerSummaryMetricGrid><BaseerSummaryMetric label={labels.totalAccounts} value={String(receipt.accounts.length)} /><BaseerSummaryMetric label={labels.totalDebitMovement} value={formatMoney(String(totalDebit))} /><BaseerSummaryMetric label={labels.totalCreditMovement} value={formatMoney(String(totalCredit))} /><BaseerSummaryMetric label={labels.asOf} value={receipt.asOfBusinessDate} /></BaseerSummaryMetricGrid>{receipt.accounts.length ? <DataTable ariaLabel={labels.title} caption={labels.title} columns={accountColumns} rows={receipt.accounts} rowKey={(item) => item.id} /> : <BaseerCard><p>{labels.noAccounts}</p></BaseerCard>}</>}
