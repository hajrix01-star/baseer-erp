...
export function AdministrationRolesPanel({ language, session, overview, owner, onDone, onError }: Props) { const text = administrationText(language);
  const [editing, setEditing] = useState(false); const [editingRole, setEditingRole] = useState<Role | null>(null); const [values, setValues] = useState<RoleForm>({ nameAr: "", nameEn: "", permissionCodes: [] }); const [busy, setBusy] = useState(false); const [confirmDelete, setConfirmDelete] = useState(false);
  ...
  const moduleKeys = [...new Set(overview.permissions.map((permission) => permission.module))];
...
function PermissionModule({ moduleKey, language, permissions, selected, onToggle }: { moduleKey: string; language: "ar" | "en"; permissions: Permission[]; selected: string[]; onToggle: (permissionCode: string, checked: boolean) => void }) {
  ...
  const sections = [...new Map(modulePermissions.map((permission) => [language === "ar" ? permission.sectionAr : permission.sectionEn, modulePermissions.filter((candidate) => (language === "ar" ? candidate.sectionAr : candidate.sectionEn) === (language === "ar" ? permission.sectionAr : permission.sectionEn))])).entries()];
