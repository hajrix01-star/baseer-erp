...
function directSelection(codes: readonly string[], permissions: Permission[]): string[] {
  const selected = new Set(codes);
  const derived = new Set([...selected].flatMap((code) => permissions.find((permission) => permission.code === code)?.requires ?? []));
  return [...selected].filter((code) => !derived.has(code));
}
...
export function AdministrationRolesPanel({ language, session, overview, owner, onDone, onError }: Props) { const text = administrationText(language);
  ...
  const startEdit = (role: Role) => { if (role.isSystem) return; setEditingRole(role); setValues({ nameAr: role.nameAr, nameEn: role.nameEn, permissionCodes: directSelection(role.permissionCodes, overview.permissions) }); setEditing(true); };
  ...
  const toggle = (permissionCode: string, checked: boolean) => setValues((current) => {
    ...
    return { ...current, permissionCodes: directSelection(selected, overview.permissions) };
  });
  ...
  return <section className="administration-section administration-roles-section"><header className="administration-section-heading"><div><h3>{text.roles}</h3></div>{owner && <BaseerButton type="button" onClick={() => { reset(); setEditing(true); }}>+ {text.addRole}</BaseerButton>}</header><div className="administration-role-cards">{overview.roles.map((role) => {
    const directPermissionCodes = directSelection(role.permissionCodes, overview.permissions);
    const automaticPermissionCodes = normalizeSelection(directPermissionCodes, overview.permissions).filter((code) => !directPermissionCodes.includes(code));
    const directPermissions = directPermissionCodes.map((code) => overview.permissions.find((permission) => permission.code === code) ?? { code, nameAr: code, nameEn: code });
    const automaticPermissions = automaticPermissionCodes.map((code) => overview.permissions.find((permission) => permission.code === code) ?? { code, nameAr: code, nameEn: code });
    const isExpanded = expandedRoleId === role.id;
    const permissionDetailsId = `administration-role-permissions-${role.id}`;
    return <article key={role.id}><BaseerButton variant="quiet" className="administration-role-card__summary" type="button" aria-expanded={isExpanded} aria-controls={permissionDetailsId} onClick={() => setExpandedRoleId((current) => current === role.id ? null : role.id)}><span className={role.isSystem ? "administration-role-card__system" : "administration-role-card__custom"}>{role.isSystem ? text.defaultRole : text.customRole}</span><strong>{displayName(language, role)}</strong><small>{language === "ar" ? role.nameEn : role.nameAr}</small><p>{directPermissionCodes.length} {text.permissions}</p><span>{isExpanded ? text.hideRolePermissions : text.showRolePermissions}</span></BaseerButton>{isExpanded && <section id={permissionDetailsId} className="administration-role-card__permissions" aria-label={text.directRolePermissions}><strong>{text.directRolePermissions}</strong><ul>{directPermissions.map((permission) => <li key={permission.code}>{displayName(language, permission)}</li>)}</ul>{automaticPermissions.length ? <><strong>{text.automaticRolePermissions}</strong><ul>{automaticPermissions.map((permission) => <li key={permission.code}>{displayName(language, permission)}</li>)}</ul></> : null}</section>}{owner && !role.isSystem && <BaseerButton variant="secondary" type="button" onClick={() => startEdit(role)}>{text.editPermissions}</BaseerButton>}</article>;
  })}</div></section>;
}
