...
function MarketingReputationSummary({ data, copy, ar }: { data: MarketingWorkspaceRead | undefined; copy: MarketingCopy; ar: boolean }) {
  ...
  return <section className="marketing-reputation"><header><h3>{copy.reputation}</h3><p>{copy.notReady}</p></header><div className="baseer-card-grid">{(data?.readiness ?? []).map((item) => { const selected = item.status === "AUTHORIZED_READ_ONLY_SELECTED"; return <BaseerCard key={item.provider}><strong>{item.provider === "GOOGLE_ADS" ? summaryCopy.ads : summaryCopy.business}</strong><span className={`baseer-status-badge ${selected ? "baseer-status-badge--info" : "baseer-status-badge--warning"}`}><i className="baseer-status-badge__dot" />{selected ? (ar ? "متصل للقراءة فقط" : "Connected read-only") : copy.notConnected}</span><p>{ar ? item.messageAr : selected ? "Google Business is connected with one selected location for read-only preparation. Review synchronization, replies, and publishing remain off." : item.provider === "GOOGLE_ADS" ? summaryCopy.adsMessage : summaryCopy.businessMessage}</p></BaseerCard>; })}</div><BaseerCard tone="muted"><strong>{copy.noWrite}</strong><p>{copy.publisherUnavailable}</p></BaseerCard></section>;
}
