...
export function BasiraInterpretationCenter({ language, canRead, canWrite }: { language: Language; canRead: boolean; canWrite: boolean }) {
  ...
  return <section className="basira-interpretation-center" aria-busy={detailBusy || commandBusy}>
    <header className="basira-interpretation-center__header">
      ...
      <label className="basira-interpretation-center__filter"><span>{language === "ar" ? "المصدر" : "Source"}</span><select value={filter} onChange={(event) => setFilter(event.target.value as typeof filter)}><option value="ALL">{language === "ar" ? "الكل" : "All"}</option><option value="DECISION_ALERT">{subjectLabel("DECISION_ALERT", language)}</option><option value="MARKETING_CAMPAIGN">{subjectLabel("MARKETING_CAMPAIGN", language)}</option></select></label>
    </header>
