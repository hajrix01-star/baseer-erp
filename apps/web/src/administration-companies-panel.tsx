...
function CompanyDialog({ session, company, owner, onDone, onError, onClose }: { session: ActiveSession; company?: Company; owner: boolean; onDone: () => Promise<void>; onError: (error: unknown) => void; onClose: () => void }) {
  ...
  const changeStatus = async () => { if (!company || !owner || reason.trim().length < 3) return; setBusy(true); try { await updateAdministrationCompanyStatus(session, company.id, company.status === "ACTIVE" ? "ARCHIVED" : "ACTIVE", reason); await onDone(); onClose(); } catch (error) { onError(error); } finally { setBusy(false); } };
  ...
  return <div className="daily-sales-dialog-backdrop" role="presentation" onMouseDown={() => !busy && onClose()}><section ref={dialogRef} className="daily-sales-dialog administration-company-dialog" role="dialog" aria-modal="true" aria-label={title} onMouseDown={(event) => event.stopPropagation()}><header className="daily-sales-dialog__header"><div><p className="eyebrow">إدارة الشركات</p><h3>{title}</h3><p>{isCreate ? "أدخل اسم الشركة بالعربية والإنجليزية لإنشائها." : "تُسجّل كل التعديلات في سجل المراجعة."}</p></div><button className="dialog-icon-button" type="button" aria-label="إغلاق" disabled={busy} onClick={onClose}>×</button></header><form className="administration-dialog-form" onSubmit={(event) => void save(event)}><div className="administration-company-editor-profile">{logoUrl ? <img alt={`شعار ${nameAr || "الشركة"}`} src={logoUrl} /> : <span>{nameAr.trim().slice(0, 1) || "ش"}</span>}<div><strong>{nameAr || "اسم الشركة"}</strong><small>{isCreate ? "شركة جديدة" : company.status === "ACTIVE" ? "شركة نشطة" : "شركة مؤرشفة"}</small></div></div><label>اسم الشركة بالعربية<input required value={nameAr} onChange={(event) => setNameAr(event.target.value)} autoFocus /></label><label>Company name in English<input required value={nameEn} onChange={(event) => setNameEn(event.target.value)} /></label><label>المنطقة الزمنية<select value={businessTimezone} onChange={(event) => setBusinessTimezone(event.target.value)}><option value="Asia/Riyadh">Asia/Riyadh</option></select></label>{!isCreate && owner && <label className="administration-company-logo-upload">رفع الشعار<input accept="image/png,image/jpeg,image/webp" disabled={busy} type="file" onChange={(event) => void selectLogo(event)} /><span>PNG أو JPG أو WebP · حتى 512 كيلوبايت</span></label>}{!isCreate && owner && <fieldset className="administration-company-status-action"><legend>{company.status === "ACTIVE" ? "أرشفة الشركة" : "إعادة تفعيل الشركة"}</legend><label>سبب التغيير<input minLength={3} value={reason} onChange={(event) => setReason(event.target.value)} placeholder="اكتب سبباً مختصراً" /></label><button className={company.status === "ACTIVE" ? "daily-sales-danger" : "daily-sales-secondary"} disabled={busy || reason.trim().length < 3} type="button" onClick={() => void changeStatus()}>{company.status === "ACTIVE" ? "أرشفة الشركة" : "إعادة التفعيل"}</button></fieldset>}<footer><button className="daily-sales-secondary" disabled={busy} type="button" onClick={onClose}>إلغاء</button>{owner && <button className="daily-sales-primary" disabled={busy}>{busy ? "جارٍ الحفظ…" : isCreate ? "إنشاء الشركة" : "حفظ التعديلات"}</button>}</footer></form></section></div>;
}