...
const newRow = (): BatchRow => ({ id: requestId(), kind: "PURCHASE", settlementKind: "PAID", categoryId: "", supplierId: "", invoiceNumber: "", missingReason: "", supplierInvoiceDate: "", grossAmount: "", isTaxable: true, vaultId: "", notes: "" });
const initialRows = () => Array.from({ length: 3 }, newRow);
const rowHasValue = (row: BatchRow) => Boolean(row.categoryId || row.supplierId || row.invoiceNumber || row.missingReason || row.supplierInvoiceDate || row.grossAmount || row.vaultId || row.notes);
