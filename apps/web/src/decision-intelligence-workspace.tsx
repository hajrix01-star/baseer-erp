...
export function DecisionIntelligenceWorkspace({ language, section }: { language: Language; section: number }) {
  ...
  const sectionContent = useMemo(() => {
    ...
    if (section === 4) return <SourcesPolicyPanel language={language} candidates={candidates} reviews={reviews} canManage={canManageGlobalContext} researchBusy={researchBusy} onResearch={async () => {
      const current = activeSession(); if (!current) return;
  ...
  }, [alerts, candidates, canGiveFeedback, canManageAlerts, canManageCompanyContext, canManageGlobalContext, canManagePolicies, companyEvents, comparison, events, globalEvents, language, load, matchedWeekdayComparison, metric, openAlerts, period.from, period.to, researchBusy, reviews, salesChangePolicy, section]);
