...
export function DecisionIntelligenceWorkspace({ language, section, permissionCodes }: { language: Language; section: number; permissionCodes: readonly string[] | null }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      // Permission discovery, session refresh and manual refresh can overlap.
      // Only the newest read may change screen state; a stale rejected request
      // must never leave a red error banner over successfully loaded evidence.
  ...
  return <section className="baseer-workspace decision-workspace" aria-busy={loading}>
    ...
    <BaseerFormDialog open={eventDialogOpen} title={companyEventEditing ? (language === "ar" ? "تعديل حدث الشركة" : "Edit company event") : (language === "ar" ? "إضافة حدث للشركة" : "Add company event")} language={language} formId="decision-company-event" submitLabel={language === "ar" ? "حفظ الحدث" : "Save event"} busy={eventBusy} error={eventError} size="standard" className="decision-context-dialog" onClose={() => !eventBusy && setEventDialogOpen(false)}>
      <form id="decision-company-event" className="baseer-form" onSubmit={(event) => void createEvent(event)}>
        <BaseerFormSection title={language === "ar" ? "تفاصيل حدث الشركة" : "Company event details"} description={language === "ar" ? "يظهر لهذا الكيان فقط كسياق للمراجعة، ولا يعدّل المبيعات." : "Visible to this company only as review context; it never changes sales."}><BaseerFormGrid>
          ...
          <label className="baseer-form-field--full">{language === "ar" ? "العنوان" : "Title"}<input required minLength={2} maxLength={240} value={eventForm.titleAr} onChange={(event) => setEventForm((value) => ({ ...value, titleAr: event.target.value }))} /></label>
          <label>{language === "ar" ? "من" : "From"}<input required type="date" value={eventForm.startsOn} onChange={(event) => setEventForm((value) => ({ ...value, startsOn: event.target.value }))} /></label><label>{language === "ar" ? "إلى" : "To"}<input required min={eventForm.startsOn} type="date" value={eventForm.endsOn} onChange={(event) => setEventForm((value) => ({ ...value, endsOn: event.target.value }))} /></label>
          <label className="baseer-form-field--full">{language === "ar" ? "مرجع أو ملاحظة المصدر" : "Source reference or note"}<input maxLength={500} value={eventForm.sourceReference} onChange={(event) => setEventForm((value) => ({ ...value, sourceReference: event.target.value }))} /></label>
    ...
    <BaseerFormDialog open={globalEventDialogOpen} title={globalEventEditing ? (language === "ar" ? "تعديل مناسبة عامة" : "Edit global event") : (language === "ar" ? "إضافة مناسبة عامة" : "Add global event")} language={language} formId="decision-global-event" submitLabel={language === "ar" ? "حفظ المناسبة" : "Save event"} busy={globalEventBusy} error={globalEventError} size="standard" className="decision-context-dialog" onClose={() => !globalEventBusy && setGlobalEventDialogOpen(false)}>
      <form id="decision-global-event" className="baseer-form" onSubmit={(event) => void submitGlobalEvent(event)}>
        <BaseerFormSection title={language === "ar" ? "تفاصيل المناسبة العامة" : "Global event details"} description={language === "ar" ? "تظهر لكل الشركات كسياق زمني، ولا تثبت سبباً للمبيعات." : "Shown to every company as time context; it does not prove a sales cause."}><BaseerFormGrid>
          ...
          <label className="baseer-form-field--full">{language === "ar" ? "العنوان" : "Title"}<input required minLength={2} maxLength={240} value={globalEventForm.titleAr} onChange={(event) => setGlobalEventForm((value) => ({ ...value, titleAr: event.target.value }))} /></label>
          ...
          <label className="baseer-form-field--full">{language === "ar" ? "مرجع أو ملاحظة" : "Reference or note"}<input maxLength={500} value={globalEventForm.sourceReference} onChange={(event) => setGlobalEventForm((value) => ({ ...value, sourceReference: event.target.value }))} /></label>
        </BaseerFormGrid></BaseerFormSection>
        <BaseerFormSection title={language === "ar" ? "سجل القرار" : "Decision record"} description={language === "ar" ? "يحفظ السبب مع إصدار المناسبة للمراجعة المستقبلية." : "The reason is retained with this event revision for later review."}><BaseerFormGrid columns="one"><label>{language === "ar" ? "سبب التسجيل أو التعديل" : "Reason for this change"}<BaseerTextArea required minLength={3} maxLength={500} compact value={globalEventForm.reason} onValueChange={(reason) => setGlobalEventForm((value) => ({ ...value, reason }))} /></label></BaseerFormGrid></BaseerFormSection>
      </form>
    </BaseerFormDialog>
