...
export function BaseerPeriodFilter({ language, value, onChange, presets = ["DAY", "MONTH", "MULTI_MONTH", "QUARTER", "YEAR", "RANGE"], className }: Props) {
  ...
  return <section className={["baseer-period-filter", className].filter(Boolean).join(" ")} aria-label={language === "ar" ? "فلترة الفترة" : "Period filter"}>
    <div className="baseer-period-filter__bar">
      <button className="baseer-period-filter__trigger" type="button" aria-expanded={open} onClick={() => setOpen((current) => !current)}><span className="baseer-period-filter__calendar" aria-hidden="true">▦</span><span className="baseer-period-filter__label">{language === "ar" ? "الفترة" : "Period"}</span><strong>{display(value, language)}</strong></button>
