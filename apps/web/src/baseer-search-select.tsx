...
export function BaseerSearchSelect({ id, label, value, options, placeholder, disabled, required, className, searchable = true, onChange }: { id?: string; label: string; value: string; options: readonly BaseerSearchOption[]; placeholder: string; disabled?: boolean; required?: boolean; className?: string; searchable?: boolean; onChange: (value: string) => void }) {
  ...
  const menu = open ? <div ref={menuRef} id={`${inputId}-options`} className="company-session-control__menu" style={menuStyle} role="listbox" aria-label={label}>{matches.length ? matches.map((option) => <button key={option.id} type="button" role="option" aria-selected={option.id === value} className={option.id === value ? "is-active" : undefined} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: ".5rem" }} onClick={() => choose(option)}><span>{option.label}</span>{option.isFavorite ? <span aria-hidden="true" style={{ color: "var(--brand)", lineHeight: 1 }}>★</span> : null}</button>) : <p className="empty-results">{placeholder}</p>}</div> : null;
