 ...
 */
export function BaseerSearchSelect({ id, label, value, options, placeholder, disabled, required, className, menuClassName, searchable = true, remoteSearch, scopeKey, onChange }: { id?: string; label: string; value: string; options: readonly BaseerSearchOption[]; placeholder: string; disabled?: boolean; required?: boolean; className?: string; menuClassName?: string; searchable?: boolean; remoteSearch?: (query: string, signal: AbortSignal) => Promise<readonly BaseerSearchOption[]>; /** Required for every new remote selector; changes clear the selected value and abort in-flight search. */ scopeKey?: string; onChange: (value: string) => void }) {
  ...
  return <Suspense fallback={<input id={id} className={className} aria-label={label} placeholder={placeholder} disabled={disabled} required={required} readOnly value={options.find((option) => option.id === value)?.label ?? ""} />}><LazyBaseerCombobox
    ...
    remoteSearch={remoteSearch ? (query, signal) => remoteSearch(query, signal) : undefined}
