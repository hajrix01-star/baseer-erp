...
const copy = {
  ar: {
    ...
    description: "محاكاة محفوظة فقط؛ لا تنشئ سدادًا أو قيدًا أو إرسالًا خارجيًا.",
  ...
  en: {
    ...
    description: "A saved simulation only; it never creates a payment, journal entry, or external submission.",
...
export function VatSimulationWorkspace({ language }: { language: Language }) {
  ...
  return <BaseerWorkspace className="vat-simulation-workspace"><div className="vat-simulation-workspace__content" dir={language === "ar" ? "rtl" : "ltr"}>
    ...
    <BaseerCard className="vat-simulation-workspace__scope" padding="compact">
    ...
    <BaseerDialog open={editorOpen && !!companyId} title={selected ? `Q${quarter} · ${year}` : text.add} language={language} busy={busy !== null} onClose={() => { setDeleteConfirmOpen(false); setEditorOpen(false); }} size="wide" className="vat-simulation-workspace__dialog" footer={<>{selected ? <BaseerButton type="button" variant="danger" disabled={busy !== null} onClick={() => setDeleteConfirmOpen(true)}>{busy === "delete" ? text.deleting : text.delete}</BaseerButton> : null}<BaseerButton type="button" variant="primary" disabled={busy !== null} onClick={() => void save()}>{busy === "save" ? text.saving : text.save}</BaseerButton></>}>
      <section className="vat-simulation-workspace__editor">
        <header className="vat-simulation-workspace__editor-header"><h3>Q{quarter} · {year}</h3></header>
        <div className="vat-simulation-workspace__editor-columns"><section className="vat-simulation-workspace__input-panel"><header><h4>{text.inputs}</h4><div className="vat-simulation-workspace__editor-actions"><label><span>{text.quarter}</span><BaseerStaticSelect label={text.quarter} value={quarter} onChange={(event) => setQuarter(Number(event.target.value))}>{([1, 2, 3, 4] as const).map((item) => <option key={item} value={item}>Q{item}</option>)}</BaseerStaticSelect></label><BaseerButton type="button" variant="secondary" className="vat-simulation-workspace__import-button" disabled={busy !== null} onClick={() => void importReport()}>{busy === "import" ? text.importing : text.import}</BaseerButton></div></header><div className="vat-simulation-workspace__form"><div className="vat-simulation-workspace__amount-grid"><label className="vat-simulation-workspace__amount-card"><span>{text.sales}</span><small>{text.beforeVat}</small><BaseerMoneyInput value={draft.salesTaxableAmount} onValueChange={(value) => setTaxable("salesTaxableAmount", value)} /></label><label className="vat-simulation-workspace__amount-card"><span>{text.purchases}</span><small>{text.beforeVat}</small><BaseerMoneyInput value={draft.purchasesTaxableAmount} onValueChange={(value) => setTaxable("purchasesTaxableAmount", value)} /></label></div><label className="vat-simulation-workspace__target"><span>{text.target}</span><BaseerMoneyInput value={draft.paymentTarget ?? "0"} onValueChange={(value) => setDraft((current) => ({ ...current, paymentTarget: value || null }))} /></label></div></section><section className="vat-simulation-workspace__result-panel"><header><h4>{text.results}</h4></header><dl className="vat-simulation-workspace__result-list"><div><dt>{text.sales} · {text.vatAmount}</dt><dd dir="ltr">{formatMoney(numberOf(draft.outputVatAmount), "SAR", language)}</dd></div><div><dt>{text.purchases} · {text.vatAmount}</dt><dd dir="ltr">{formatMoney(numberOf(draft.inputVatAmount), "SAR", language)}</dd></div><div><dt>{text.net}</dt><dd dir="ltr">{formatMoney(net, "SAR", language)}</dd></div></dl><dl className="vat-simulation-workspace__summary"><div className="vat-simulation-workspace__summary-primary"><dt>{text.additionalPurchases}</dt><dd dir="ltr">{formatMoney(additionalPurchases, "SAR", language)}</dd></div><div><dt>{text.totalPurchases}</dt><dd dir="ltr"><button type="button" className="vat-simulation-workspace__copy-value" onClick={() => void copyTotalPurchases()} aria-label={text.copyValue}><span>{formatMoney(totalRequiredPurchases, "SAR", language)}</span><small>{copiedValue === cleanTotalPurchases ? text.copied : text.copyValue}</small></button></dd></div></dl></section></div>
      </section>
    </BaseerDialog>
