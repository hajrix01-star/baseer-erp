...
export type PurchaseCreditWorkspace = { companyId: string; asOfBusinessDate: string; openSupplierCount: number; openInvoiceCount: number; originalAmount: string; paidAmount: string; remainingAmount: string; suppliers: Array<{ supplierId: string; supplierNameAr: string; supplierNameEn: string | null; invoiceCount: number; originalAmount: string; paidAmount: string; remainingAmount: string; dues: Array<{ id: string; documentNumber: string; kind: "PURCHASE" | "EXPENSE"; businessDate: string; dueDate: string | null; categoryNameAr: string | null; categoryNameEn: string | null; originalAmount: string; paidAmount: string; remainingAmount: string }> }>; hasMore: boolean; nextCursor: string | null };
type CreditPaymentForm = { vaultId: string; amount: string; businessDate: string };

export function PurchaseExpenseCreditPanel({ credit, language, vaults, reload }: { credit: PurchaseCreditWorkspace | null; language: "ar" | "en"; vaults: ReadonlyArray<{ id: string; nameAr: string; nameEn: string }>; reload: (cursor?: string) => Promise<void> }) {
  const text = financeText(language);
  const [target, setTarget] = useState<PurchaseCreditWorkspace["suppliers"][number]["dues"][number] | null>(null);
...
function CreditPaymentDialog({ language, due, vaults, defaultBusinessDate, onClose, onSaved }: { language: "ar" | "en"; due: PurchaseCreditWorkspace["suppliers"][number]["dues"][number] | null; vaults: ReadonlyArray<{ id: string; nameAr: string; nameEn: string }>; defaultBusinessDate: string; onClose: () => void; onSaved: () => Promise<void> }) {
  ...
  if (!due) return null;
  const submit = async (value: CreditPaymentForm) => { const current = activeSession(); if (!current || saving) return; setSaving(true); setError(""); try { await api(current, "/finance/supplier-dues/payments", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ dueId: due.id, vaultId: value.vaultId, businessDate: value.businessDate, amount: value.amount, idempotencyKey: requestId() }) }); await onSaved(); } catch (requestError) { setError(presentBaseerApiError(requestError, language, text.recordSettlement)); } finally { setSaving(false); } };
  return <div className="daily-sales-dialog-backdrop" role="presentation" onMouseDown={() => !saving && onClose()}><section ref={dialogRef} className="daily-sales-dialog" role="dialog" aria-modal="true" aria-labelledby="credit-payment-dialog-title" onMouseDown={(event) => event.stopPropagation()}><header className="daily-sales-dialog__header"><div><h3 id="credit-payment-dialog-title">{text.recordSettlement}</h3><p>{due.documentNumber} · {formatMoney(due.remainingAmount)}</p></div><button className="dialog-icon-button" type="button" aria-label={text.cancel} disabled={saving} onClick={onClose}>×</button></header><form className="daily-sales-dialog__form" data-baseer-rhf-form="true" noValidate onSubmit={form.handleSubmit((value) => void submit(value))}><label>{text.paymentDate}<BaseerDatePicker language={language} label={text.paymentDate} max={defaultBusinessDate} value={businessDate} onChange={(value) => form.setValue("businessDate", value, { shouldDirty: true, shouldValidate: true })} /></label><label>{text.paymentChannel}<select aria-invalid={Boolean(form.formState.errors.vaultId)} {...form.register("vaultId")}><option value="">{text.selectVault}</option>{vaults.map((vault) => <option key={vault.id} value={vault.id}>{displayName(language, vault)}</option>)}</select>{form.formState.errors.vaultId ? <small role="alert">{form.formState.errors.vaultId.message}</small> : null}</label><label>{text.repaymentAmount} (SAR)<input inputMode="decimal" aria-invalid={Boolean(form.formState.errors.amount)} {...form.register("amount")} />{form.formState.errors.amount ? <small role="alert">{form.formState.errors.amount.message}</small> : null}</label>{error ? <p className="daily-sales-message error">{error}</p> : null}<footer className="daily-sales-dialog__actions"><BaseerButton variant="primary" disabled={saving}>{saving ? text.saving : text.recordSettlement}</BaseerButton></footer></form></section></div>;
}
