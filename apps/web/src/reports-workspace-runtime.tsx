...
type ReportMetadata = Readonly<{ selectedPeriod: { from: string; to: string; months?: readonly string[] }; basisLabelAr: string; company: { displayName: string; functionalCurrency: string } }>;
type ReportResult =
...
type EvidenceReceipt = Readonly<{ rowCode: string; nextCursor: string | null; items: readonly EvidenceItem[] }>;
...
function PersonalCashPerformanceWorkspace({ language }: { language: Language }) {
  ...
  return <BaseerCompanyReadQuery session={session} resource="reports.personal-cash-performance" scope={[language, period.preset, period.from, period.to, period.months.join(','), String(vatInclusive)]} mode="live" load={(current, signal) => api<ReportResult>(current, `/reports/personal-cash-performance?${query.toString()}`, { signal })}>
...
function PersonalCashPerformanceContent({ language, session, period, setPeriod, vatInclusive, setVatInclusive, report, loading, loadError, refetch }: { language: Language; session: ActiveSession; period: BaseerPeriodRange; setPeriod: (value: BaseerPeriodRange) => void; vatInclusive: boolean; setVatInclusive: (value: boolean) => void; report: ReportResult | null; loading: boolean; loadError: unknown; refetch: () => Promise<void> }) {
  ...
  const openEvidence = async (row: ReportRow, cursor?: string) => {
    ...
    try {
      const query = new URLSearchParams({ from: period.from, to: period.to, vatInclusive: String(vatInclusive), rowCode: row.code, ...(period.preset === 'MONTH' && period.months.length > 1 ? { months: period.months.join(',') } : {}), ...(cursor ? { cursor } : {}) });
      const next = await api<EvidenceReceipt>(session, `/reports/personal-cash-performance/live/evidence?${query.toString()}`, { signal: controller.signal });
  ...
  const openSource = async (eventId: string) => {
    ...
    const epoch = ++readEpoch.current; readAbort.current?.abort(); const controller = new AbortController(); readAbort.current = controller;
    setEvidenceBusy(true);
    try { const query = new URLSearchParams({ from: period.from, to: period.to, vatInclusive: String(vatInclusive), ...(period.preset === 'MONTH' && period.months.length > 1 ? { months: period.months.join(',') } : {}) }); const next = await api<SourceReceipt>(session, `/reports/personal-cash-performance/live/evidence/${eventId}/source?${query.toString()}`, { signal: controller.signal }); if (!controller.signal.aborted && epoch === readEpoch.current && activeSession()?.companyId === session.companyId && activeSession()?.sessionExpiresAt === session.sessionExpiresAt) setSource(next); }
...
export function ReportsWorkspaceRuntime({ language, initialReport }: { language: Language; initialReport?: 'trial-balance' | 'cash-performance' }) {
