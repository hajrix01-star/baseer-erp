...
export function HrPayrollWorkspace({ language }: { language: Language }) {
  ...
  const saveSalary = async (event: React.FormEvent) => { event.preventDefault(); const current = activeSession(); if (!current || busy) return; setBusy(true); try { await setHrEmployeeCompensation(current, { ...salary, notes: salary.notes || undefined, idempotencyKey: requestId() }); setSalaryOpen(false); setMessage(ar ? "تم حفظ الراتب الشهري؛ سيظهر في المسير من تاريخ سريانه." : "Monthly compensation saved; it will appear in payroll from its effective date."); } catch (error) { setMessage(presentBaseerApiError(error, language, ar ? "حفظ الراتب" : "Saving compensation")); } finally { setBusy(false); } };
