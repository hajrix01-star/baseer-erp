...
export function FinanceSetupWorkspaceContent({ language, view = "setup" }: { language: Language; view?: FinanceSetupView }) {
  ...
  return <section className="finance-setup-workspace">
    <BaseerCard>
      <div className="baseer-workspace__heading"><div><p className="overline">{text.eyebrow}</p><h2>{text.title}</h2></div><div className="baseer-workspace__actions"><BaseerButton type="button" onClick={() => setOpenRuntime(true)}>{text.open}</BaseerButton></div></div>
