...
export function MarketingCalendarWorkspaceRuntime({ language, session, embedded = false }: { language: MarketingLanguage; session: ActiveSession; embedded?: boolean }) {
  const ar = marketingIsArabic(language);
  return <section className="baseer-workspace marketing-workspace" dir={ar ? "rtl" : "ltr"}><MarketingCalendarWorkspace language={language} session={session} embedded={embedded} /></section>;
}

function MarketingCalendarWorkspace({ language, session, embedded }: { language: MarketingLanguage; session: ActiveSession; embedded: boolean }) {
