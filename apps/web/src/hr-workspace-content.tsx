    ...
    {section === 0 ? (
      ...
      <>
        ...
        {loading ? <BaseerCard><p>{text.loading}</p></BaseerCard> : isAdvance ? (
          <>
            {advanceLoadError ? <BaseerNotice tone="danger" title={language === "ar" ? "تعذر تحميل السلف" : "Unable to load advances"}>{advanceLoadError} <BaseerButton type="button" variant="quiet" onClick={() => void load()}>{language === "ar" ? "إعادة المحاولة" : "Retry"}</BaseerButton></BaseerNotice> : advanceGroups.length ? <section className="hr-advance-groups" aria-label={language === "ar" ? "السلف مجمعة حسب الموظف" : "Advances grouped by employee"}><div className="hr-advance-groups__header" aria-hidden="true"><span>{text.employeeName}</span><span>{language === "ar" ? "عدد السلف" : "Advance count"}</span><span>{language === "ar" ? "إجمالي السلف" : "Total advances"}</span><span>{language === "ar" ? "المسدّد" : "Settled"}</span><span>{text.remainingAmount}</span><span>{language === "ar" ? "التفاصيل" : "Details"}</span></div>{advanceGroups.map((group) => { const employeeName = language === "ar" ? group.employeeNameAr : group.employeeNameEn ?? group.employeeNameAr; return <details key={group.employeeId} className="hr-advance-group"><summary><span className="hr-advance-group__employee"><strong>{employeeName}</strong><small>{group.openCount ? (language === "ar" ? `${group.openCount} مفتوحة` : `${group.openCount} open`) : (language === "ar" ? "كلها مسددة" : "All settled")}</small></span><span><bdi>{group.advances.length}</bdi></span><span><BaseerMoney value={group.originalAmount} language={language} /></span><span><BaseerMoney value={group.settledAmount} language={language} /></span><span><BaseerMoney value={group.remainingAmount} language={language} /></span><b className="hr-advance-group__expand">{language === "ar" ? "عرض السلف" : "View advances"}</b></summary><div className="hr-advance-group__details"><BaseerDataGrid ariaLabel={`${text.advances} · ${employeeName}`} caption={`${text.advances} · ${employeeName}`} columns={advanceColumns} rows={group.advances} rowKey={(row) => row.id} /></div></details>; })}</section> : <BaseerEmptyState title={language === "ar" ? "لا توجد سلف مسجلة" : "No advances recorded"} />}
