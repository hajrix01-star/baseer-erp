...
type AdvanceGroup = { employeeId: string; employeeNameAr: string; employeeNameEn: string | null; advances: HrAdvance[]; originalAmount: string; settledAmount: string; remainingAmount: string; openCount: number };
...
function groupAdvancesByEmployee(advances: readonly HrAdvance[]): AdvanceGroup[] {
  const groups = new Map<string, { employeeId: string; employeeNameAr: string; employeeNameEn: string | null; advances: HrAdvance[] }>();
  for (const advance of advances) {
    const group = groups.get(advance.employeeId) ?? { employeeId: advance.employeeId, employeeNameAr: advance.employeeNameAr, employeeNameEn: advance.employeeNameEn, advances: [] };
    group.advances.push(advance);
    groups.set(advance.employeeId, group);
  }
  return [...groups.values()].map((group) => ({
    ...group,
    originalAmount: sumMoneyDecimals(group.advances.map((advance) => advance.originalAmount)),
    settledAmount: sumMoneyDecimals(group.advances.map((advance) => advance.settledAmount)),
    remainingAmount: sumMoneyDecimals(group.advances.map((advance) => advance.remainingAmount)),
    openCount: group.advances.filter((advance) => advance.remainingAmount !== "0.0000").length,
  })).sort((left, right) => left.employeeNameAr.localeCompare(right.employeeNameAr, "ar"));
}
...
export function HrWorkspaceCore({ language, section, stage }: { language: Language; section: number; stage?: string | null }) {
  ...
  const [expandedAdvanceEmployeeIds, setExpandedAdvanceEmployeeIds] = useState<ReadonlySet<string>>(() => new Set());
  ...
  const advanceGroups = useMemo(() => groupAdvancesByEmployee(visibleAdvances), [visibleAdvances]);
  ...
  return <BaseerWorkspace className={section === 1 ? "hr-employees-workspace" : undefined} aria-label={text.title}>
    ...
    {section === 0 ? (
      ...
      <>
        <BaseerFilterBar className={isAdvance ? "hr-advances-filter-bar" : undefined} controlsPresentation={section === 1 ? "menu" : "inline"} language={language} search={search} searchLabel={sectionTitle} searchPlaceholder={isAdvance ? (language === "ar" ? "ابحث برقم السلفة أو اسم الموظف" : "Search by advance number or employee") : sectionTitle} onSearchChange={setSearch} controls={section === 1 ? <BaseerFilterSelect label={language === "ar" ? "نوع الموظف" : "Employee status"} value={employeeStatusFilter} onChange={(event) => setEmployeeStatusFilter(event.target.value as HrEmployeeStatus | "ALL")}>{employeeStatusOptions.map((option) => <option key={option.id} value={option.id}>{option.label}</option>)}</BaseerFilterSelect> : isAdvance ? <BaseerCombobox className="hr-advances-filter-bar__employee" label={text.selectEmployee} value={employeeFilter} placeholder={language === "ar" ? "فلترة حسب الموظف" : "Filter by employee"} options={activeEmployees.map((item) => ({ id: item.id, label: `${item.employeeNumber} · ${language === "ar" ? item.nameAr : (item.nameEn ?? item.nameAr)}` }))} remoteSearch={searchEmployeeOptions} scopeKey={session?.companyId ?? "signed-out"} onChange={setEmployeeFilter} /> : undefined} appliedFilters={[...(search ? [{ id: "q", label: search, onRemove: () => setSearch("") }] : []), ...(section === 1 && employeeStatusFilter !== "ACTIVE" ? [{ id: "employee-status", label: employeeStatusOptions.find((item) => item.id === employeeStatusFilter)?.label ?? employeeStatusFilter, onRemove: () => setEmployeeStatusFilter("ACTIVE") }] : []), ...(employeeFilter ? [{ id: "employee", label: activeEmployees.find((item) => item.id === employeeFilter)?.nameAr ?? employeeFilter, onRemove: () => setEmployeeFilter("") }] : [])]} onClear={() => { setSearch(""); setEmployeeFilter(""); setEmployeeStatusFilter("ACTIVE"); }} />
        {loading ? <BaseerCard><p>{text.loading}</p></BaseerCard> : isAdvance ? (
          <>
            {advanceLoadError ? <BaseerNotice tone="danger" title={language === "ar" ? "تعذر تحميل السلف" : "Unable to load advances"}>{advanceLoadError} <BaseerButton type="button" variant="quiet" onClick={() => void load()}>{language === "ar" ? "إعادة المحاولة" : "Retry"}</BaseerButton></BaseerNotice> : advanceGroups.length ? <section className="hr-advance-groups" aria-label={language === "ar" ? "السلف مجمعة حسب الموظف" : "Advances grouped by employee"}><div className="hr-advance-groups__header" aria-hidden="true"><span>{text.employeeName}</span><span>{language === "ar" ? "عدد السلف" : "Advance count"}</span><span>{language === "ar" ? "إجمالي السلف" : "Total advances"}</span><span>{language === "ar" ? "المسدّد" : "Settled"}</span><span>{text.remainingAmount}</span><span>{language === "ar" ? "التفاصيل" : "Details"}</span></div>{advanceGroups.map((group) => { const employeeName = language === "ar" ? group.employeeNameAr : group.employeeNameEn ?? group.employeeNameAr; const expanded = expandedAdvanceEmployeeIds.has(group.employeeId); return <article key={group.employeeId} className="hr-advance-group"><button type="button" aria-expanded={expanded} aria-controls={`advance-group-${group.employeeId}`} onClick={() => setExpandedAdvanceEmployeeIds((current) => { const next = new Set(current); if (next.has(group.employeeId)) next.delete(group.employeeId); else next.add(group.employeeId); return next; })}><span className="hr-advance-group__employee"><strong>{employeeName}</strong><small>{group.openCount ? (language === "ar" ? `${group.openCount} مفتوحة` : `${group.openCount} open`) : (language === "ar" ? "كلها مسددة" : "All settled")}</small></span><span><bdi>{group.advances.length}</bdi></span><span><BaseerMoney value={group.originalAmount} language={language} /></span><span><BaseerMoney value={group.settledAmount} language={language} /></span><span><BaseerMoney value={group.remainingAmount} language={language} /></span><b className="hr-advance-group__expand">{expanded ? (language === "ar" ? "إخفاء السلف" : "Hide advances") : (language === "ar" ? "عرض السلف" : "View advances")}</b></button>{expanded ? <div id={`advance-group-${group.employeeId}`} className="hr-advance-group__details"><BaseerDataGrid ariaLabel={`${text.advances} · ${employeeName}`} caption={`${text.advances} · ${employeeName}`} columns={advanceColumns} rows={group.advances} rowKey={(row) => row.id} /></div> : null}</article>; })}</section> : <BaseerEmptyState title={language === "ar" ? "لا توجد سلف مسجلة" : "No advances recorded"} />}
