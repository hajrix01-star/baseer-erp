...
export function AttendanceEmployeePortal({ language: initialLanguage }: { language: Language }) {
  ...
  return <main className="attendance-pwa" dir={ar ? "rtl" : "ltr"}>
    <section className="attendance-pwa__card" aria-live="polite">
      ...
      </> : <><h1>{ar ? "الحضور والانصراف" : "Attendance"}</h1><p>{ar ? "ادخل إلى حسابك بكودك الشخصي فقط، أو سجّل الحضور بخطوة واحدة: الكود ثم مسح QR." : "Open your account with your personal code only, or record attendance in one flow: code then QR scan."}</p><div className="attendance-pwa__entry-actions"><BaseerButton className="attendance-pwa__operation" type="button" onClick={() => startIntent("RECORD")}>{ar ? "تسجيل حضور أو انصراف" : "Check in or out"}</BaseerButton><BaseerButton type="button" variant="secondary" onClick={() => startIntent("ACCOUNT")}>{ar ? "دخول حسابي" : "Open my account"}</BaseerButton></div></>}
      {notice ? <p className="attendance-pwa__notice" role="alert">{notice}</p> : null}
    </section>
    <BaseerDialog open={intent !== null} title={intent === "RECORD" ? (ar ? "أدخل كودك ثم امسح QR" : "Enter your code, then scan QR") : (ar ? "دخول حسابي" : "Open my account")} language={language} className="attendance-pwa__pin-dialog" busy={busy} onClose={closeIntent} footer={<BaseerButton type="submit" form="attendance-employee-code" disabled={busy}>{busy ? (ar ? "جارٍ التحقق…" : "Verifying…") : intent === "RECORD" ? (ar ? "متابعة إلى الكاميرا" : "Continue to camera") : (ar ? "دخول" : "Open account")}</BaseerButton>}><form id="attendance-employee-code" className="attendance-pwa__pin-form" onSubmit={(event) => { event.preventDefault(); void openPortal(); }}><p>{ar ? "الكود خاص بك ولا يُحفظ على الجوال." : "Your code is personal and is not saved on this phone."}</p><label>{ar ? "الكود الشخصي" : "Personal code"}<BaseerTextInput dir="ltr" autoFocus autoComplete="one-time-code" inputMode="numeric" pattern="[0-9]*" type="password" maxLength={4} value={pin} onChange={(event) => setPin(normalizeBaseerNumericInput(event.target.value, { allowNegative: false }).replace(/\D/g, "").slice(0, 4))} disabled={busy} /></label></form></BaseerDialog>
