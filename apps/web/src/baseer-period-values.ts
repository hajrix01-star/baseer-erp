...
export type BaseerPeriodPreset = "ALL" | "DAY" | "MONTH" | "QUARTER" | "YEAR" | "RANGE";
...
/** `ALL` deliberately remains an ISO range, not empty parameters, so every
 * existing server-side period contract stays typed and company-scoped. */
export function baseerPeriodRange(preset: Exclude<BaseerPeriodPreset, "RANGE">): BaseerPeriodRange { const today = riyadhToday(); const todayIso = iso(today.year, today.month, today.day); const month = `${today.year}-${String(today.month).padStart(2, "0")}`; if (preset === "ALL") return { preset, from: "0001-01-01", to: todayIso, months: [] }; if (preset === "DAY") return { preset, from: todayIso, to: todayIso, months: [] }; if (preset === "MONTH") return { preset, from: iso(today.year, today.month, 1), to: todayIso, months: [month] }; if (preset === "YEAR") return { preset, from: iso(today.year, 1, 1), to: todayIso, months: [] }; const start = Math.floor((today.month - 1) / 3) * 3 + 1; return { preset, from: iso(today.year, start, 1), to: todayIso, months: [] }; }
