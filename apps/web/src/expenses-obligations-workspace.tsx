...
function ExpenseSettlementBatch({ language, configuration, profiles, businessDate: serverBusinessDate, reload }: { language: "ar" | "en"; configuration: Configuration; profiles: Profile[]; businessDate: string; reload: () => Promise<void> }) {
  ...
  return <div className="expenses-obligations-batches">
    <BaseerCard><form className="purchase-batch-form" onSubmit={(event) => void submit(event)}><div className="purchase-batch-header"><label>{text.batchDate}<BaseerDatePicker language={language} label={text.batchDate} max={serverBusinessDate} value={businessDate} onChange={setBusinessDate} /></label></div>{message.kind !== "idle" ? <p className={`daily-sales-message ${message.kind}`}>{message.text}</p> : null}
      ...
      <footer className="purchase-batch-footer"><span className="purchase-batch-total" aria-hidden="true" /><div><BaseerButton type="button" variant="secondary" onClick={() => setRows((current) => [...current, newExpenseRow()])}>{text.addRow}</BaseerButton><BaseerButton variant="primary" disabled={saving}>{saving ? text.saving : text.savePaymentCount(enteredRows.length)}</BaseerButton></div></footer>
    </form></BaseerCard>
