...
type Workspace = { companyId: string; campaigns: Campaign[]; readiness: Array<{ provider: "GOOGLE_ADS" | "GOOGLE_BUSINESS"; status: "NOT_CONNECTED"; messageAr: string }> };
...
export function MarketingWorkspaceContent({ language, section, permissionCodes }: { language: Language; section: number; permissionCodes: readonly string[] | null }) {
  ...
  return <BaseerCompanyReadQuery session={session} resource="marketing.workspace" scope={scope} load={(current, signal) => api<Workspace>(current, "/marketing", { signal })}>{({ data, loading, error, refetch }) => <section className="baseer-workspace marketing-workspace" dir={ar ? "rtl" : "ltr"}>
    ...
    {section === 2 ? <Reputation data={data} copy={copy} ar={ar} /> : null}
...
function CampaignRegister({ data, copy, ar, writable, onEdit, onArchive }: { data: Workspace | undefined; copy: Record<string, string>; ar: boolean; writable: boolean; onEdit: (campaign: Campaign) => void; onArchive: (campaign: Campaign) => void }) { const campaigns = data?.campaigns ?? []; if (!campaigns.length) return <BaseerEmptyState title={copy.noData} description={copy.noDataDetail} />; return <div className="marketing-campaign-list">{campaigns.map((campaign) => <BaseerCard key={campaign.id} className="marketing-campaign"><header><div><strong>{ar ? campaign.titleAr : campaign.titleEn ?? campaign.titleAr}</strong><small>{campaign.platform.replace("_", " ")} · {campaign.startsOn ?? "—"} — {campaign.endsOn ?? "—"}</small></div><span className="baseer-status-badge baseer-status-badge--neutral"><i className="baseer-status-badge__dot" />{campaign.status}</span></header>{campaign.objective ? <p>{campaign.objective}</p> : null}{campaign.notes ? <small>{campaign.notes}</small> : null}{writable && campaign.status !== "ARCHIVED" ? <footer><BaseerButton type="button" variant="secondary" onClick={() => onEdit(campaign)}>{copy.edit}</BaseerButton><BaseerButton type="button" variant="quiet" onClick={() => onArchive(campaign)}>{copy.archive}</BaseerButton></footer> : null}</BaseerCard>)}</div>; }
function Reputation({ data, copy, ar }: { data: Workspace | undefined; copy: Record<string, string>; ar: boolean }) { return <section className="marketing-reputation"><header><h3>{copy.reputation}</h3><p>{copy.notReady}</p></header><div className="baseer-card-grid">{(data?.readiness ?? []).map((item) => <BaseerCard key={item.provider}><strong>{item.provider === "GOOGLE_ADS" ? "Google Ads" : "Google Business"}</strong><span className="baseer-status-badge baseer-status-badge--warning"><i className="baseer-status-badge__dot" />{copy.notConnected}</span><p>{ar ? item.messageAr : item.provider === "GOOGLE_ADS" ? "Google Ads is not connected at this stage; no spend, conversions, or spending decisions are available." : "Google Business is not connected; no reviews, posts, or publishing authority are available at this stage."}</p></BaseerCard>)}</div><BaseerCard tone="muted"><strong>{copy.noWrite}</strong></BaseerCard></section>; }
...
function CampaignDialog({ open, language, busy, editing, initial, copy, error, onClose, onSubmit }: { open: boolean; language: Language; busy: boolean; editing: boolean; initial: Draft; copy: Record<string, string>; error: string | null; onClose: () => void; onSubmit: (values: Draft) => void }) {
  ...
  return <BaseerFormDialog open={open} title={editing ? copy.editTitle : copy.createTitle} language={language} formId="marketing-campaign-form" submitLabel={copy.save} onClose={onClose} busy={busy} error={error} size="wide">
    <form id="marketing-campaign-form" data-baseer-rhf-form="true" className="marketing-campaign-form" noValidate onSubmit={form.handleSubmit((values) => onSubmit(values))}>
      ...
      <label>{copy.platform}<BaseerCombobox label={copy.platform} placeholder={ar ? "اختر المنصة" : "Select platform"} value={value.platform} options={["MANUAL", "GOOGLE_ADS", "META", "TIKTOK", "SNAPCHAT", "OTHER"].map((item) => ({ id: item, label: item.replace("_", " ") }))} onChange={(next) => set("platform", next)} /></label>
      ...
      <label>{copy.lifecycle}<BaseerCombobox label={copy.lifecycle} placeholder={ar ? "اختر الحالة" : "Select status"} value={value.status} options={["DRAFT", "PLANNED", "ACTIVE", "COMPLETED", "CANCELLED"].map((item) => ({ id: item, label: item }))} onChange={(next) => set("status", next)} /></label>
