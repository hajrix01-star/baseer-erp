...
export function HrEmployeeLettersPanel({ employeeId, language, hasCurrentCompensation, canIssue = true, onManageCompensation, onError, onChanged }: { employeeId: string; language: Language; hasCurrentCompensation: boolean; canIssue?: boolean; onManageCompensation: () => void; onError: (message: string) => void; onChanged: () => Promise<void> }) {
  ...
  return <>{canIssue ? <div className="baseer-inline-actions"><BaseerButton type="button" onClick={() => { setMessage(""); setIssueOpen(true); }}>{ar ? "إصدار خطاب" : "Issue letter"}</BaseerButton></div> : null}{message ? <BaseerCard>{message}</BaseerCard> : null}{loading ? <BaseerCard>{ar ? "جارٍ تحميل الخطابات…" : "Loading letters…"}</BaseerCard> : letters.length ? <DataTable ariaLabel={ar ? "سجل خطابات الموظف" : "Employee letter history"} caption={ar ? "سجل الخطابات" : "Letter history"} rows={letters} columns={columns} rowKey={(row) => row.id} /> : <BaseerCard>{ar ? "لا توجد خطابات صادرة." : "No letters have been issued."}</BaseerCard>}{nextCursor ? <BaseerButton type="button" variant="secondary" disabled={loading} onClick={() => void load(nextCursor, true)}>{text.loadMore}</BaseerButton> : null}
    ...
    <BaseerDialog open={Boolean(selected)} title={selected?.letterNumber ?? ""} language={language} busy={busy} onClose={() => setSelected(null)} footer={selected ? <><BaseerButton type="button" disabled={busy || selected.status !== "ISSUED"} onClick={() => void preview(selected)}>{ar ? "معاينة وطباعة" : "Preview & print"}</BaseerButton>{canIssue ? <BaseerButton type="button" variant="danger" disabled={busy || selected.status !== "ISSUED"} onClick={() => setRevokeOpen(true)}>{ar ? "إلغاء الخطاب" : "Revoke letter"}</BaseerButton> : null}</> : undefined}>{selected ? <dl className="administration-details"><div><dt>{ar ? "النوع" : "Type"}</dt><dd>{letterTypeLabel(selected.letterType)}</dd></div><div><dt>{ar ? "اللغة" : "Language"}</dt><dd>{localeLabel(selected.locale)}</dd></div><div><dt>{ar ? "موجه إلى" : "Recipient"}</dt><dd>{selected.recipient ?? "—"}</dd></div><div><dt>{ar ? "تاريخ الإصدار" : "Issued at"}</dt><dd>{selected.issuedAt.slice(0, 10)}</dd></div><div><dt>{ar ? "الحالة" : "Status"}</dt><dd>{statusLabel(selected.status)}</dd></div>{selected.revokedReason ? <div><dt>{ar ? "سبب الإلغاء" : "Revocation reason"}</dt><dd>{selected.revokedReason}</dd></div> : null}</dl> : null}</BaseerDialog>
