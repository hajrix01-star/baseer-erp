...
type Movement = { id: string; source: MovementSource; sourceType: string; documentNumber: string; displayLabelAr: string; displayLabelEn: string; businessDate: string; supplierInvoiceDate: string | null; kind: InvoiceKind; settlementKind: "PAID" | "PAYABLE" | null; status: InvoiceStatus; supplier: Option | null; category: Option | null; debitTotal: string; creditTotal: string; grossAmount: string; netAmount: string; vatAmount: string; payrollAccrual: PayrollAccrual | null; journalEntryId: string; batchNumber: string | null; notes: string | null; recurring: boolean; createdAt: string };
...
export function InvoiceRegisterWorkspace({ language }: { language: Language }) {
  ...
  const columns: DataTableColumn<Movement>[] = useMemo(() => [
    ...
    { id: "source", header: text.documentSource, cell: (item) => <div className="invoice-register__source"><span className="daily-sales-badge">{sourceLabel(item)}{item.recurring ? ` · ${text.recurring}` : ""}</span>{item.payrollAccrual ? <small>{text.payrollExpense} {formatMoney(item.payrollAccrual.grossExpense)} · {text.advanceSettlement} {formatMoney(item.payrollAccrual.advanceSettlement)} · {text.netPayrollPayable} {formatMoney(item.payrollAccrual.netPayable)} · {text.noCashMovement}</small> : null}</div> },
    ...
    { id: "debitTotal", header: text.journalDebitTotal, numeric: true, align: "end", cell: (item) => formatMoney(item.debitTotal) },
    { id: "creditTotal", header: text.journalCreditTotal, numeric: true, align: "end", cell: (item) => formatMoney(item.creditTotal) },
...
function MovementFile({ detail, language, text, optionName, sourceLabel, pane, onPaneChange }: { detail: MovementDetail; language: Language; text: ReturnType<typeof financeText>; optionName: (value: Option | null) => string; sourceLabel: (movement: Movement) => string; pane: "operation" | "journal"; onPaneChange: (value: "operation" | "journal") => void }) {
  ...
  const operationAmounts = movement.payrollAccrual
    ...
    : movement.source === "JOURNAL"
      ? <><label>{text.journalDebitTotal}<output>{formatMoney(movement.debitTotal)}</output></label><label>{text.journalCreditTotal}<output>{formatMoney(movement.creditTotal)}</output></label></>
