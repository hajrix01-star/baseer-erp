...
function AiSettingsContent({ language, session, owner, configuration, loading, loadError, refetch, activeTab, settingsPanel, onSettingsPanelChange, onNavigate }: { language: Language; session: ActiveSession; owner: boolean; configuration: Configuration | undefined; loading: boolean; loadError: unknown; refetch: () => Promise<void>; activeTab: BasiraTab; settingsPanel: BasiraSettingsPanel; onSettingsPanelChange: (panel: BasiraSettingsPanel) => void; onNavigate: (tab: BasiraTab, panel?: BasiraSettingsPanel) => void }) {
  ...
  const nextStep = !provider
    ? { text: ar ? "اربط نموذج الذكاء وحدد حد الإنفاق قبل أي تجربة." : "Connect an AI model and set a spending limit before any pilot.", label: ar ? "إعداد الربط والحدود" : "Set connection and limits", tab: "settings" as const, panel: "connection" as const }
    : displayedConnection?.state !== "READY"
      ? { text: ar ? "تحقق من الاتصال قبل فتح أي تجربة." : "Verify the connection before opening any pilot.", label: ar ? "فحص الاتصال" : "Check connection", tab: "settings" as const, panel: "connection" as const }
      : !systemIdentity
        ? { text: ar ? "أضف شخصية بصيرة المركزية لتوحيد أسلوبها في كل الشركات." : "Add Basira’s central personality to unify her style across companies.", label: ar ? "إعداد شخصية بصيرة" : "Set Basira personality", tab: "settings" as const, panel: "identity" as const }
        : { text: ar ? "الاتصال والهوية جاهزان. اختر مهارة لتجربة بشرية محدودة." : "Connection and identity are ready. Choose a skill for a limited human pilot.", label: ar ? "استعراض المهارات" : "Browse skills", tab: "skills" as const };
  if (activeTab === "overview") return <section id="basira-panel-overview" role="tabpanel" aria-labelledby="basira-tab-overview" className="basira-tab-panel" aria-label={ar ? "الرئيسية" : "Basira home"}>
    <BaseerCard className="basira-overview__next"><p className="eyebrow">{ar ? "الخطوة التالية" : "Next step"}</p><strong>{nextStep.text}</strong><span>{ar ? "بصيرة لا تعمل تلقائياً ولا تنفذ أي إجراء خارجي." : "Basira never runs automatically or performs an external action."}</span><BaseerButton type="button" variant="secondary" onClick={() => onNavigate(nextStep.tab, nextStep.panel)}>{nextStep.label}</BaseerButton></BaseerCard>
  ...
  return <section id="basira-panel-settings" role="tabpanel" aria-labelledby="basira-tab-settings" className="administration-section administration-ai-settings" aria-label={ar ? "إعدادات بصيرة المتقدمة" : "Basira advanced settings"}>
    {settingsNavigation}
    <header className="administration-section-heading"><div><p className="eyebrow">Baseer / Basira</p><h3>{ar ? "ربط الذكاء والحدود" : "AI connection and limits"}</h3><p>{ar ? "إعداد مركزي محفوظ بشكل مشفر. لا يفعّل التفسير وحده؛ يبقى التفعيل وفحص الاتصال صريحين." : "A centrally encrypted setting. Saving it alone never enables explanations; activation and connection verification remain explicit."}</p></div><span className="basira-scope-badge">{ar ? "إعداد مركزي" : "Central setting"}</span></header>
