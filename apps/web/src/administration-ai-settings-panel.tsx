...
import { BaseerIntegerInput, BaseerTextArea } from "./baseer-form-fields";
...
function AiSettingsContent({ language, session, owner, configuration, loading, loadError, refetch, activeTab, settingsPanel, onSettingsPanelChange, onNavigate }: { language: Language; session: ActiveSession; owner: boolean; configuration: Configuration | undefined; loading: boolean; loadError: unknown; refetch: () => Promise<void>; activeTab: BasiraTab; settingsPanel: BasiraSettingsPanel; onSettingsPanelChange: (panel: BasiraSettingsPanel) => void; onNavigate: (tab: BasiraTab, panel?: BasiraSettingsPanel) => void }) {
  ...
  const connectionSettingsForm = owner ? <BaseerCard tone="muted" className="administration-ai-settings__configuration"><header><h4>{ar ? "إعداد الربط والحدود" : "Connection and limits"}</h4><p>{ar ? "غيّر الإعداد ثم احفظه." : "Update the settings, then save."}</p></header><BaseerValidatedFormField<FormValues> id="administration-ai-provider" className="administration-ai-settings__form" values={values} schemaFactory={schemaFactory} onValid={(next) => void submit(next)} errorSummaryLabel={ar ? "تحقق من الحقول المطلوبة." : "Check the required fields."}>{({ errors }) => <>
    ...
    <label>{ar ? "مفتاح مزوّد الذكاء" : "AI provider API key"}<input type="password" autoComplete="new-password" value={values.apiKey} aria-invalid={Boolean(errors.apiKey)} aria-describedby={errors.apiKey ? "administration-ai-key-error" : undefined} onChange={(event) => setValues((current) => ({ ...current, apiKey: event.target.value }))} />{errors.apiKey ? <small id="administration-ai-key-error" role="alert">{errors.apiKey.message}</small> : <small>{ar ? "لن يظهر المفتاح بعد الحفظ." : "The key is hidden after saving."}</small>}</label>
