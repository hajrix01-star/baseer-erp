...
export function AdministrationAiSettingsPanel({ language, session, owner }: { language: Language; session: ActiveSession; owner: boolean }) {
  ...
  const administration = administrationText(language);
  ...
  return <div className="basira-workspace administration-section">
    <header className="administration-section-heading basira-workspace__heading">
      ...
      <span className="basira-workspace__mark" aria-hidden="true">{administration.basiraMark}</span>
    </header>
...
function AiSettingsContent({ language, session, owner, configuration, loading, loadError, refetch, activeTab, settingsPanel, onSettingsPanelChange, onNavigate }: { language: Language; session: ActiveSession; owner: boolean; configuration: Configuration | undefined; loading: boolean; loadError: unknown; refetch: () => Promise<void>; activeTab: BasiraTab; settingsPanel: BasiraSettingsPanel; onSettingsPanelChange: (panel: BasiraSettingsPanel) => void; onNavigate: (tab: BasiraTab, panel?: BasiraSettingsPanel) => void }) {
  ...
  const connectionSettingsForm = owner ? <BaseerCard tone="muted" className="administration-ai-settings__configuration"><header><h4>{ar ? "إعداد الربط والحدود" : "Connection and limits"}</h4><p>{ar ? "غيّر الإعداد ثم احفظه." : "Update the settings, then save."}</p></header><BaseerValidatedFormField<FormValues> id="administration-ai-provider" className="administration-ai-settings__form" values={values} schemaFactory={schemaFactory} onValid={(next) => void submit(next)} errorSummaryLabel={ar ? "تحقق من الحقول المطلوبة." : "Check the required fields."}>{({ errors }) => <>
    <label>{ar ? "المزود" : "Provider"}<BaseerTextInput value={selectedCapability ? providerLabel(selectedCapability.provider) : (ar ? "لا يوجد مزود معتمد" : "No approved provider")} disabled readOnly /></label>
    <label>{ar ? "مفتاح مزوّد الذكاء" : "AI provider API key"}<BaseerTextInput type="password" autoComplete="new-password" value={values.apiKey} aria-invalid={Boolean(errors.apiKey)} aria-describedby={errors.apiKey ? "administration-ai-key-error" : undefined} onChange={(event) => setValues((current) => ({ ...current, apiKey: event.target.value }))} />{errors.apiKey ? <small id="administration-ai-key-error" role="alert">{errors.apiKey.message}</small> : <small>{ar ? "لن يظهر المفتاح بعد الحفظ." : "The key is hidden after saving."}</small>}</label>
    <label>{ar ? "نموذج الذكاء المستخدم" : "AI model in use"}<BaseerStaticSelect label={ar ? "نموذج الذكاء المستخدم" : "AI model in use"} value={selectedCapability?.model ?? ""} disabled={!selectedCapability} aria-invalid={Boolean(errors.model)} onChange={(event) => setValues((current) => ({ ...current, model: event.target.value }))}>{configurableCapabilities.map((capability) => <option key={`${capability.provider}:${capability.model}`} value={capability.model}>{ar ? capability.displayNameAr : capability.displayNameEn}</option>)}</BaseerStaticSelect>{errors.model ? <small role="alert">{errors.model.message}</small> : selectedCapability ? <small>{ar ? selectedCapability.summaryAr : selectedCapability.summaryEn}</small> : <small>{ar ? "لا يوجد نموذج مؤهل للحفظ." : "No model is eligible to save."}</small>}</label>
    <label>{ar ? "حد التفسيرات اليومي" : "Daily explanation limit"}<BaseerIntegerInput value={values.dailyRequestLimit} aria-invalid={Boolean(errors.dailyRequestLimit)} onValueChange={(dailyRequestLimit) => setValues((current) => ({ ...current, dailyRequestLimit }))} />{errors.dailyRequestLimit ? <small role="alert">{errors.dailyRequestLimit.message}</small> : <small>{ar ? "ابدأ بـ10 للتجربة." : "Start with 10 for the pilot."}</small>}</label>
    <label>{ar ? "حد الإنفاق اليومي بالدولار" : "Daily spending limit (USD)"}<BaseerMoneyInput value={values.dailyCostLimit} aria-invalid={Boolean(errors.dailyCostLimit)} onValueChange={(dailyCostLimit) => setValues((current) => ({ ...current, dailyCostLimit }))} />{errors.dailyCostLimit ? <small role="alert">{errors.dailyCostLimit.message}</small> : <small>{ar ? "سقف حازم يمنع الطلب قبل الإرسال." : "A hard cap blocks the request before sending."}</small>}</label>
    {error ? <p className="daily-sales-message error" role="alert">{error}</p> : null}{message ? <p className="daily-sales-message success" role="status">{message}</p> : null}
    <footer><BaseerButton type="submit" disabled={busy || !selectedCapability}>{busy ? (ar ? "جارٍ الحفظ…" : "Saving…") : (provider ? (ar ? "تحديث إعداد بصيرة" : "Update Basira configuration") : (ar ? "حفظ إعداد بصيرة" : "Save Basira configuration"))}</BaseerButton></footer>
  </>}</BaseerValidatedFormField></BaseerCard> : <BaseerCard tone="muted" className="administration-ai-settings__configuration"><strong>{ar ? "إعداد بصيرة للمالك فقط." : "Only the owner can configure Basira."}</strong></BaseerCard>;
...
function BasiraGovernanceContent({ language, session, governance, loading, loadError, refetch, activeTab }: { language: Language; session: ActiveSession; governance: Governance | undefined; loading: boolean; loadError: unknown; refetch: () => Promise<void>; activeTab: BasiraTab }) {
  ...
  if (loading) return <BaseerAsyncState status="loading" language={language} title={ar ? "جارٍ تحميل حوكمة بصيرة…" : "Loading Basira governance…"} />;
  if (!governance) return <BaseerAsyncState status="error" language={language} title={ar ? "حوكمة بصيرة غير متاحة لهذا الدور" : "Basira governance is unavailable for this role"} description={loadError ? presentBaseerApiError(loadError, language, ar ? "تحتاج صلاحيات حوكمة بصيرة للشركة الحالية." : "You need Basira governance permissions for the active company.") : undefined} onRetry={() => void refetch()} />;
  ...
  return <section id={`basira-panel-${activeTab}`} role="tabpanel" aria-labelledby={`basira-tab-${activeTab}`} className="basira-tab-panel basira-governance" data-active-tab={activeTab} aria-label={ar ? "حوكمة بصيرة" : "Basira governance"}>
    ...
    <section className="basira-governance__card basira-governance__card--skills basira-skills"><header className="basira-governance__section"><div><p className="basira-scope-badge">{ar ? "الشركة الحالية" : "Current company"}</p><h4>{ar ? "المهارات" : "Skills"}</h4><p>{ar ? "اختر المهارة المناسبة للشركة؛ التفعيل لا يضيف صلاحيات." : "Choose the right skill for the company; activation adds no permissions."}</p></div></header><div className="basira-skill-grid">{governance.catalogue.map((skill) => {
      ...
      return <BaseerCard key={skill.key} className="basira-skill-card"><header><div><strong>{ar ? skill.nameAr : skill.nameEn}</strong><span>{skill.key} · {skill.riskTier}</span></div><span className="basira-skill-card__status">{catalogueStatusLabel(skill.status, language)}</span></header><p>{skillPurpose(skill.key, skill.purpose, language)}</p><details><summary>{ar ? "تفاصيل الجاهزية" : "Readiness details"}</summary><dl><div><dt>{ar ? "التشغيل" : "Runtime"}</dt><dd>{runtimeStateLabel(skill.runtime.state, language)}{skill.runtime.nextRequirement ? ` · ${skill.runtime.nextRequirement}` : ""}</dd></div><div><dt>{ar ? "آخر فحص" : "Latest check"}</dt><dd>{skill.latestOfflineEvaluation ? `${evaluationStatusLabel(skill.latestOfflineEvaluation.status, language)} · ${skill.latestOfflineEvaluation.passedCaseCount}/${skill.latestOfflineEvaluation.totalCaseCount}` : (ar ? "لم يُفحص بعد" : "Not checked yet")}</dd></div><div><dt>{ar ? "شرط التفعيل" : "Activation condition"}</dt><dd>{skill.activationCondition}</dd></div></dl></details><footer>{activation ? <><span className="basira-governance__state">{activationStatusLabel(activation.status, language)} · {formatUsd(activation.dailyCostLimit, language)}</span><BaseerButton type="button" variant="quiet" disabled={busy || activation.status === "SUSPENDED"} onClick={() => { setSuspensionReason(""); setSuspendActivation(activation); }}>{ar ? "إيقاف" : "Suspend"}</BaseerButton></> : candidate && !evaluationPassed ? <BaseerButton type="button" variant="secondary" disabled={busy} onClick={() => void runOfflineEvaluation(skill)}>{ar ? "فحص الجاهزية" : "Check readiness"}</BaseerButton> : candidate && !runtimeReady ? <><span className="basira-governance__state">{ar ? "الفحص مكتمل؛ بوابة الخادم مغلقة" : "Check passed; server gate is closed"}</span><BaseerButton type="button" variant="quiet" disabled={busy} onClick={() => void runOfflineEvaluation(skill)}>{ar ? "إعادة الفحص" : "Recheck"}</BaseerButton></> : candidate ? <BaseerButton type="button" variant="secondary" disabled={busy} onClick={() => { setCommandError(null); setActivationBudget(initialActivationBudget); setActivationSkill(skill); }}>{skill.status === "ACTIVE" ? (ar ? "تفعيل للشركة" : "Activate for company") : (ar ? "بدء تجربة للشركة" : "Start company pilot")}</BaseerButton> : <span className="basira-governance__state">{ar ? "غير مؤهلة للتفعيل" : "Not eligible for activation"}</span>}</footer></BaseerCard>;
    })}</div></section>
    ...
    <BaseerFormDialog open={draftOpen} title={ar ? "إضافة معلومة للشركة" : "Add company knowledge"} language={language} formId="basira-context-draft" submitLabel={ar ? "حفظ مسودة" : "Save draft"} onClose={() => setDraftOpen(false)} busy={busy} error={commandError} size="wide"><form id="basira-context-draft" className="basira-governance__form" onSubmit={(event) => { event.preventDefault(); void createContext(); }}>
      <p>{ar ? "اختر نوع المعلومة أولاً؛ تظهر فقط الحقول اللازمة. لا تكتب أرقاماً متغيرة أو رسائل أو تعليمات لتنفيذ إجراء." : "Choose the knowledge type first; only its necessary fields appear. Do not enter changing figures, messages, or execution instructions."}</p>
      <label>{ar ? "نوع المعلومة" : "Knowledge type"}<BaseerStaticSelect label={ar ? "نوع المعلومة" : "Knowledge type"} value={draft.kind} onChange={(event) => setDraft((current) => ({ ...initialContextDraft, kind: event.target.value as ContextDraft["kind"] }))}><option value="TERMINOLOGY">{ar ? "مصطلح وتعريف" : "Term and definition"}</option><option value="BUSINESS_SCOPE">{ar ? "نطاق عمل" : "Business scope"}</option><option value="POLICY_REFERENCE">{ar ? "مرجع سياسة" : "Policy reference"}</option></BaseerStaticSelect></label>
      <label>{ar ? "القسم المرتبط" : "Related area"}<BaseerTextInput required dir="ltr" value={draft.moduleScope} onChange={(event) => setDraft((current) => ({ ...current, moduleScope: event.target.value }))} placeholder="marketing / finance / general" /></label>
      {draft.kind !== "POLICY_REFERENCE" ? <><label>{ar ? "العنوان أو المصطلح" : "Title or term"}<BaseerTextInput required value={draft.termAr} onChange={(event) => setDraft((current) => ({ ...current, termAr: event.target.value }))} /></label><label>{ar ? "الوصف المختصر" : "Short definition"}<BaseerTextArea required value={draft.definitionAr} onValueChange={(definitionAr) => setDraft((current) => ({ ...current, definitionAr }))} /></label></> : null}
      {draft.kind === "BUSINESS_SCOPE" ? <label className="basira-governance__form-full">{ar ? "مجال العمل" : "Business domain"}<BaseerTextInput required value={draft.businessDomain} onChange={(event) => setDraft((current) => ({ ...current, businessDomain: event.target.value }))} /></label> : null}
      {draft.kind === "POLICY_REFERENCE" ? <><label>{ar ? "رمز السياسة" : "Policy code"}<BaseerTextInput required dir="ltr" value={draft.policyCode} onChange={(event) => setDraft((current) => ({ ...current, policyCode: event.target.value }))} /></label><label>{ar ? "إصدار السياسة" : "Policy version"}<BaseerTextInput required dir="ltr" value={draft.policyVersion} onChange={(event) => setDraft((current) => ({ ...current, policyVersion: event.target.value }))} /></label><label className="basira-governance__form-full">{ar ? "عنوان السياسة" : "Policy title"}<BaseerTextInput required value={draft.policyTitleAr} onChange={(event) => setDraft((current) => ({ ...current, policyTitleAr: event.target.value }))} /></label></> : null}
      {draft.kind !== "POLICY_REFERENCE" ? <label>{ar ? "أسلوب العرض" : "Presentation style"}<BaseerStaticSelect label={ar ? "أسلوب العرض" : "Presentation style"} value={draft.presentationStyle} onChange={(event) => setDraft((current) => ({ ...current, presentationStyle: event.target.value as ContextDraft["presentationStyle"] }))}><option value="CONCISE">{ar ? "مختصر" : "Concise"}</option><option value="DETAILED">{ar ? "تفصيلي" : "Detailed"}</option></BaseerStaticSelect></label> : null}
      <label className="basira-governance__form-full">{ar ? "مرجع المصدر اختياري" : "Optional source reference"}<BaseerTextInput dir="ltr" value={draft.sourceReference} onChange={(event) => setDraft((current) => ({ ...current, sourceReference: event.target.value }))} /></label>
    </form></BaseerFormDialog>
    <BaseerFormDialog open={Boolean(activationSkill)} title={ar ? "بدء تجربة مهارة بصيرة" : "Start a Basira skill pilot"} language={language} formId="basira-activate-skill" submitLabel={ar ? "بدء التجربة" : "Start pilot"} onClose={() => setActivationSkill(null)} busy={busy} error={commandError} submitDisabled={!activationBudget.dailyRequestLimit || !activationBudget.dailyCostLimit} size="compact"><form id="basira-activate-skill" className="basira-governance__form" onSubmit={(event) => { event.preventDefault(); void submitActivation(); }}><p>{activationSkill ? (ar ? `ستعمل التجربة المحدودة لمهارة «${activationSkill.nameAr}» في الشركة الحالية فقط.` : `The limited pilot applies only to “${activationSkill.nameEn}” in the active company.`) : null}</p><label>{ar ? "حد الطلبات اليومي لهذه المهارة" : "Daily request limit for this skill"}<BaseerIntegerInput autoFocus value={activationBudget.dailyRequestLimit} onValueChange={(dailyRequestLimit) => setActivationBudget((current) => ({ ...current, dailyRequestLimit }))} /></label><label>{ar ? "سقف التكلفة اليومي بالدولار" : "Daily USD cost cap"}<BaseerMoneyInput value={activationBudget.dailyCostLimit} onValueChange={(dailyCostLimit) => setActivationBudget((current) => ({ ...current, dailyCostLimit }))} /><small>{ar ? "السقف حازم ويمنع الطلب قبل إرساله؛ استخدام تفسير محفوظ لا يستهلكه." : "This is a hard pre-send cap; reusing a saved explanation consumes none of it."}</small></label></form></BaseerFormDialog>
    <BaseerFormDialog open={Boolean(suspendActivation)} title={ar ? "إيقاف مهارة بصيرة" : "Suspend Basira skill"} language={language} formId="basira-suspend-skill" submitLabel={ar ? "إيقاف المهارة" : "Suspend skill"} onClose={() => setSuspendActivation(null)} busy={busy} error={commandError} submitDisabled={!suspensionReason.trim()} size="compact"><form id="basira-suspend-skill" className="basira-governance__form" onSubmit={(event) => { event.preventDefault(); void submitSuspension(); }}><p>{ar ? "يوقف هذا التفعيل للشركة الحالية فوراً مع الاحتفاظ بسجل التدقيق." : "This immediately stops the activation for the active company and preserves the audit record."}</p><label className="basira-governance__form-full">{ar ? "سبب الإيقاف" : "Suspension reason"}<BaseerTextArea compact autoFocus value={suspensionReason} onValueChange={setSuspensionReason} /></label></form></BaseerFormDialog>
    <BaseerFormDialog open={Boolean(feedbackReceipt)} title={ar ? "تقييم إجابة بصيرة" : "Evaluate a Basira answer"} language={language} formId="basira-feedback" submitLabel={ar ? "حفظ التقييم" : "Save evaluation"} onClose={() => setFeedbackReceipt(null)} busy={busy} error={commandError} size="compact"><form id="basira-feedback" className="basira-governance__form" onSubmit={(event) => { event.preventDefault(); void submitFeedback(); }}><p>{ar ? "التقييم يحسن النسخ القادمة بمراجعة بشرية؛ لا يغير الحقائق أو يتعلم تلقائياً." : "Evaluation improves future versions through human review; it never changes facts or learns automatically."}</p><label>{ar ? "التقييم" : "Evaluation"}<BaseerStaticSelect label={ar ? "التقييم" : "Evaluation"} value={feedbackKind} onChange={(event) => setFeedbackKind(event.target.value as typeof feedbackKind)}>{(["USEFUL", "NOT_USEFUL", "DATA_INCOMPLETE", "COMPARISON_UNFAIR", "CONTEXT_DIFFERENT", "OTHER"] as const).map((kind) => <option key={kind} value={kind}>{evaluationLabel(kind, language)}</option>)}</BaseerStaticSelect></label><label className="basira-governance__form-full">{ar ? "ملاحظة اختيارية" : "Optional note"}<BaseerTextArea compact value={feedbackNote} onValueChange={setFeedbackNote} /></label></form></BaseerFormDialog>
  </section>;
}
...
function skillPurpose(skillKey: string, fallback: string, language: Language) {
  if (language !== "ar") return fallback;
  return ({
    "administration.guide": "يشرح إعدادات الشركة والمستخدمين والصلاحيات دون تغييرها.",
    "operations.daily_sales_explainer": "يشرح إقفال اليوم والورديات ويوم التشغيل.",
    "finance.accounting_advisor": "يفسر الحسابات المتصالحة والاستثناءات.",
    "decision.command_center_analyst": "يفسر تنبيه القرار مع المصدر والتغطية والسياق الزمني.",
    "owner.daily_brief_analyst": "يجيب من ملخص يومي خادمي دون تنفيذ.",
    "finance.document_classification_draft": "يقترح تصنيفاً أو ربطاً ليراجعه الموظف.",
    "inbound.document_intelligence": "يستخرج ويلخص مستنداً واردًا للمراجعة فقط.",
    "marketing.performance_analyst": "يفسر الأداء والحملات والفجوات دون ادعاء السببية.",
    "marketing.google_review_reply_automation": "مسار مستقبلي؛ النشر الحالي يتطلب مراجعة وتأكيداً بشرياً.",
  })[skillKey] ?? fallback;
}
