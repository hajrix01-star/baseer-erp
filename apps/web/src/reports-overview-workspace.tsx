...
export function ReportsOverviewWorkspace({ language }: { language: Language }) {
  ...
  const load = useCallback(async () => {
    ...
    try { setCatalogue(await api<Catalogue>(current, "/reports/catalogue")); }
    catch (error) { setCatalogue(null); setMessage(presentBaseerApiError(error, language, text.title)); }
  ...
  return <section className="reports-overview" aria-label={text.title} dir={language === "ar" ? "rtl" : "ltr"}>
    ...
    {message ? <div className="reports-overview__notice is-error"><p>{message}</p><BaseerButton type="button" variant="secondary" onClick={() => void load()}>{text.retry}</BaseerButton></div> : null}
