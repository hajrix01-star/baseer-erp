...
type ApplicationEligibilityRule = { onLeave: boolean; fullMonthNewHire: boolean };
...
export function HrPayrollCreateDialog({ open, onClose, onCreated, language, onError }: {
  ...
  const [includeFullMonthNewHireIds, setIncludeFullMonthNewHireIds] = useState<string[]>([]);
  ...
  const applicationLines = useMemo(() => Object.entries(applications).flatMap(([employeeId, choices]) => {
    const rule = applicationEligibilityRules[employeeId] ?? { onLeave: false, fullMonthNewHire: false };
    const isEligible = (!rule.onLeave || includeOnLeaveIds.includes(employeeId)) && (!rule.fullMonthNewHire || includeFullMonthNewHireIds.includes(employeeId));
  ...
  }), [applicationEligibilityRules, applications, includeFullMonthNewHireIds, includeOnLeaveIds]);
  ...
  const loadPreview = useCallback(async (cursor?: string, append = false) => {
    ...
    try {
      const receipt = await previewHrPayrollRun(session, { payrollMonth: draft.payrollMonth, businessDate: draft.businessDate, includeOnLeaveEmployeeIds: includeOnLeaveIds, includeFullMonthNewHireEmployeeIds: includeFullMonthNewHireIds, lines: applicationLines, cursor, pageSize: 50 });
      setPreview(receipt);
  ...
  }, [applicationLines, ar, draft.businessDate, draft.payrollMonth, includeFullMonthNewHireIds, includeOnLeaveIds, language, onError]);
  ...
  const updateNewHireInclusion = (employeeId: string, enabled: boolean) => setIncludeFullMonthNewHireIds((ids) => enabled ? [...new Set([...ids, employeeId])] : ids.filter((id) => id !== employeeId));
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    setBusy(true);
    try {
      await createHrPayrollRun(session, { payrollMonth: draft.payrollMonth, businessDate: draft.businessDate, notes: draft.notes || undefined, includeAllEligible: true, includeOnLeaveEmployeeIds: includeOnLeaveIds, includeFullMonthNewHireEmployeeIds: includeFullMonthNewHireIds, lines: applicationLines, idempotencyKey: requestId() });
      onClose();
      await onCreated();
  ...
  const reason = (employee: HrPayrollPreviewEmployee) => ({
    ...
    NEW_HIRE_REQUIRES_FULL_MONTH_EXCEPTION: ar ? "يتطلب استثناء راتب شهر كامل" : "Requires a full-month salary exception",
  ...
  const inclusionControl = (employee: HrPayrollPreviewEmployee) => {
    const needsFullMonthException = employee.reason === "NEW_HIRE_REQUIRES_FULL_MONTH_EXCEPTION" || includeFullMonthNewHireIds.includes(employee.id);
    const canIncludeLeave = employee.status === "ON_LEAVE" && employee.reason !== "ON_LEAVE_MISSING_COMPENSATION";
    if (!needsFullMonthException && !canIncludeLeave) return employee.included ? (ar ? "مدرج تلقائياً" : "Included by default") : (ar ? "مستثنى" : "Excluded");
    return <>{needsFullMonthException ? <label><input type="checkbox" checked={includeFullMonthNewHireIds.includes(employee.id)} disabled={previewLoading} onChange={(event) => updateNewHireInclusion(employee.id, event.target.checked)} /> {ar ? "شهر كامل" : "Full month"}</label> : null}{canIncludeLeave ? <label><input type="checkbox" checked={includeOnLeaveIds.includes(employee.id)} disabled={previewLoading} onChange={(event) => updateLeaveInclusion(employee.id, event.target.checked)} /> {ar ? "إدراج الإجازة" : "Include leave"}</label> : null}</>;
  };
  ...
  return <BaseerDialog open={open} title={ar ? "إنشاء مسير رواتب" : "Create payroll run"} language={language} busy={busy || previewLoading} onClose={onClose} footer={<><BaseerButton type="button" variant="secondary" disabled={busy} onClick={onClose}>{ar ? "إلغاء" : "Cancel"}</BaseerButton><BaseerButton type="submit" form="hr-payroll-create-form" disabled={busy || previewLoading || !preview || preview.counts.included === 0 || preview.counts.exceptions > 0}>{ar ? "حفظ مسودة" : "Save draft"}</BaseerButton></>}>
    <form id="hr-payroll-create-form" className="administration-form" onSubmit={(event) => void submit(event)}>
      <label>{ar ? "شهر المسير" : "Payroll month"}<input required type="month" value={draft.payrollMonth.slice(0, 7)} onChange={(event) => { setDraft((value) => ({ ...value, payrollMonth: `${event.target.value}-01` })); setIncludeOnLeaveIds([]); setIncludeFullMonthNewHireIds([]); setApplications({}); setApplicationEligibilityRules({}); setApplicationEmployeeId(null); }} /></label>
      <label>{ar ? "تاريخ الاستحقاق" : "Accrual date"}<input required type="date" max={today()} value={draft.businessDate} onChange={(event) => { setDraft((value) => ({ ...value, businessDate: event.target.value })); setIncludeOnLeaveIds([]); setIncludeFullMonthNewHireIds([]); setApplications({}); setApplicationEligibilityRules({}); setApplicationEmployeeId(null); }} /></label>
    ...
    {preview ? <><BaseerSummaryMetricGrid ariaLabel={ar ? "ملخص معاينة المسير" : "Payroll preview summary"}><BaseerSummaryMetric label={ar ? "نشطون" : "Active"} value={preview.counts.active} /><BaseerSummaryMetric label={ar ? "في إجازة" : "On leave"} value={preview.counts.onLeave} /><BaseerSummaryMetric label={ar ? "موظفو المسير" : "Payroll employees"} value={preview.totals.employeeCount} /><BaseerSummaryMetric label={ar ? "إجمالي تقديري" : "Estimated gross"} value={money(preview.totals.grossAmount)} /><BaseerSummaryMetric label={ar ? "تسوية السلف" : "Advance settlements"} value={money(preview.totals.advanceSettlementAmount)} /><BaseerSummaryMetric label={ar ? "الخصومات الإدارية" : "Administrative deductions"} value={money(preview.totals.administrativeDeductionAmount)} /><BaseerSummaryMetric label={ar ? "صافي تقديري" : "Estimated net"} value={money(preview.totals.netPayableAmount)} /><BaseerSummaryMetric label={ar ? "استثناءات" : "Exceptions"} value={preview.counts.exceptions} /></BaseerSummaryMetricGrid>
      <BaseerCard><strong>{ar ? "قاعدة المسير" : "Payroll rule"}</strong><p>{ar ? "الموظفون النشطون ذوو اتفاق راتب صالح يدرجون تلقائياً. موظف الإجازة لا يدرج إلا باختيار صريح، ويستحق كامل الاتفاق؛ لا يوجد خصم إجازة تلقائي. الموظف المعيّن خلال الشهر يحتاج اختيار «شهر كامل» صريحاً، أما من عُيّن بعد تاريخ الاستحقاق فلا يمكن إدراجه." : "Active employees with a valid compensation agreement are included by default. An employee on leave is included only explicitly and receives the full agreement; no leave deduction is automatic. An employee hired during the month needs an explicit full-month selection; one hired after the accrual date cannot be included."}</p></BaseerCard>
      {preview.exceptions.length ? <BaseerCard><strong>{ar ? "يجب معالجة الاستثناءات قبل الإنشاء" : "Exceptions must be resolved before creation"}</strong><ul>{preview.exceptions.map((item) => <li key={item.employeeId}>{item.employeeNumber} · {item.employeeNameAr} — {item.reason === "ACTIVE_MISSING_COMPENSATION" || item.reason === "ON_LEAVE_MISSING_COMPENSATION" ? (ar ? "لا يوجد اتفاق راتب صالح للشهر المحدد." : "No valid compensation agreement for this month.") : item.reason === "NEW_HIRE_REQUIRES_FULL_MONTH_EXCEPTION" ? (ar ? "اختر استثناء راتب شهر كامل إذا كان مستحقاً." : "Select the full-month salary exception if it is due.") : (ar ? "تاريخ التعيين بعد تاريخ الاستحقاق؛ لا يمكن إدراجه." : "The hire date is after the accrual date, so this employee cannot be included.")}</li>)}</ul></BaseerCard> : null}
