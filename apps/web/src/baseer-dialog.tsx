...
type Props = { open: boolean; title: string; children: ReactNode; language: BaseerLanguage; busy?: boolean; onClose: () => void; footer?: ReactNode; size?: "default" | "wide"; className?: string };

/** Central modal shell for forms and read-only details. Uses the shared focus trap and close convention. */
export function BaseerDialog({ open, title, children, language, busy = false, onClose, footer, size = "default", className }: Props) {
  ...
  return <div className="daily-sales-dialog-backdrop" role="presentation" onMouseDown={() => !busy && onClose()}>
    <section ref={dialogRef} className={["daily-sales-dialog", "baseer-dialog", `baseer-dialog--${size}`, className].filter(Boolean).join(" ")} role="dialog" aria-modal="true" aria-label={title} onMouseDown={(event) => event.stopPropagation()}>
