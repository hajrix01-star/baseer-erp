...
/** Lets a record reuse dialog content inside its workspace without a backdrop. */
export const BaseerDialogPresentationContext = createContext<"modal" | "inline">("modal");
...
export function BaseerDialog({ open, title, eyebrow, children, language, busy = false, error, reserveErrorSpace = false, onClose, footer, size = "default", className }: Props) {
  ...
  const presentation = useContext(BaseerDialogPresentationContext);
  ...
  const dialogRef = useDialogFocusTrap({ open: open && presentation === "modal", saving: busy, onClose, onError: setReportedError, onTopmostChange: setIsTopmost });
  if (!open) return null;
  ...
  const dialog = <section ref={dialogRef} className={["daily-sales-dialog", "baseer-dialog", `baseer-dialog--${size}`, className].filter(Boolean).join(" ")} role={presentation === "modal" ? "dialog" : "region"} aria-modal={presentation === "modal" && isTopmost ? "true" : undefined} aria-hidden={presentation === "modal" && !isTopmost ? true : undefined} aria-label={title} onMouseDown={(event) => event.stopPropagation()}>
      ...
      <div className="daily-sales-dialog__body">
        ...
        <BaseerDialogPresentationContext.Provider value="modal">{children}</BaseerDialogPresentationContext.Provider>
      </div>
      {footer ? <footer className="daily-sales-dialog__actions">{footer}</footer> : null}
    </section>;
  return presentation === "inline" ? dialog : createPortal(<div className="daily-sales-dialog-backdrop" role="presentation" onMouseDown={() => !busy && onClose()}>{dialog}</div>, document.body);
}
