...
export function HrLeaveCreateDialog({ open, language, busy, employees, remoteSearch, scopeKey, onClose, onSubmit }: { open: boolean; language: Language; busy: boolean; employees: readonly HrEmployee[]; remoteSearch: RemoteEmployeeSearch; scopeKey: string; onClose: () => void; onSubmit: (value: LeaveForm) => Promise<void> | void }) {
  ...
  return <BaseerFormDialog open={open} title={ar ? "تسجيل إجازة" : "Record leave"} language={language} busy={busy} size="standard" formId="hr-leave-create" submitLabel={ar ? "حفظ الإجازة" : "Save leave"} onClose={onClose}>
    <form id="hr-leave-create" className="baseer-form" data-baseer-rhf-form="true" noValidate onSubmit={form.handleSubmit((values) => void onSubmit(values))}><BaseerFormSection title={ar ? "بيانات الإجازة" : "Leave details"}><BaseerFormGrid>
      <label className="baseer-form-field--full">{ar ? "الموظف" : "Employee"}<BaseerCombobox required label={ar ? "الموظف" : "Employee"} value={value.employeeId} placeholder={ar ? "اختر الموظف" : "Select employee"} options={activeEmployees.map((employee) => ({ id: employee.id, label: employeeLabel(language, employee) }))} remoteSearch={remoteSearch} scopeKey={scopeKey} onChange={(employeeId) => form.setValue("employeeId", employeeId, { shouldDirty: true, shouldValidate: true })} />{form.formState.errors.employeeId ? <small role="alert">{form.formState.errors.employeeId.message}</small> : null}</label>
      <label>{ar ? "النوع" : "Type"}<BaseerStaticSelect label={ar ? "النوع" : "Type"} value={value.leaveType} onChange={(event) => form.setValue("leaveType", event.target.value as LeaveForm["leaveType"], { shouldDirty: true })}>{(["ANNUAL", "SICK", "UNPAID", "OTHER"] as const).map((type) => <option key={type} value={type}>{typeLabel(type)}</option>)}</BaseerStaticSelect></label>
