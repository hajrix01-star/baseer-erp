...
function ConnectionPanelV2({ language, session, onChanged }: { language: Language; session: NonNullable<ReturnType<typeof activeSession>>; onChanged: () => void }) {
  ...
  const refreshQr = async () => { setBusy(true); setError(null); try { const value = await api<LiveQrReceipt>(session, "/whatsapp-invoice-monitoring/settings/connection/qr"); if (value.qr && value.expiresAt) setQr(value); else setError(ar ? "لا يوجد QR جاهز الآن؛ انتظر لحظة ثم حدّثه." : "No QR is ready yet; wait briefly and refresh it."); } catch (cause) { setError(settingsError(cause, language, ar ? "تعذر جلب رمز QR المؤقت." : "Unable to get the temporary QR.")); } finally { setBusy(false); } };
