...
import { activeSession, api, monthRange, requestId } from "./daily-sales-client";
...
type PilotConnectionStatus = "NOT_CONFIGURED" | "DISCONNECTED" | "CONNECTED" | "GAP_DETECTED" | "BLOCKED" | "PILOT_STARTING" | "PILOT_ACTIVE" | "PILOT_STOPPED";
/* Compatibility shapes retained only for the retired ConnectionPanel below.
 * ConnectionPanelV2 is the rendered implementation and uses Live* contracts. */
type SettingsConnection = Readonly<{ id: string | null; status: PilotConnectionStatus; phoneNumberHint: string | null; lastSyncedAt: string | null; messageAr?: string; messageEn?: string; pilotExpiresAt?: string | null }>;
type GroupBinding = Readonly<{ id: string; groupJid: string; displayName: string; active: boolean; bindingRevision: number; groupName: string; companyId: string; companyNameAr: string; companyNameEn: string | null; status: "ACTIVE" | "INACTIVE"; updatedAt: string }>;
type SettingsCompany = Readonly<{ id: string; nameAr: string; nameEn: string | null }>;
type SettingsResponse = Readonly<{ connection: SettingsConnection; groupBindings: readonly GroupBinding[]; companies: readonly SettingsCompany[] }>;
type ConnectionControlReceipt = Readonly<{ connectionId: string; status: PilotConnectionStatus }>;
type PilotQrReceipt = Readonly<{ qr: string | null; expiresAt: string | null }>;
type PilotGroup = Readonly<{ jid: string; displayName: string }>;
type PilotReceipt = Readonly<{ connection: SettingsConnection; qrToken: string | null; qrExpiresAt: string | null }>;
type PilotGroupReceipt = Readonly<{ groups: readonly Readonly<{ jid: string; name: string }>[] }>;
...
function MonitoringContent({ session, language, permissionCodes, tab, setTab, dateBasis, setDateBasis, from, setFrom, to, setTo, search, setSearch, cursor, setCursor, data, loading, error, onRefresh }: ContentProps) {
  ...
  const filterControls = <><BaseerFilterSelect label={text.dateBasis} value={dateBasis} onChange={(event) => setDateBasis(event.target.value as DateBasis)}><option value="RECEIVED_AT">{text.received}</option><option value="INVOICE_DATE">{text.invoiceDate}</option></BaseerFilterSelect><label className="whatsapp-invoice-monitoring__date"><span>{text.from}</span><input type="date" value={from} onChange={(event) => setFrom(event.target.value)} /></label><label className="whatsapp-invoice-monitoring__date"><span>{text.to}</span><input type="date" value={to} onChange={(event) => setTo(event.target.value)} /></label><BaseerButton type="button" variant="quiet" onClick={clear}>{text.clear}</BaseerButton></>;
  ...
  return <BaseerWorkspace className="whatsapp-invoice-monitoring">
    ...
    <nav className="whatsapp-invoice-monitoring__tabs" role="tablist" aria-label={text.title}>{tabs.map((item) => <button key={item} id={`whatsapp-invoice-tab-${item}`} type="button" role="tab" aria-selected={tab === item} aria-controls={`whatsapp-invoice-panel-${item}`} onClick={() => { setTab(item); setCursor(null); }}>{tabTitle(item, text)}</button>)}</nav>
...
function InvoiceDetailForm({ language, suppliers, draft, busy, onChange, onSubmit }: { language: Language; suppliers: readonly SupplierOption[]; draft: Draft; busy: boolean; onChange: <K extends keyof Draft>(key: K, value: Draft[K]) => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  ...
  return <form id="whatsapp-invoice-monitoring-edit" className="whatsapp-invoice-monitoring__edit-form" onSubmit={onSubmit}>
    ...
    <label>{ar ? "حالة الأرشفة" : "Archive state"}<select value={draft.archiveState} disabled={busy} onChange={(event) => onChange("archiveState", event.target.value as Draft["archiveState"])}><option value="ACTIVE">{ar ? "نشطة" : "Active"}</option><option value="ARCHIVED">{ar ? "مؤرشفة" : "Archived"}</option></select></label>
    <label className="whatsapp-invoice-monitoring__edit-reason">{ar ? "سبب التعديل أو الأرشفة" : "Reason for correction or archive"}<textarea className="baseer-textarea" value={draft.reason} disabled={busy} minLength={3} maxLength={500} required onChange={(event) => onChange("reason", event.target.value)} /></label>
  </form>;
}
...
type BindingDraft = { id: string | null; groupJid: string; groupName: string; companyId: string; status: GroupBinding["status"] };
...
function ConnectionPanelV2({ language, session, onChanged }: { language: Language; session: NonNullable<ReturnType<typeof activeSession>>; onChanged: () => void }) {
  ...
  return <section className="whatsapp-invoice-monitoring__settings" dir={ar ? "rtl" : "ltr"}>
    ...
    {!noConnection ? <div className="whatsapp-invoice-monitoring__settings-actions"><BaseerButton type="button" variant="secondary" disabled={busy} onClick={() => void refreshQr()}>{ar ? "عرض أو تحديث QR" : "Show or refresh QR"}</BaseerButton><BaseerButton type="button" variant="secondary" disabled={busy} onClick={() => void refreshGroups()}>{ar ? "تحديث مجموعات واتساب" : "Refresh WhatsApp groups"}</BaseerButton><BaseerButton type="button" disabled={busy} onClick={() => setBinding({ id: null, groupJid: "", displayName: "", active: true, bindingRevision: null })}>{ar ? "إضافة مجموعة لهذه الشركة" : "Add group to this company"}</BaseerButton></div> : null}
    ...
    <BaseerDialog open={pilotOpen} title={connected ? (ar ? "إيقاف الربط" : "Stop connection") : (ar ? "بدء ربط واتساب" : "Start WhatsApp connection")} language={language} busy={busy} error={null} onClose={() => !busy && setPilotOpen(false)} footer={<><BaseerButton type="button" variant="secondary" disabled={busy} onClick={() => setPilotOpen(false)}>{ar ? "إلغاء" : "Cancel"}</BaseerButton><BaseerButton type="button" variant={connected ? "danger" : "primary"} disabled={busy} onClick={() => void control(connected ? "stop" : "start")}>{connected ? (ar ? "إيقاف" : "Stop") : (ar ? "بدء" : "Start")}</BaseerButton></>}><BaseerNotice tone={connected ? "warning" : "info"} title={ar ? "تأكيد صريح مطلوب" : "Explicit confirmation required"}>{connected ? (ar ? "الإيقاف لا يحذف أي فاتورة أو ملف." : "Stopping does not delete any invoice or file.") : (ar ? "بعد البدء اضغط عرض QR ثم امسحه من واتساب على الهاتف." : "After starting, show the QR and scan it from WhatsApp on the phone.")}</BaseerNotice></BaseerDialog>
    <BaseerDialog open={binding !== null} title={binding?.id ? (ar ? "تعديل ربط مجموعة" : "Edit group mapping") : (ar ? "إضافة مجموعة للشركة الحالية" : "Add group to current company")} language={language} busy={busy} error={null} onClose={() => !busy && setBinding(null)} footer={<><BaseerButton type="button" variant="secondary" disabled={busy} onClick={() => setBinding(null)}>{ar ? "إلغاء" : "Cancel"}</BaseerButton><BaseerButton type="submit" form="wai-binding" disabled={busy}>{ar ? "حفظ" : "Save"}</BaseerButton></>}><form id="wai-binding" className="whatsapp-invoice-monitoring__binding-form" onSubmit={saveBinding}>{!binding?.id && groups.length ? <label className="whatsapp-invoice-monitoring__binding-group-picker">{ar ? "مجموعة واتساب المكتشفة" : "Discovered WhatsApp group"}<select value={groups.some((group) => group.jid === binding?.groupJid) ? binding?.groupJid : ""} disabled={busy} onChange={(event) => { const group = groups.find((item) => item.jid === event.target.value); if (group) setBinding((value) => value ? { ...value, groupJid: group.jid, displayName: group.displayName } : value); }}><option value="">{ar ? "اختر مجموعة" : "Choose group"}</option>{groups.map((group) => <option key={group.jid} value={group.jid}>{group.displayName}</option>)}</select></label> : null}<label>{ar ? "معرّف المجموعة (JID)" : "Group JID"}<BaseerTextInput dir="ltr" value={binding?.groupJid ?? ""} disabled={busy || Boolean(binding?.id)} required maxLength={240} onChange={(event) => setBinding((value) => value ? { ...value, groupJid: event.target.value } : value)} /></label><label>{ar ? "اسم العرض" : "Display name"}<BaseerTextInput value={binding?.displayName ?? ""} disabled={busy} required maxLength={160} onChange={(event) => setBinding((value) => value ? { ...value, displayName: event.target.value } : value)} /></label><label><input type="checkbox" checked={binding?.active ?? true} disabled={busy} onChange={(event) => setBinding((value) => value ? { ...value, active: event.target.checked } : value)} /> {ar ? "نشطة" : "Active"}</label></form></BaseerDialog>
...
function PilotQr({ token, expiresAt, language, onRefresh, busy }: { token: string; expiresAt: string; language: Language; onRefresh: () => void; busy: boolean }) {
  const ar = language === "ar"; const [image, setImage] = useState<string | null>(null);
...
function connectionToneFor(status: PilotConnectionStatus): BaseerStatusTone { return status === "CONNECTED" ? "success" : status === "PILOT_ACTIVE" || status === "PILOT_STARTING" ? "info" : status === "GAP_DETECTED" ? "warning" : status === "BLOCKED" ? "danger" : "neutral"; }
...
function connectionTextForSettings(status: PilotConnectionStatus, language: Language) { const ar = language === "ar"; return status === "CONNECTED" ? (ar ? "متصل للقراءة فقط" : "Connected read-only") : status === "PILOT_ACTIVE" ? (ar ? "جلسة Pilot نشطة" : "Pilot session active") : status === "PILOT_STARTING" ? (ar ? "جارٍ تهيئة جلسة Pilot" : "Pilot session starting") : status === "PILOT_STOPPED" ? (ar ? "جلسة Pilot متوقفة" : "Pilot session stopped") : status === "GAP_DETECTED" ? (ar ? "فجوة مزامنة تحتاج متابعة" : "Sync gap needs follow-up") : status === "BLOCKED" ? (ar ? "الربط محجوب" : "Connection blocked") : status === "DISCONNECTED" ? (ar ? "غير متصل" : "Disconnected") : (ar ? "الربط غير مهيأ" : "Connection not configured"); }
