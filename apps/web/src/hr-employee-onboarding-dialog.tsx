...
export function HrEmployeeOnboardingDialog({ open, language, onClose, onSaved, onError }: { open: boolean; language: Language; onClose: () => void; onSaved: () => Promise<void>; onError: (message: string) => void }) {
  ...
  const compensationMethod = Number(draft.scheduledHoursPerDay) > 8 ? "INCLUSIVE_OVERTIME" : "FIXED_MONTHLY";
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!calculation.valid) { onError(ar ? "أكمل بيانات الاتفاق المالي قبل الحفظ." : "Complete the compensation details before saving."); return; }
    const scheduledHoursPerDay = Number(draft.scheduledHoursPerDay);
    const scheduledWorkDays = Number(draft.scheduledWorkDays);
    if (!Number.isInteger(scheduledHoursPerDay) || scheduledHoursPerDay < 8 || scheduledHoursPerDay > 12 || !Number.isInteger(scheduledWorkDays) || scheduledWorkDays < 1 || scheduledWorkDays > 31) {
      onError(ar ? "أدخل ساعات الدوام اليومية من 8 إلى 12 وأيام العمل الشهرية." : "Enter daily hours from 8 to 12 and monthly working days."); return;
    }
    ...
    <form id="hr-employee-onboarding" className="hr-onboarding" noValidate onSubmit={(event) => void submit(event)}>
      <div className="hr-onboarding__workspace">
        <div className="hr-onboarding__entry">
          ...
          <section className="hr-onboarding__section"><header><span>02</span><div><h3>{ar ? "الاتفاق المالي الأول" : "Initial compensation agreement"}</h3><p>{ar ? "أدخل الإجمالي والبدلات والجدول؛ يسري الاتفاق تلقائياً من شهر التعيين." : "Enter the total, allowances and schedule; the agreement starts automatically in the hire month."}</p></div></header><div className="hr-onboarding__grid"><label><span className="hr-onboarding__field-label">{ar ? "إجمالي الراتب الشهري" : "Total monthly salary"}<em>*</em></span><input required min="0.01" inputMode="decimal" value={draft.monthlyGross} placeholder="2500" onChange={(event) => set("monthlyGross", event.target.value)} /></label><label>{ar ? "بدل الأكل الشهري" : "Monthly food allowance"}<input inputMode="decimal" value={draft.foodAllowance} placeholder="500" onChange={(event) => set("foodAllowance", event.target.value)} /></label><label>{ar ? "بدل السكن" : "Housing allowance"}<input inputMode="decimal" value={draft.housingAllowance} onChange={(event) => set("housingAllowance", event.target.value)} /></label><label>{ar ? "بدل المواصلات" : "Transport allowance"}<input inputMode="decimal" value={draft.transportAllowance} onChange={(event) => set("transportAllowance", event.target.value)} /></label><label>{ar ? "بدلات أخرى" : "Other allowances"}<input inputMode="decimal" value={draft.otherAllowance} onChange={(event) => set("otherAllowance", event.target.value)} /></label><label><span className="hr-onboarding__field-label">{ar ? "ساعات الدوام يومياً" : "Daily scheduled hours"}<em>*</em></span><input required type="number" min="8" max="12" value={draft.scheduledHoursPerDay} onChange={(event) => set("scheduledHoursPerDay", event.target.value)} /></label><label><span className="hr-onboarding__field-label">{ar ? "أيام العمل شهرياً" : "Monthly working days"}<em>*</em></span><input required type="number" min="1" max="31" value={draft.scheduledWorkDays} onChange={(event) => set("scheduledWorkDays", event.target.value)} /></label></div></section>
