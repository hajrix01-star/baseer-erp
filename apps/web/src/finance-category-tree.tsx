...
export function FinanceCategoryTree({ language, categories, onOpen }: { language: "ar" | "en"; categories: readonly FinanceCategoryTreeItem[]; onOpen: (item: FinanceCategoryTreeItem) => void }) {
  ...
  const render = (item: FinanceCategoryTreeItem, level: number): React.ReactNode => {
    ...
    return <li key={item.id} role="treeitem" aria-level={level} aria-expanded={hasChildren ? isExpanded : undefined} style={{ listStyle: "none", marginInlineStart: `${(level - 1) * 1.1}rem` }}>
      <div style={rowStyle}>
      ...
      {hasChildren && isExpanded ? <ul role="group" style={{ margin: 0, padding: 0 }}>{nested.map((child) => render(child, level + 1))}</ul> : null}
    </li>;
  };
  if (!roots.length) return null;
  return <ul role="tree" aria-label={language === "ar" ? "شجرة البنود المالية" : "Financial category tree"} style={{ margin: 0, padding: 0 }}>{roots.map((root) => render(root, 1))}</ul>;
}
