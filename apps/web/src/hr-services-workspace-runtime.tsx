...
import { BaseerCheckbox, BaseerIntegerInput, BaseerMoneyInput, BaseerTextArea, BaseerTextInput } from "./baseer-form-fields";
...
function RequiredMark() { return <span aria-hidden="true" className="baseer-required-mark">*</span>; }
function FieldError({ message }: { message?: string }) {
  return message ? <small role="alert" className="visually-hidden">{message}</small> : null;
}
...
export function HrServicesWorkspace({ language, stage }: { language: Language; stage?: string | null }) {
  ...
  const openLinkedOperation = (service: HrEmployeeServiceRecord) => { if (!service.outflowDocumentId) return; window.sessionStorage.setItem("baseer-open-outflow-document", service.outflowDocumentId); window.location.hash = pageRouteHash("operations-purchases", "history"); };
  const columns: readonly BaseerDataGridColumn<HrEmployeeServiceRecord>[] = [
    ...
    { id: "operation", header: ar ? "العملية" : "Operation", cell: (row) => row.outflowDocumentId ? <BaseerButton type="button" variant="quiet" onClick={() => openLinkedOperation(row)}>{ar ? "فتح العملية" : "Open operation"}</BaseerButton> : "—", width: "10rem" },
  ];
  ...
  const requiredLabel = (label: string) => <span>{label}<RequiredMark /></span>;
  ...
  const serviceFormBody = (id: string, submit: () => Promise<void>, employeeLocked = false, issueCost = false) => <form id={id} className="baseer-form" data-baseer-rhf-form="true" noValidate onSubmit={serviceFormState.handleSubmit(() => void submit())}>
    <BaseerFormSection title={ar ? "بيانات الخدمة" : "Service details"}><BaseerFormGrid>
      ...
      <label>{serviceRequiresExpiry ? requiredLabel(ar ? "تاريخ الانتهاء" : "Expiry date") : (ar ? "تاريخ الانتهاء" : "Expiry date")}<BaseerDatePicker language={language} label={ar ? "تاريخ الانتهاء" : "Expiry date"} clearable={!serviceRequiresExpiry} min={serviceForm.issueDate || undefined} value={serviceForm.expiryDate} onChange={(expiryDate) => setServiceForm((value) => ({ ...value, expiryDate }))} /><FieldError message={serviceFormState.formState.errors.expiryDate?.message} /></label>
      {serviceForm.serviceType === "EXIT_REENTRY_VISA" ? <label>{requiredLabel(ar ? "المدة بالأشهر" : "Duration in months")}<BaseerIntegerInput aria-invalid={Boolean(serviceFormState.formState.errors.visaDurationMonths)} required min="1" max="5" value={serviceForm.visaDurationMonths} onValueChange={(visaDurationMonths) => setServiceForm((value) => ({ ...value, visaDurationMonths }))} /><FieldError message={serviceFormState.formState.errors.visaDurationMonths?.message} /></label> : null}
      {!issueCost ? <label className="baseer-form-field--full">{ar ? "ملاحظات" : "Notes"}<BaseerTextArea value={serviceForm.notes} onValueChange={(notes) => setServiceForm((value) => ({ ...value, notes }))} /></label> : null}
    </BaseerFormGrid></BaseerFormSection>
    {issueCost ? <BaseerFormSection title={ar ? "الفاتورة والدفع" : "Invoice and payment"}><BaseerFormGrid>
      <label>{requiredLabel(ar ? "قيمة الخدمة" : "Service amount")}<BaseerMoneyInput aria-invalid={Boolean(costFormState.formState.errors.grossAmount)} required value={costForm.grossAmount} onValueChange={(grossAmount) => setCostForm((value) => ({ ...value, grossAmount }))} /><FieldError message={costFormState.formState.errors.grossAmount?.message} /></label>
      <label>{requiredLabel(ar ? "الخزينة أو البنك" : "Vault or bank")}<BaseerCombobox required label={ar ? "الخزينة أو البنك" : "Vault or bank"} value={costForm.vaultId} placeholder={ar ? "اختر جهة الدفع" : "Select payment destination"} options={activeVaults.map((vault) => ({ id: vault.id, label: ar ? vault.nameAr : vault.nameEn }))} onChange={(vaultId) => setCostForm((value) => ({ ...value, vaultId }))} /><FieldError message={costFormState.formState.errors.vaultId?.message} /></label>
      <label>{ar ? "رقم فاتورة المورد" : "Supplier invoice number"}<BaseerTextInput value={costForm.supplierInvoiceNumber} onChange={(event) => setCostForm((value) => ({ ...value, supplierInvoiceNumber: event.target.value, supplierInvoiceMissingReason: event.target.value ? "" : value.supplierInvoiceMissingReason }))} /></label>
      <label>{ar ? "أو سبب عدم وجود رقم فاتورة" : "Or invoice-number missing reason"}<BaseerTextInput value={costForm.supplierInvoiceMissingReason} onChange={(event) => setCostForm((value) => ({ ...value, supplierInvoiceMissingReason: event.target.value, supplierInvoiceNumber: event.target.value ? "" : value.supplierInvoiceNumber }))} /></label>
      <label className="baseer-form-checkbox"><BaseerCheckbox checked={costForm.isTaxable} onChange={(event) => setCostForm((value) => ({ ...value, isTaxable: event.target.checked }))} /> {ar ? "الفاتورة خاضعة للضريبة" : "Invoice is taxable"}</label>
      <details className="baseer-form-advanced"><summary>{ar ? "تفاصيل اختيارية" : "Optional details"}</summary><div><label>{ar ? "تاريخ الفاتورة والدفع" : "Invoice and payment date"}<BaseerDatePicker language={language} label={ar ? "تاريخ الفاتورة والدفع" : "Invoice and payment date"} max={today()} value={costForm.businessDate} onChange={(businessDate) => setCostForm((value) => ({ ...value, businessDate, supplierInvoiceDate: value.supplierInvoiceDate || businessDate }))} /></label><label>{ar ? "تاريخ فاتورة المورد" : "Supplier invoice date"}<BaseerDatePicker language={language} label={ar ? "تاريخ فاتورة المورد" : "Supplier invoice date"} clearable max={costForm.businessDate} value={costForm.supplierInvoiceDate} onChange={(supplierInvoiceDate) => setCostForm((value) => ({ ...value, supplierInvoiceDate }))} /></label><label className="baseer-form-field--full">{ar ? "ملاحظات" : "Notes"}<BaseerTextArea value={serviceForm.notes} onValueChange={(notes) => setServiceForm((value) => ({ ...value, notes }))} /></label></div></details>
  ...
  return <section className="administration-panel" aria-label={ar ? "خدمات الموظفين والامتثال" : "Employee services and compliance"}>
    ...
    <BaseerFilterBar language={language} search={search} searchLabel={ar ? "البحث في خدمات الموظفين" : "Search employee services"} searchPlaceholder={ar ? "ابحث بالموظف أو الرقم المرجعي" : "Search employee or reference"} onSearchChange={setSearch} controlsPresentation="menu" controls={<><BaseerCombobox label={ar ? "الموظف" : "Employee"} value={employeeFilter} placeholder={ar ? "كل الموظفين" : "All employees"} options={employees.map((employee) => ({ id: employee.id, label: employeeLabel(language, employee) }))} remoteSearch={searchAllEmployeeOptions} scopeKey={session?.companyId ?? "signed-out"} onChange={setEmployeeFilter} /><BaseerFilterSelect label={ar ? "النوع" : "Type"} value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as typeof typeFilter)}><option value="">{ar ? "كل الأنواع" : "All types"}</option>{serviceTypes.map((type) => <option key={type} value={type}>{serviceTypeLabel(type)}</option>)}</BaseerFilterSelect><BaseerFilterSelect label={ar ? "السجل التشغيلي" : "Operational status"} value={complianceFilter} onChange={(event) => setComplianceFilter(event.target.value as typeof complianceFilter)}><option value="">{ar ? "كل الحالات" : "All statuses"}</option>{(["ACTIVE", "RENEWED", "CANCELLED"] as const).map((status) => <option key={status} value={status}>{complianceLabel(status)}</option>)}</BaseerFilterSelect><BaseerFilterSelect label={ar ? "الانتهاء" : "Expiry"} value={expiryFilter} onChange={(event) => setExpiryFilter(event.target.value as typeof expiryFilter)}><option value="">{ar ? "كل الفترات" : "All periods"}</option>{(["EXPIRED", "DUE_30", "DUE_60", "DUE_90", "NO_EXPIRY"] as const).map((status) => <option key={status} value={status}>{expiryLabel(status)}</option>)}</BaseerFilterSelect></>} appliedFilters={appliedFilters} onClear={() => { setEmployeeFilter(""); setTypeFilter(""); setComplianceFilter(""); setExpiryFilter(""); }} />
    ...
    <BaseerFormDialog open={cancelOpen} title={ar ? "إلغاء الخدمة" : "Cancel service"} size="compact" language={language} busy={busy} formId="hr-service-cancel" submitLabel={ar ? "تأكيد الإلغاء" : "Confirm cancellation"} cancelLabel={ar ? "رجوع" : "Back"} onClose={() => setCancelOpen(false)}><form id="hr-service-cancel" className="baseer-form" data-baseer-rhf-form="true" noValidate onSubmit={cancelFormState.handleSubmit(({ reason }) => void saveCancel(reason), () => cancelReasonRef.current?.focus())}><BaseerFormSection title={ar ? "سبب الإلغاء" : "Cancellation reason"}><BaseerFormGrid columns="one"><label>{ar ? "سبب الإلغاء" : "Cancellation reason"}<BaseerTextArea ref={cancelReasonRef} compact aria-invalid={Boolean(cancelFormState.formState.errors.reason)} disabled={busy} value={cancelFormState.watch("reason")} onValueChange={(reason) => cancelFormState.setValue("reason", reason, { shouldDirty: true, shouldValidate: true })} /><FieldError message={cancelFormState.formState.errors.reason?.message} /></label></BaseerFormGrid></BaseerFormSection></form></BaseerFormDialog>
    <BaseerFormDialog open={costOpen} title={ar ? "إصدار تكلفة الخدمة" : "Issue service cost"} size="standard" language={language} busy={busy} formId="hr-service-cost" submitLabel={ar ? "إصدار التكلفة" : "Issue cost"} onClose={() => setCostOpen(false)}><form id="hr-service-cost" className="baseer-form" data-baseer-rhf-form="true" noValidate onSubmit={costFormState.handleSubmit(() => void saveCost())}><BaseerFormSection title={ar ? "بيانات الفاتورة والدفع" : "Invoice and payment details"}><BaseerFormGrid><label>{ar ? "التاريخ" : "Date"}<BaseerDatePicker language={language} label={ar ? "التاريخ" : "Date"} max={today()} value={costForm.businessDate} onChange={(businessDate) => setCostForm((value) => ({ ...value, businessDate }))} /><FieldError message={costFormState.formState.errors.businessDate?.message} /></label><label>{ar ? "المبلغ الإجمالي" : "Gross amount"}<BaseerMoneyInput aria-invalid={Boolean(costFormState.formState.errors.grossAmount)} required value={costForm.grossAmount} onValueChange={(grossAmount) => setCostForm((value) => ({ ...value, grossAmount }))} /><FieldError message={costFormState.formState.errors.grossAmount?.message} /></label><label>{ar ? "الخزينة" : "Vault"}<BaseerCombobox required label={ar ? "الخزينة" : "Vault"} value={costForm.vaultId} placeholder={ar ? "اختر الخزينة" : "Select vault"} options={activeVaults.map((vault) => ({ id: vault.id, label: ar ? vault.nameAr : vault.nameEn }))} onChange={(vaultId) => setCostForm((value) => ({ ...value, vaultId }))} /><FieldError message={costFormState.formState.errors.vaultId?.message} /></label><label>{ar ? "رقم فاتورة المورد" : "Supplier invoice number"}<BaseerTextInput value={costForm.supplierInvoiceNumber} onChange={(event) => setCostForm((value) => ({ ...value, supplierInvoiceNumber: event.target.value }))} /></label><label>{ar ? "سبب عدم وجود الفاتورة" : "Invoice missing reason"}<BaseerTextInput value={costForm.supplierInvoiceMissingReason} onChange={(event) => setCostForm((value) => ({ ...value, supplierInvoiceMissingReason: event.target.value }))} /></label><label>{ar ? "تاريخ الفاتورة" : "Invoice date"}<BaseerDatePicker language={language} label={ar ? "تاريخ الفاتورة" : "Invoice date"} clearable max={costForm.businessDate} value={costForm.supplierInvoiceDate} onChange={(supplierInvoiceDate) => setCostForm((value) => ({ ...value, supplierInvoiceDate }))} /><FieldError message={costFormState.formState.errors.supplierInvoiceDate?.message} /></label><label className="baseer-form-checkbox"><BaseerCheckbox checked={costForm.isTaxable} onChange={(event) => setCostForm((value) => ({ ...value, isTaxable: event.target.checked }))} /> {ar ? "خاضعة للضريبة" : "Taxable"}</label><label className="baseer-form-field--full">{ar ? "ملاحظات" : "Notes"}<BaseerTextArea value={costForm.notes} onValueChange={(notes) => setCostForm((value) => ({ ...value, notes }))} /></label></BaseerFormGrid></BaseerFormSection></form></BaseerFormDialog>
    <BaseerFormDialog open={costReversalOpen} title={ar ? "إلغاء تكلفة الخدمة" : "Cancel service cost"} size="compact" language={language} busy={busy} formId="hr-service-cost-reversal" submitLabel={ar ? "إلغاء" : "Cancel"} onClose={() => setCostReversalOpen(false)}><form id="hr-service-cost-reversal" className="baseer-form" data-baseer-rhf-form="true" noValidate onSubmit={costReversalFormState.handleSubmit(() => void reverseCost())}><BaseerFormSection title={ar ? "سبب الإلغاء" : "Cancellation reason"}><BaseerFormGrid columns="one"><label>{ar ? "تاريخ الإلغاء" : "Cancellation date"}<BaseerDatePicker language={language} label={ar ? "تاريخ الإلغاء" : "Cancellation date"} max={today()} value={costReversal.businessDate} onChange={(businessDate) => setCostReversal((value) => ({ ...value, businessDate }))} /><FieldError message={costReversalFormState.formState.errors.businessDate?.message} /></label><label>{ar ? "سبب الإلغاء" : "Cancellation reason"}<BaseerTextArea compact aria-invalid={Boolean(costReversalFormState.formState.errors.reason)} required value={costReversal.reason} onValueChange={(reason) => setCostReversal((value) => ({ ...value, reason }))} /><FieldError message={costReversalFormState.formState.errors.reason?.message} /></label></BaseerFormGrid></BaseerFormSection></form></BaseerFormDialog>
    <BaseerDialog open={Boolean(detail)} title={detail ? serviceTypeLabel(detail.serviceType) : ar ? "تفاصيل الخدمة" : "Service details"} language={language} busy={busy} onClose={() => setDetail(null)} footer={detail ? <>{detail.outflowDocumentId ? <BaseerButton type="button" variant="secondary" onClick={() => openLinkedOperation(detail)}>{ar ? "فتح العملية" : "Open operation"}</BaseerButton> : null}<BaseerButton type="button" variant="secondary" disabled={busy || detail.status !== "DRAFT"} onClick={() => void prepareService("edit", detail)}>{ar ? "تعديل" : "Edit"}</BaseerButton><BaseerButton type="button" variant="secondary" disabled={busy || detail.status === "CANCELLED"} onClick={() => void prepareService("renew", detail)}>{ar ? "تجديد" : "Renew"}</BaseerButton>{detail.status === "DRAFT" ? <BaseerButton type="button" disabled={busy} onClick={() => void openCost(detail)}>{ar ? "إصدار التكلفة" : "Issue cost"}</BaseerButton> : null}{detail.costStatus === "POSTED" && detail.status === "ISSUED" && hasActivePermission("hr.employees.write") && hasActivePermission("finance.purchase_expense.cancel") ? <BaseerButton type="button" variant="danger" disabled={busy} onClick={() => { costReversalFormState.reset({ serviceId: detail.id, businessDate: today(), reason: "" }); setCostReversalOpen(true); }}>{ar ? "إلغاء التكلفة" : "Cancel cost"}</BaseerButton> : null}<BaseerButton type="button" variant="quiet" disabled={busy || detail.status !== "DRAFT"} onClick={() => { cancelFormState.reset({ reason: "" }); setActionService(detail); setDetail(null); setCancelOpen(true); }}>{ar ? "إلغاء الخدمة" : "Cancel service"}</BaseerButton></> : undefined}>{detail ? <dl className="administration-details"><div><dt>{ar ? "الموظف" : "Employee"}</dt><dd>{employeeLabel(language, detail.employee)}</dd></div><div><dt>{ar ? "نوع الخدمة" : "Service type"}</dt><dd>{serviceTypeLabel(detail.serviceType)}</dd></div><div><dt>{ar ? "الرقم المرجعي" : "Reference"}</dt><dd>{detail.referenceNumber ?? "—"}</dd></div><div><dt>{ar ? "الفترة" : "Period"}</dt><dd>{`${detail.issueDate ?? "—"} — ${detail.expiryDate ?? "—"}`}</dd></div><div><dt>{ar ? "الحالة التشغيلية" : "Operational status"}</dt><dd>{complianceLabel(detail.complianceStatus)}</dd></div><div><dt>{ar ? "حالة الانتهاء" : "Expiry state"}</dt><dd>{expiryLabel(expiryState(detail))}</dd></div><div><dt>{ar ? "الحالة" : "Status"}</dt><dd>{statusLabel(detail.status)}</dd></div><div><dt>{ar ? "العملية المالية" : "Financial transaction"}</dt><dd>{detail.costStatus === "REVERSED" ? (ar ? "ملغاة" : "Cancelled") : detail.outflowDocumentId ? <BaseerButton type="button" variant="quiet" onClick={() => openLinkedOperation(detail)}>{ar ? "فتح العملية المرتبطة" : "Open linked operation"}</BaseerButton> : (ar ? "لم تُصدر" : "Not issued")}</dd></div>{detail.notes ? <div><dt>{ar ? "ملاحظات" : "Notes"}</dt><dd>{detail.notes}</dd></div> : null}</dl> : null}</BaseerDialog>
  </section>;
}
