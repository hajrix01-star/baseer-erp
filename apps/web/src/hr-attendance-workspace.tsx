...
type CloseSessionForm = { employeeId: string; employeeName: string; businessDate: string; checkOutDate: string; checkOutTime: string; reason: string };
type AttendanceWorkspaceTab = "today" | "manager" | "schedules" | "reports" | "settings";
...
const blankCloseSession = (): CloseSessionForm => ({ employeeId: "", employeeName: "", businessDate: today(), checkOutDate: today(), checkOutTime: "17:00", reason: "" });
...
export function HrAttendanceWorkspace({ language }: { language: Language }) {
  ...
  const [closeSessionForm, setCloseSessionForm] = useState<CloseSessionForm>(blankCloseSession);
  ...
  const [companySettings, setCompanySettings] = useState<AttendanceCompanySettings | null>(null);
  const [openSessionsForClose, setOpenSessionsForClose] = useState<AttendanceOpenSession[]>([]);
  const [openSessionsCursor, setOpenSessionsCursor] = useState<string | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [branchReceipt, settingsReceipt] = await Promise.all([listAttendanceBranches(current, readOptions), getAttendanceCompanySettings(current, readOptions)]);
      if (request !== loadRequest.current) return;
  ...
  const saveCompanySettings = async () => {
    const current = activeSession();
    if (!current || !companySettings || busy) return;
    setBusy(true); setSaveError(null);
    try {
      const saved = await updateAttendanceCompanySettings(current, { locationEnabled: companySettings.locationEnabled, idempotencyKey: requestId() });
      setCompanySettings(saved);
      setMessage({ tone: "success", text: saved.locationEnabled ? (ar ? "تم تفعيل طلب الموقع عند الحضور والانصراف. يحتفظ النظام به لمدة أسبوعين فقط." : "Location is required at check-in and check-out and retained for two weeks only.") : (ar ? "تم تعطيل طلب الموقع. لن يطلب النظام الموقع أو يحفظه." : "Location is disabled. The system will neither request nor retain it.") });
    } catch (error) {
      const text = presentBaseerApiError(error, language, ar ? "تعذر حفظ إعداد الموقع. إعدادات الموقع للمالك فقط." : "Could not save location settings. Location settings are owner-only.");
      setSaveError(text); setMessage({ tone: "danger", text });
    } finally { setBusy(false); }
  };
  const submitCloseSession = async (event: React.FormEvent) => {
    event.preventDefault();
    const current = activeSession();
    if (!current || busy) return;
    if (!closeSessionForm.reason.trim() || !closeSessionForm.checkOutDate || !closeSessionForm.checkOutTime) { setSaveError(ar ? "حدد وقت الانصراف واكتب سبب الإغلاق الإداري." : "Choose the check-out time and enter an administrative-close reason."); return; }
    const checkOutAt = new Date(`${closeSessionForm.checkOutDate}T${closeSessionForm.checkOutTime}:00+03:00`);
    if (Number.isNaN(checkOutAt.valueOf())) { setSaveError(ar ? "حدد وقت انصراف صالحاً." : "Choose a valid check-out time."); return; }
    setBusy(true); setSaveError(null);
    try {
      await closeAttendanceSession(current, { employeeId: closeSessionForm.employeeId, businessDate: closeSessionForm.businessDate, checkOutAt: `${closeSessionForm.checkOutDate}T${closeSessionForm.checkOutTime}:00+03:00`, reason: closeSessionForm.reason.trim(), idempotencyKey: requestId() });
      setCloseSessionOpen(false); setCloseSessionForm(blankCloseSession());
      setMessage({ tone: "success", text: ar ? "تم تسجيل الانصراف الإداري بالوقت والسبب المحددين، مع بقاء تسجيل الدخول الأصلي محفوظاً." : "Administrative check-out was recorded with the selected time and reason; the original check-in remains preserved." });
      await load();
    } catch (error) {
      const text = presentBaseerApiError(error, language, ar ? "تعذر إغلاق جلسة الحضور." : "Could not close the attendance session.");
      setSaveError(text); setMessage({ tone: "danger", text });
    } finally { setBusy(false); }
  };
  ...
  return <BaseerWorkspace>
    <BaseerSectionHeader eyebrow={ar ? "HR · حضور وانصراف" : "HR · Timekeeping"} title={ar ? "الحضور والانصراف" : "Attendance & timekeeping"} description={ar ? "سجل تشغيلي مستقل. وقت ما بعد الجدول والنقص يظهران كمرجع للمراجعة؛ ولا يتغير الراتب تلقائياً." : "A separate operational record. Post-schedule and missing time are review references only; payroll never changes automatically."} actions={activeTab === "settings" ? <BaseerButton type="button" onClick={() => { setSaveError(null); setBranchOpen(true); }}>{ar ? "إضافة موقع حضور" : "Add attendance location"}</BaseerButton> : undefined} />
    ...
    <nav className="hr-attendance__tabs" aria-label={ar ? "أقسام الحضور والانصراف" : "Attendance and timekeeping sections"}>
      {([
        ...
        ["manager", ar ? "إغلاق جلسة" : "Close session"],
    ...
    {roster ? <Suspense fallback={<AttendancePanelFallback ar={ar} />}><HrAttendanceRosterEditor roster={roster} language={language} busy={busy} onSave={saveRoster} onApprove={approveRoster} printAction={activeSession() ? <BaseerOutputActions session={activeSession()!} reportCode="hr.attendance-weekly-roster" language={language} filters={{ weekStart: roster.weekStart }} printLabel={ar ? "طباعة جدول الأسبوع A4" : "Print weekly schedule A4"} allowExport={false} /> : null} /></Suspense> : null}
    ...
    {activeTab === "manager" ? <section className="hr-attendance__tab-panel" aria-label={ar ? "إغلاق الجلسات المفتوحة" : "Close open sessions"}><BaseerCard><div className="hr-attendance__card-header"><div><span>{ar ? "إجراء المدير" : "Manager action"}</span><h3>{ar ? "إغلاق الجلسات المفتوحة" : "Close open sessions"}</h3><p>{ar ? "قائمة محدودة للجلسات المفتوحة فقط. اختر وقت الانصراف الفعلي واكتب السبب؛ لا تمنح هذه الشاشة وصولاً للوحة الحضور أو الرواتب." : "A limited list of open sessions only. Choose the actual check-out time and record a reason; this screen grants no access to attendance dashboards or payroll."}</p></div><BaseerButton type="button" variant="secondary" disabled={busy} onClick={() => void load()}>{ar ? "تحديث" : "Refresh"}</BaseerButton></div>{openSessionsForClose.length ? <div className="baseer-data-grid__scroll"><table className="baseer-data-grid"><thead><tr><th>{ar ? "الموظف" : "Employee"}</th><th>{ar ? "تاريخ الدخول" : "Check-in date"}</th><th>{ar ? "وقت الدخول" : "Check-in time"}</th><th>{ar ? "إجراء" : "Action"}</th></tr></thead><tbody>{openSessionsForClose.map((session) => <tr key={session.sessionId}><td>{session.employeeNameAr}</td><td dir="ltr">{session.businessDate}</td><td>{formatTime(session.checkInAt, language, "Asia/Riyadh")}</td><td><BaseerButton type="button" onClick={() => { setSaveError(null); setCloseSessionForm({ employeeId: session.employeeId, employeeName: session.employeeNameAr, businessDate: session.businessDate, checkOutDate: session.businessDate, checkOutTime: "17:00", reason: "" }); setCloseSessionOpen(true); }}>{ar ? "تسجيل خروج إداري" : "Administrative check-out"}</BaseerButton></td></tr>)}</tbody></table></div> : <p className="hr-attendance__quiet-state">{ar ? "لا توجد جلسات مفتوحة تحتاج إغلاقاً." : "There are no open sessions requiring closure."}</p>}{openSessionsCursor ? <BaseerButton type="button" variant="secondary" disabled={openSessionsLoadingMore} onClick={() => void loadMoreOpenSessionsForClose()}>{openSessionsLoadingMore ? (ar ? "جارٍ تحميل المزيد…" : "Loading more…") : (ar ? "تحميل المزيد" : "Load more")}</BaseerButton> : null}</BaseerCard></section> : null}
    {activeTab === "today" ? <section className="hr-attendance__tab-panel" aria-label={ar ? "متابعة اليوم" : "Today monitoring"}>
    <BaseerCard variant="record" className="hr-attendance__daily-overview"><div className="hr-attendance__card-header"><div><span>{ar ? "المتابعة اليومية" : "Daily monitoring"}</span><h3>{ar ? "المخطط مقابل الفعلي" : "Planned versus actual"}</h3><p>{ar ? "المؤشرات تشغيلية للمراجعة فقط؛ لا تنشئ خصماً أو أثراً مالياً تلقائياً." : "These operational indicators never create a payroll action automatically."}</p></div><div className="hr-attendance__date-action"><BaseerDatePicker language={language} label={ar ? "تاريخ المتابعة" : "Attendance date"} value={selectedDate} onChange={setSelectedDate} /><BaseerButton type="button" onClick={() => void load()} disabled={busy}>{ar ? "تحديث" : "Refresh"}</BaseerButton></div></div>{dashboard ? <div className="hr-attendance__metrics"><div><small>{ar ? "الموظفون النشطون" : "Active employees"}</small><b>{dashboard.summary.activeEmployees}</b></div><div><small>{ar ? "في الدوام" : "At work"}</small><b>{dashboard.summary.openSessions}</b></div><div><small>{ar ? "لم يسجلوا" : "Missing check-in"}</small><b>{dashboard.summary.notRecorded}</b></div><div><small>{ar ? "تنبيهات اليوم" : "Today alerts"}</small><b>{alerts?.alerts.length ?? "—"}</b></div></div> : null}</BaseerCard>
    ...
    {activeTab === "settings" ? <BaseerCard className="hr-attendance__employee-portal-link"><div><span>{ar ? "رابط الموظفين" : "Employee portal link"}</span><h3>{ar ? "صفحة الموظف بدون دخول ERP" : "Employee page without ERP login"}</h3><p>{companySettings?.locationEnabled ? (ar ? "شارك هذا الرابط مع الموظفين. يدخل الموظف بكوده فقط؛ ويتطلب التسجيل QR والموقع لأن المالك فعّل التحقق بالموقع." : "Share this link with employees. They use their personal code; recording requires QR and location because the owner enabled location verification.") : (ar ? "شارك هذا الرابط مع الموظفين. يدخل الموظف بكوده فقط؛ ويتطلب التسجيل QR فقط، ولا يطلب النظام الموقع." : "Share this link with employees. They use their personal code; recording requires QR only and never requests location.")}</p></div>{employeePortalUrl ? <a dir="ltr" href={employeePortalUrl}>{employeePortalUrl}</a> : <p className="hr-attendance__quiet-state">{ar ? "تعذر تجهيز الرابط. سجل دخول ERP ثم أعد فتح الإعدادات." : "The link could not be prepared. Sign in to ERP, then reopen settings."}</p>}</BaseerCard> : null}
    {activeTab === "settings" ? <section className="hr-attendance__tab-panel" aria-label={ar ? "الإعدادات" : "Settings"}>{companySettings ? <BaseerCard><h3>{ar ? "التحقق بالموقع" : "Location verification"}</h3><p>{ar ? "إعداد خاص بالمالك. عند التعطيل لا يطلب النظام الموقع ولا يحفظه. عند التفعيل يحفظ موقع لحظة التسجيل فقط لمدة أسبوعين، ولا يوجد تتبع مستمر." : "Owner-only setting. When disabled, location is neither requested nor retained. When enabled, only the check-in/out location is retained for two weeks; there is no continuous tracking."}</p><fieldset className="baseer-form"><legend>{ar ? "طلب الموقع عند التسجيل" : "Request location when recording"}</legend><label><BaseerRadio name="attendance-location-enabled" checked={companySettings.locationEnabled} disabled={busy} onChange={() => setCompanySettings((current) => current ? { ...current, locationEnabled: true } : current)} />{ar ? "مفعّل" : "Enabled"}</label><label><BaseerRadio name="attendance-location-enabled" checked={!companySettings.locationEnabled} disabled={busy} onChange={() => setCompanySettings((current) => current ? { ...current, locationEnabled: false } : current)} />{ar ? "معطّل" : "Disabled"}</label></fieldset><p>{ar ? "مدة الاحتفاظ ثابتة: أسبوعان." : "Retention is fixed at two weeks."}</p><BaseerButton type="button" disabled={busy} onClick={() => void saveCompanySettings()}>{ar ? "حفظ إعداد الموقع" : "Save location setting"}</BaseerButton></BaseerCard> : <BaseerNotice tone="info">{ar ? "جارٍ تحميل إعداد التحقق بالموقع…" : "Loading location-verification settings…"}</BaseerNotice>}<BaseerCard><h3>{ar ? "موقع الحضور ورمز QR" : "Attendance location and QR"}</h3><p>{companySettings?.locationEnabled ? (ar ? "PIN شخصي من 4 أرقام + QR متغير + موقع دقيق لحظة الحضور أو الانصراف. لا يوجد تتبع مستمر أو ربط هاتف." : "A four-digit personal PIN, rotating QR, and precise location only at check-in or check-out. No continuous tracking or phone binding.") : (ar ? "PIN شخصي من 4 أرقام + QR متغير. التحقق بالموقع معطّل لهذه الشركة." : "A four-digit personal PIN and rotating QR. Location verification is disabled for this company.")}</p><section className="baseer-card-grid">{branches.map((branch) => <BaseerCard key={branch.id} padding="compact"><h4>{ar ? branch.nameAr : branch.nameEn ?? branch.nameAr}</h4>{companySettings?.locationEnabled ? <><p dir="ltr">{formatNumberFixed(branch.latitude, 6, language)}, {formatNumberFixed(branch.longitude, 6, language)}</p><p>{ar ? `النطاق ${branch.radiusMeters} م · الدقة ${branch.maxAccuracyMeters} م · QR كل ${branch.qrValiditySeconds} ث` : `Radius ${branch.radiusMeters}m · accuracy ${branch.maxAccuracyMeters}m · QR every ${branch.qrValiditySeconds}s`}</p></> : <p>{ar ? `QR كل ${branch.qrValiditySeconds} ث` : `QR every ${branch.qrValiditySeconds}s`}</p>}<BaseerButton type="button" onClick={() => void refreshQr(branch)}>{ar ? "عرض QR الحالي" : "Show current QR"}</BaseerButton></BaseerCard>)}</section>{!branches.length ? <BaseerNotice tone="info">{ar ? "أضف موقع المنشأة أولاً، ثم ضع شاشة QR في الكاشير." : "Add the location first, then place the QR screen at the cashier."}</BaseerNotice> : null}</BaseerCard>
    ...
    <BaseerFormDialog open={closeSessionOpen} title={ar ? "تسجيل خروج إداري" : "Administrative check-out"} language={language} formId="attendance-close-session-form" submitLabel={ar ? "تسجيل الخروج" : "Record check-out"} onClose={() => setCloseSessionOpen(false)} busy={busy} error={saveError}><form id="attendance-close-session-form" className="baseer-form" onSubmit={submitCloseSession}><p><strong>{closeSessionForm.employeeName}</strong></p><p>{ar ? "استخدمه فقط لجلسة مفتوحة. يحدد المدير وقت الانصراف الفعلي ويكتب السبب؛ يبقى تسجيل الدخول الأصلي وسجل التعديل محفوظين للمراجعة." : "Use only for an open session. The manager selects the actual check-out time and records a reason; the original check-in and audit record are preserved."}</p><BaseerDatePicker language={language} label={ar ? "تاريخ الانصراف الفعلي" : "Actual check-out date"} value={closeSessionForm.checkOutDate} onChange={(checkOutDate) => setCloseSessionForm((current) => ({ ...current, checkOutDate }))} /><label>{ar ? "وقت الانصراف الفعلي" : "Actual check-out time"}<BaseerTimeInput required value={closeSessionForm.checkOutTime} onChange={(event) => setCloseSessionForm((current) => ({ ...current, checkOutTime: event.target.value }))} /></label><label>{ar ? "سبب الإغلاق الإداري" : "Administrative close reason"}<BaseerTextArea compact required value={closeSessionForm.reason} onValueChange={(reason) => setCloseSessionForm((current) => ({ ...current, reason }))} /></label></form></BaseerFormDialog>
