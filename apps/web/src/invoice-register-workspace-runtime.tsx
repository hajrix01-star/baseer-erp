...
import { formatCount, formatDate, formatDateTime, formatMoney } from "./number-format";
...
type Movement = { id: string; source: MovementSource; sourceType: string; documentNumber: string; displayLabelAr: string; displayLabelEn: string; businessDate: string; supplierInvoiceDate: string | null; kind: InvoiceKind; operationFamily: OperationFamily; operationClass: OperationClass; settlementKind: "PAID" | "PAYABLE" | null; status: InvoiceStatus; supplier: Option | null; category: Option | null; parentClassification: Option | null; grossAmount: string; netAmount: string; vatAmount: string; payrollAccrual: PayrollAccrual | null; journalEntryId: string; batchNumber: string | null; notes: string | null; recurring: boolean; createdAt: string };
...
export function InvoiceRegisterWorkspaceRuntime({ language }: { language: Language }) {
  ...
  const columns: BaseerDataGridColumn<Movement>[] = useMemo(() => [
    ...
    { id: "date", header: text.documentDate, cell: (item) => <bdi dir="ltr">{formatDate(item.businessDate, language)}</bdi> },
    ...
    { id: "classification", header: language === "ar" ? "التصنيف الأب" : "Parent classification", cell: (item) => <span className="invoice-register__classification">{optionName(item.parentClassification)}</span> },
...
function MovementFile({ detail, language, text, optionName, sourceLabel, pane, onPaneChange }: { detail: MovementDetail; language: Language; text: ReturnType<typeof financeText>; optionName: (value: Option | null) => string; sourceLabel: (movement: Movement) => string; pane: "operation" | "journal"; onPaneChange: (value: "operation" | "journal") => void }) {
  ...
  return <>
    ...
    {pane === "operation" ? <div className="administration-form">
      <label>{text.documentSource}<output>{sourceLabel(movement)}</output></label><label>{text.documentDate}<output dir="ltr">{formatDate(movement.businessDate, language)}</output></label><label>{text.status}<output>{movement.status === "POSTED" ? text.posted : text.cancelled}</output></label><label>{text.supplier}<output>{optionName(movement.supplier)}</output></label><label>{text.financialCategory}<output>{optionName(movement.category)}</output></label><label>{text.settlement}<output>{movement.settlementKind === "PAYABLE" ? text.payable : movement.settlementKind === "PAID" ? text.paid : "—"}</output></label>{operationAmounts}<label>{text.notes}<output>{movement.notes ?? "—"}</output></label>
      ...
      {detail.batch ? <><label>{text.batchContext}<output><bdi dir="ltr">{detail.batch.batchNumber} · {formatCount(detail.batch.documentCount, language)}</bdi></output></label><label>{text.totalAmount}<output>{formatMoney(detail.batch.grossAmount, "SAR", language)}</output></label></> : null}
    ...
    </div> : <div className="administration-form"><label>{text.documentSource}<output>{language === "ar" ? detail.journal.displayLabelAr : detail.journal.displayLabelEn}</output></label><label>{text.sourceReference}<output dir="ltr">{detail.journal.displayReference}</output></label><label>{text.documentDate}<output dir="ltr">{formatDate(detail.journal.businessDate, language)}</output></label><label>{text.postedAt}<output dir="ltr">{formatDateTime(detail.journal.postedAt, language, "Asia/Riyadh")}</output></label><label>{text.status}<output>{detail.journal.status === "REVERSED" ? text.movementReversed : text.posted}</output></label><label>{text.notes}<output>{detail.journal.description ?? "—"}</output></label><section><h4>{text.journalLines}</h4><DataTable ariaLabel={text.journalLines} caption={text.journalLines} columns={journalColumns} rows={detail.journal.lines} rowKey={(item) => item.id} /></section></div>}
  </>;
}
