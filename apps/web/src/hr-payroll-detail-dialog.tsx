...
export function HrPayrollDetailDialog({ runId, language, onClose, onChanged, onError }: { runId: string; language: Language; onClose: () => void; onChanged: () => Promise<void>; onError: (message: string) => void }) {
  ...
  return <><BaseerDialog open title={detail.payrollRun.runNumber} className={isDraft ? "hr-payroll-create-dialog" : undefined} language={language} busy={busy} onClose={onClose} footer={<div className="hr-payroll-detail__footer">{isDraft ? <div className="hr-payroll-create__total"><span>{ar ? "صافي المستحق" : "Net payable"}</span><strong dir="ltr">{money(detail.payrollRun.netPayableAmount)} SAR</strong></div> : null}<div className="page-actions"><BaseerOutputActions session={activeSession()!} reportCode="hr.payroll-run" language={language} filters={{ payrollRunId: detail.payrollRun.id }} printLabel={ar ? "معاينة وطباعة A4" : "Preview & print A4"} />{isDraft ? <BaseerButton type="button" variant="danger" onClick={() => setDiscardOpen(true)}>{ar ? "حذف المسودة" : "Discard draft"}</BaseerButton> : null}{isDraft ? <BaseerButton type="button" onClick={() => void approve()}>{ar ? "اعتماد" : "Approve"}</BaseerButton> : null}{status === "APPROVED" || status === "PARTIALLY_PAID" ? <BaseerButton type="button" onClick={openPayment}>{ar ? "سداد" : "Pay"}</BaseerButton> : null}{status === "APPROVED" ? <BaseerButton type="button" variant="danger" onClick={() => setReverseOpen(true)}>{ar ? "عكس" : "Reverse"}</BaseerButton> : null}</div></div>}>{isDraft ? <section className="hr-payroll-detail__draft-table"><header><strong>{ar ? `قائمة الموظفين (${detail.lines.length})` : `Employees (${detail.lines.length})`}</strong><span>{ar ? "مسودة قابلة للمراجعة قبل الاعتماد" : "Draft ready for review before approval"}</span></header><DataTable ariaLabel={ar ? "مسودة مسير الرواتب" : "Payroll draft"} caption={ar ? "مسودة مسير الرواتب" : "Payroll draft"} rows={detail.lines} rowKey={(line) => line.id} columns={draftLineColumns} /></section> : <><BaseerSummaryMetricGrid><BaseerSummaryMetric label={ar ? "الإجمالي" : "Gross"} value={money(detail.payrollRun.grossAmount)} /><BaseerSummaryMetric label={ar ? "السلف" : "Advances"} value={money(detail.payrollRun.advanceSettlementAmount)} /><BaseerSummaryMetric label={ar ? "الخصومات" : "Deductions"} value={money(detail.payrollRun.administrativeDeductionAmount)} /><BaseerSummaryMetric label={ar ? "الصافي" : "Net"} value={money(detail.payrollRun.netPayableAmount)} /></BaseerSummaryMetricGrid><DataTable ariaLabel={ar ? "سطور المسير" : "Payroll lines"} caption={ar ? "سطور المسير" : "Payroll lines"} rows={detail.lines} rowKey={(line) => line.id} columns={lineColumns} /></>}</BaseerDialog>
