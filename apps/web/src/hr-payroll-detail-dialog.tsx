...
export function HrPayrollDetailDialog({ runId, language, onClose, onChanged, onError }: { runId: string; language: Language; onClose: () => void; onChanged: () => Promise<void>; onError: (message: string) => void }) {
  ...
  const load = async () => {
    ...
    try { setDetail(await getHrPayrollRun(session, runId)); }
  ...
  const draftLineColumns = [
    { id: "employee", header: ar ? "الموظف" : "Employee", cell: (line: HrPayrollDetail["lines"][number]) => <strong>{line.employeeNumber} · {ar ? line.employeeNameAr : line.employeeNameEn ?? line.employeeNameAr}</strong> },
    { id: "gross", header: ar ? "إجمالي الراتب" : "Gross salary", cell: (line: HrPayrollDetail["lines"][number]) => money(line.grossSalary) },
    { id: "advances", header: ar ? "السلف" : "Advances", cell: (line: HrPayrollDetail["lines"][number]) => line.advances.length ? <div className="hr-payroll-create__application-list hr-payroll-detail__applications">{line.advances.map((item) => <label key={item.id} className="is-selected"><input type="checkbox" checked disabled aria-label={item.referenceNumber} /><span>{item.referenceNumber}</span><b dir="ltr">{money(item.amount)}</b></label>)}</div> : "—" },
    { id: "deductions", header: ar ? "الخصومات الإدارية" : "Administrative deductions", cell: (line: HrPayrollDetail["lines"][number]) => line.administrativeDeductions.length ? <div className="hr-payroll-create__application-list hr-payroll-detail__applications">{line.administrativeDeductions.map((item) => <label key={item.id} className="is-selected"><input type="checkbox" checked disabled aria-label={item.referenceNumber} /><span>{item.referenceNumber}</span><b dir="ltr">{money(item.amount)}</b></label>)}</div> : "—" },
    { id: "net", header: ar ? "الصافي" : "Net payable", cell: (line: HrPayrollDetail["lines"][number]) => <strong dir="ltr">{money(line.netPayableAmount)}</strong> },
  ];
  ...
  const showFullTable = Boolean(status);
  return <><BaseerDialog open title={detail.payrollRun.runNumber} className="hr-payroll-create-dialog" language={language} busy={busy} onClose={onClose} footer={<div className="hr-payroll-detail__footer"><div className="hr-payroll-create__total"><span>{ar ? "صافي المستحق" : "Net payable"}</span><strong dir="ltr">{money(detail.payrollRun.netPayableAmount)} SAR</strong></div><div className="page-actions"><BaseerOutputActions session={activeSession()!} reportCode="hr.payroll-run" language={language} filters={{ payrollRunId: detail.payrollRun.id }} printLabel={ar ? "معاينة وطباعة A4" : "Preview & print A4"} />{isDraft ? <BaseerButton type="button" variant="danger" onClick={() => setDiscardOpen(true)}>{ar ? "حذف المسودة" : "Discard draft"}</BaseerButton> : null}{isDraft ? <BaseerButton type="button" onClick={() => void approve()}>{ar ? "اعتماد" : "Approve"}</BaseerButton> : null}{status === "APPROVED" || status === "PARTIALLY_PAID" ? <BaseerButton type="button" onClick={() => void openPayment()}>{ar ? "سداد" : "Pay"}</BaseerButton> : null}{status === "APPROVED" ? <BaseerButton type="button" variant="danger" onClick={() => setReverseOpen(true)}>{ar ? "عكس" : "Reverse"}</BaseerButton> : null}</div></div>}>{showFullTable ? <section className="hr-payroll-detail__draft-table"><header><strong>{ar ? `قائمة الموظفين (${detail.lines.length})` : `Employees (${detail.lines.length})`}</strong><span>{isDraft ? (ar ? "مسودة قابلة للمراجعة قبل الاعتماد" : "Draft ready for review before approval") : (ar ? "مسير محفوظ للقراءة فقط" : "Payroll saved for review only")}</span></header><div className="hr-payroll-create__table-wrap"><table><thead><tr><th>{ar ? "الموظف" : "Employee"}</th><th>{ar ? "إجمالي الراتب" : "Gross salary"}</th><th>{ar ? "السلف" : "Advances"}</th><th>{ar ? "الخصومات الإدارية" : "Administrative deductions"}</th><th>{ar ? "الصافي المستحق" : "Net payable"}</th></tr></thead><tbody>{detail.lines.map((line) => <tr key={line.id}><td><strong>{line.employeeNumber} · {ar ? line.employeeNameAr : line.employeeNameEn ?? line.employeeNameAr}</strong></td><td><bdi>{money(line.grossSalary)}</bdi></td><td>{selectedApplications(line.advances)}</td><td>{selectedApplications(line.administrativeDeductions)}</td><td><strong dir="ltr">{money(line.netPayableAmount)}</strong></td></tr>)}</tbody></table></div></section> : <><BaseerSummaryMetricGrid><BaseerSummaryMetric label={ar ? "الإجمالي" : "Gross"} value={money(detail.payrollRun.grossAmount)} /><BaseerSummaryMetric label={ar ? "السلف" : "Advances"} value={money(detail.payrollRun.advanceSettlementAmount)} /><BaseerSummaryMetric label={ar ? "الخصومات" : "Deductions"} value={money(detail.payrollRun.administrativeDeductionAmount)} /><BaseerSummaryMetric label={ar ? "الصافي" : "Net"} value={money(detail.payrollRun.netPayableAmount)} /></BaseerSummaryMetricGrid><DataTable ariaLabel={ar ? "سطور المسير" : "Payroll lines"} caption={ar ? "سطور المسير" : "Payroll lines"} rows={detail.lines} rowKey={(line) => line.id} columns={lineColumns} /></>}</BaseerDialog>
