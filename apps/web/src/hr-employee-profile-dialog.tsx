...
export function HrEmployeeProfileDialog({ detail, language, onClose, onEdit, onManageCompensation, onLoadMoreMovements, onError, onChanged }: { detail: HrDetail; language: Language; onClose: () => void; onEdit: () => void; onManageCompensation: () => void; onLoadMoreMovements: () => Promise<void>; onError: (message: string) => void; onChanged: () => Promise<void> }) {
  ...
  useEffect(() => { const session = activeSession(); if (!session || !canManageAttendance) { setAttendancePinDisplay(null); return; } let active = true; void getAttendanceEmployeePin(session, employeeId).then((value) => { if (active) setAttendancePinDisplay(value); }).catch(() => { if (active) setAttendancePinDisplay(null); }); return () => { active = false; }; }, [canManageAttendance, employeeId]);
  const refreshEmployeeSchedule = async () => {
    const session = activeSession(); if (!session || !canManageAttendance) return;
    const [schedule, templates] = await Promise.all([getAttendanceEmployeeSchedule(session, employeeId), listAttendanceScheduleTemplates(session)]);
    setEmployeeSchedule(schedule); setScheduleTemplates(templates.templates);
  };
  ...
  useEffect(() => {
    ...
    const request = requestedTab === "payroll"
      ? Promise.all([
        ...
        setPayroll(payrollReceipt?.lines ?? []); setPayrollCursor(payrollReceipt?.nextCursor ?? null); setAdvances(advanceReceipt?.advances ?? []); setAdvanceCursor(advanceReceipt?.nextCursor ?? null); setDeductions(deductionReceipt?.deductions ?? []); setDeductionCursor(deductionReceipt?.nextCursor ?? null); setSettlements(settlementReceipt?.settlements ?? []); setSettlementCursor(settlementReceipt?.nextCursor ?? null);
      })
  ...
  useEffect(() => { if (!tabs.some((item) => item.id === tab)) setTab(tabs[0]?.id ?? "overview"); }, [tab, tabs]);
  ...
  const refreshAttendancePin = async () => { const session = activeSession(); if (session) { try { setAttendancePinDisplay(await getAttendanceEmployeePin(session, employeeId)); } catch { setAttendancePinDisplay(null); } } await onChanged(); };
  const saveEmployeeSchedule = async (event: React.FormEvent) => {
    event.preventDefault(); const session = activeSession();
    if (!session || scheduleBusy) return;
    if (!scheduleTemplateId) { setScheduleError(ar ? "اختر قالب الدوام." : "Choose a work template."); return; }
    if (weeklyKind === "CUSTOM_PERIODS" && (!weeklyStart || !weeklyEnd || weeklyStart === weeklyEnd)) { setScheduleError(ar ? "أدخل فترة نصف دوام صحيحة." : "Enter a valid half-day period."); return; }
    ...
    try {
      ...
      await refreshEmployeeSchedule(); setScheduleOpen(false); await onChanged();
    } catch (error) { setScheduleError(error instanceof Error ? error.message : (ar ? "تعذر حفظ دوام الموظف." : "Could not save the employee schedule.")); }
  ...
  const currentWeeklyAdjustment = activeWeeklyAdjustments[0] ?? null;
