...
import {
  ...
  listHrCompensationPolicies, type HrAdministrativeDeduction, type HrAdvance, type HrCompensationPolicy, type HrDetail, type HrEmployeeLeave, type HrEmployeePayrollHistoryLine,
} from "./hr-client";
...
export function HrEmployeeProfileDialog({ detail, language, onClose, onEdit, onManageCompensation, onLoadMoreMovements, onError, onChanged }: {
  ...
  return <><BaseerDialog open title={name} size="wide" language={language} onClose={onClose} footer={<><BaseerButton type="button" variant="secondary" onClick={onManageCompensation}>{ar ? "اتفاق الراتب" : "Compensation agreement"}</BaseerButton><BaseerButton type="button" variant="secondary" onClick={onEdit}>{ar ? "تعديل الموظف" : "Edit employee"}</BaseerButton><BaseerButton type="button" variant="secondary" onClick={onClose}>{ar ? "إغلاق" : "Close"}</BaseerButton></>}><header style={{ display: "flex", gap: ".5rem", flexWrap: "wrap", alignItems: "baseline", marginBlockEnd: ".75rem" }}><strong>{detail.employee.employeeNumber}</strong><span>{detail.employee.jobTitle ?? "—"}</span><span>{status}</span></header><div style={{ overflowX: "auto" }}><BaseerWorkspaceTabs ariaLabel={ar ? "تبويبات ملف الموظف" : "Employee profile tabs"} idPrefix="hr-employee-profile" tabs={tabs} activeId={tab} onChange={(value) => setTab(value as ProfileTab)} /></div><BaseerBatchPanel id={`hr-employee-profile-panel-${tab}`} labelledBy={`hr-employee-profile-${tab}`}>
    {tab === "overview" ? <div className="administration-list"><article><strong>{ar ? "بيانات الموظف" : "Employee details"}</strong><span>{ar ? "تاريخ التعيين" : "Hired"}: {detail.employee.hireDate}</span><span>{ar ? "الهاتف" : "Phone"}: {detail.employee.phone ?? "—"}</span><span>{ar ? "البريد" : "Email"}: {detail.employee.email ?? "—"}</span></article><article><strong>{ar ? "اتفاق الراتب الحالي" : "Current compensation agreement"}</strong>{detail.compensation ? <><span>{ar ? "الإجمالي الشهري" : "Monthly total"}: {money(detail.compensation.monthlyGross)}</span><span>{detail.compensation.compensationMethod === "INCLUSIVE_OVERTIME" ? (ar ? "شامل الأوفر تايم" : "Inclusive of overtime") : (ar ? "راتب ثابت" : "Fixed salary")}</span><span>{ar ? "بدل الأكل" : "Food allowance"}: {money(detail.compensation.foodAllowance)}</span><span>{ar ? "بدلات ثابتة أخرى" : "Other fixed allowances"}: {money(detail.compensation.otherAllowance)}</span><span>{ar ? "إصدار السياسة" : "Policy version"}: {policyVersion ? `${policyVersion.policyCode} · v${policyVersion.versionNumber}` : (ar ? "الافتراضي للشركة" : "Company default")}</span><span>{ar ? "السريان" : "Effective"}: {detail.compensation.effectiveFrom}</span>{detail.compensation.scheduledHoursPerDay ? <span>{ar ? "الدوام" : "Schedule"}: {detail.compensation.scheduledHoursPerDay}h × {detail.compensation.scheduledWorkDays}</span> : null}</> : <span>{ar ? "لا يوجد اتفاق راتب نشط." : "No active compensation agreement."}</span>}</article></div> : null}
