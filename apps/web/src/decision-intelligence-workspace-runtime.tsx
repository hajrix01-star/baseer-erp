...
import { BaseerCheckbox, BaseerMoneyInput, BaseerNumericSuffixInput, BaseerTextArea, BaseerTextInput } from "./baseer-form-fields";
...
import { BaseerNoticeGroup } from "./baseer-workspace";
...
import { formatDateTime, formatMoney, formatNumber, formatPercent } from "./number-format";
import { BaseerCompanyReadQuery } from "./baseer-company-read-query";
import { BasiraInterpretationCenter } from "./basira-interpretation-center";
...
type SalesComparison = {
  ...
  /**
   * The API deliberately withholds comparison figures when either period is
   * incomplete.  Keep that contract nullable in the client: a missing figure
   * is a data-quality state, never a zero and never a rendering failure.
   */
  payload: { currencyCode: "SAR"; currentNetAmount: string | null; comparisonNetAmount: string | null; differenceNetAmount: string | null; percentDifference: string | null; currentCustomerCount: number | null; comparisonCustomerCount: number | null };
};
...
type AlertExplanation = { receiptId: string; interpretationId: string; disposition: "GENERATED" | "REUSED"; alertId: string; skillKey: "decision.command_center_analyst"; model: string; explanation: { summary: string; evidence: string[]; limitations: string[]; reviewSteps: string[] }; createdAt: string; replayed: boolean };
...
async function loadDecisionReads(session: ActiveSession, signal: AbortSignal, { section, period, canReadMetrics, canReadContext, canReadAlerts, canManageGlobalContext, canManagePolicies, migrationReviewLocked }: { section: number; period: BaseerPeriodRange; canReadMetrics: boolean; canReadContext: boolean; canReadAlerts: boolean; canManageGlobalContext: boolean; canManagePolicies: boolean; migrationReviewLocked: boolean }): Promise<DecisionReads> {
  ...
  const readPolicy = canManagePolicies && section === 3 && !migrationReviewLocked;
...
export function DecisionIntelligenceWorkspaceRuntime({ language, section, permissionCodes, migrationReviewLocked }: { language: Language; section: number; permissionCodes: readonly string[] | null; migrationReviewLocked: boolean }) {
  ...
  const [aiExplanationBusy, setAiExplanationBusy] = useState(false);
  ...
  const canUseAi = hasPermission("platform.ai.use");
  ...
  const canReadHumanInsights = hasPermission("decision.human_insights.read") && canUseAi && (canReadAlerts || hasPermission("marketing.insights.read"));
  const canWriteHumanInsights = hasPermission("decision.human_insights.write") && canUseAi && (canReadAlerts || hasPermission("marketing.insights.read"));
  ...
  const presentReadError = useCallback((reason: unknown) => {
    ...
    // The diagnostic belongs in developer tooling/logs, never in a user-facing ERP notice.
    setError(presentBaseerApiError(reason, language, fallback));
  }, [language]);
  ...
  const sectionContent = useMemo(() => {
    ...
    if (section === 2) return <AlertsPanel language={language} alerts={alerts} canGiveFeedback={canGiveFeedback} canManage={canManageAlerts} canUseAi={canUseAi} onExplain={async (alertId) => {
      const current = activeSession(); if (!current) return;
      setAiExplanationBusy(true); setAiExplanationError(null); setAiExplanation(null);
      try { const idempotencyKey = requestId(); const result = await api<AlertExplanation>(current, "/administration/ai/runtime/decision-alert-explanations", { method: "POST", headers: { "Content-Type": "application/json", "X-Idempotency-Key": idempotencyKey }, body: JSON.stringify({ alertId, language, idempotencyKey }) }); setAiExplanation(result); }
      catch (reason) { setAiExplanationError(presentBaseerApiError(reason, language, language === "ar" ? "تعذر إعداد تفسير التنبيه." : "The alert explanation could not be prepared.")); }
      finally { setAiExplanationBusy(false); }
    }} onViewEvidence={async (alertId) => {
      if (!session) return;
    ...
    if (section === 3) return <><QualityPanel language={language} metric={metric} canManagePolicies={canManagePolicies && !migrationReviewLocked} onEvaluate={async () => {
      const current = activeSession(); if (!current) return;
    ...
    }} />{canManagePolicies && migrationReviewLocked ? <BaseerCard className="decision-sales-policy decision-sales-policy--locked"><header className="decision-card__header"><div><h3>{language === "ar" ? "سياسة تنبيه تغير المبيعات" : "Sales-change alert policy"}</h3><p>{language === "ar" ? "الشركة في وضع مراجعة ترحيل؛ القراءة متاحة فقط حتى يزيل المالك قفل المراجعة." : "The company is in migration-review mode; only reads are available until an owner removes the review lock."}</p></div><BaseerStatusBadge tone="warning">{language === "ar" ? "مراجعة ترحيل" : "Migration review"}</BaseerStatusBadge></header></BaseerCard> : null}{canManagePolicies && !migrationReviewLocked && <SalesChangePolicyCard language={language} policy={salesChangePolicy} onEvaluate={async () => {
      const current = activeSession(); if (!current) return;
    ...
    if (section === 5) return <BasiraInterpretationCenter language={language} canRead={canReadHumanInsights} canWrite={canWriteHumanInsights} />;
  ...
  }, [alerts, candidates, canGiveFeedback, canManageAlerts, canManageCompanyContext, canManageGlobalContext, canManagePolicies, canReadHumanInsights, canUseAi, canWriteHumanInsights, companyEvents, comparison, events, globalEvents, language, load, matchedWeekdayComparison, metric, migrationReviewLocked, openAlerts, period.from, period.to, researchBusy, reviews, salesChangePolicy, section, sourceHealth]);
  ...
  return <section className="baseer-workspace decision-workspace" aria-busy={loading}>
    <DecisionReadCoordinator session={session} language={language} section={section} period={period} permissionScope={permissionCodes ?? []} canReadMetrics={canReadMetrics} canReadContext={canReadContext} canReadAlerts={canReadAlerts} canManageGlobalContext={canManageGlobalContext} canManagePolicies={canManagePolicies} migrationReviewLocked={migrationReviewLocked} onData={applyReads} onLoading={setLoading} onError={presentReadError} onRefetch={(refetch) => { refetchReads.current = refetch; }} />
    <header className="baseer-section-header">
      <div className="baseer-section-header__copy"><p className="baseer-section-header__eyebrow">Decision Intelligence &amp; Context</p><h2>{language === "ar" ? "قراءات موثقة قبل أي تفسير" : "Evidence before interpretation"}</h2></div>
    ...
    <BaseerFormDialog open={eventDialogOpen} title={companyEventEditing ? (language === "ar" ? "تعديل حدث الشركة" : "Edit company event") : (language === "ar" ? "إضافة حدث للشركة" : "Add company event")} language={language} formId="decision-company-event" submitLabel={language === "ar" ? "حفظ الحدث" : "Save event"} busy={eventBusy} error={eventError} size="standard" className="decision-context-dialog" onClose={() => !eventBusy && setEventDialogOpen(false)}>
      <BaseerValidatedFormField id="decision-company-event" className="baseer-form" values={eventForm} schemaFactory={({ z }) => z.object({ eventKind: z.string().min(1), titleAr: z.string().trim().min(2, language === "ar" ? "أدخل عنواناً من حرفين على الأقل." : "Enter a title of at least two characters.").max(240), startsOn: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, language === "ar" ? "اختر تاريخاً صحيحاً." : "Choose a valid date."), endsOn: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, language === "ar" ? "اختر تاريخاً صحيحاً." : "Choose a valid date."), sourceReference: z.string().max(500) }).refine((value) => value.endsOn >= value.startsOn, { path: ["endsOn"], message: language === "ar" ? "يجب ألا يسبق تاريخ النهاية تاريخ البداية." : "End date cannot precede start date." })} onValid={(values) => void createEvent(values)} errorSummaryLabel={language === "ar" ? "راجع الحقول المطلوبة." : "Review the required fields."}>
        ...
        <BaseerFormSection title={language === "ar" ? "تفاصيل حدث الشركة" : "Company event details"} description={language === "ar" ? "يظهر لهذا الكيان فقط كسياق للمراجعة، ولا يعدّل المبيعات." : "Visible to this company only as review context; it never changes sales."}><BaseerFormGrid>
          ...
          <label className="baseer-form-field--full">{language === "ar" ? "مرجع أو ملاحظة المصدر" : "Source reference or note"}<BaseerTextInput aria-invalid={Boolean(errors.sourceReference)} maxLength={500} value={eventForm.sourceReference} onChange={(event) => setEventForm((value) => ({ ...value, sourceReference: event.target.value }))} />{errors.sourceReference ? <small role="alert">{errors.sourceReference.message}</small> : null}</label>
    ...
    <BaseerFormDialog open={globalEventDialogOpen} title={globalEventEditing ? (language === "ar" ? "تعديل مناسبة عامة" : "Edit global event") : (language === "ar" ? "إضافة مناسبة عامة" : "Add global event")} language={language} formId="decision-global-event" submitLabel={language === "ar" ? "حفظ المناسبة" : "Save event"} busy={globalEventBusy} error={globalEventError} size="standard" className="decision-context-dialog" onClose={() => !globalEventBusy && setGlobalEventDialogOpen(false)}>
      <BaseerValidatedFormField id="decision-global-event" className="baseer-form" values={globalEventForm} schemaFactory={({ z }) => z.object({ eventKind: z.string().min(1), titleAr: z.string().trim().min(2, language === "ar" ? "أدخل عنواناً من حرفين على الأقل." : "Enter a title of at least two characters.").max(240), startsOn: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, language === "ar" ? "اختر تاريخاً صحيحاً." : "Choose a valid date."), endsOn: z.string().regex(/^\d{4}-\d{2}-\d{2}$/, language === "ar" ? "اختر تاريخاً صحيحاً." : "Choose a valid date."), sourceReference: z.string().max(500), reason: z.string().trim().min(3, language === "ar" ? "أدخل سبباً من ثلاثة أحرف على الأقل." : "Enter a reason of at least three characters.").max(500) }).refine((value) => value.endsOn >= value.startsOn, { path: ["endsOn"], message: language === "ar" ? "يجب ألا يسبق تاريخ النهاية تاريخ البداية." : "End date cannot precede start date." })} onValid={(values) => void submitGlobalEvent(values)} errorSummaryLabel={language === "ar" ? "راجع الحقول المطلوبة." : "Review the required fields."}>
        ...
        <BaseerFormSection title={language === "ar" ? "تفاصيل المناسبة العامة" : "Global event details"} description={language === "ar" ? "تظهر لكل الشركات كسياق زمني، ولا تثبت سبباً للمبيعات." : "Shown to every company as time context; it does not prove a sales cause."}><BaseerFormGrid>
          <label>{language === "ar" ? "نوع المناسبة" : "Event kind"}<BaseerStaticSelect label={language === "ar" ? "نوع المناسبة" : "Event kind"} aria-invalid={Boolean(errors.eventKind)} value={globalEventForm.eventKind} onChange={(event) => setGlobalEventForm((value) => ({ ...value, eventKind: event.target.value }))}><option value="PUBLIC_EVENT">{language === "ar" ? "مناسبة عامة" : "Public event"}</option><option value="ECONOMIC_EVENT">{language === "ar" ? "حدث اقتصادي" : "Economic event"}</option><option value="EXCEPTIONAL_EVENT">{language === "ar" ? "حدث استثنائي" : "Exceptional event"}</option></BaseerStaticSelect>{errors.eventKind ? <small role="alert">{errors.eventKind.message}</small> : null}</label>
          ...
          <label className="baseer-form-field--full">{language === "ar" ? "مرجع أو ملاحظة" : "Reference or note"}<BaseerTextInput aria-invalid={Boolean(errors.sourceReference)} maxLength={500} value={globalEventForm.sourceReference} onChange={(event) => setGlobalEventForm((value) => ({ ...value, sourceReference: event.target.value }))} />{errors.sourceReference ? <small role="alert">{errors.sourceReference.message}</small> : null}</label>
        </BaseerFormGrid></BaseerFormSection>
    ...
    <BaseerFormDialog open={aiExplanation !== null || aiExplanationBusy || aiExplanationError !== null} title={language === "ar" ? "تفسير بصيرة للتنبيه" : "Basira alert explanation"} language={language} formId="decision-alert-explanation" submitLabel={language === "ar" ? "إغلاق" : "Close"} busy={aiExplanationBusy} error={aiExplanationError} onClose={() => { if (!aiExplanationBusy) { setAiExplanation(null); setAiExplanationError(null); } }}>
      <form id="decision-alert-explanation" className="decision-evidence" onSubmit={(event) => { event.preventDefault(); setAiExplanation(null); setAiExplanationError(null); }}>
        {aiExplanation ? <BasiraExplanationContent language={language} explanation={aiExplanation} /> : <p>{language === "ar" ? "جارٍ إعداد التفسير من حزمة الأدلة الموثقة…" : "Preparing an explanation from the verified evidence package…"}</p>}
      </form>
    </BaseerFormDialog>
...
function BasiraExplanationContent({ language, explanation }: { language: Language; explanation: AlertExplanation }) {
  const ar = language === "ar";
  const evidence = presentExplanationItems(explanation.explanation.evidence, ar ? "تُظهر حزمة الأدلة أن البيانات المتاحة للفترة تحتاج مراجعة قبل الاعتماد." : "The evidence package indicates that the available period data needs review before it is relied on.", ar);
  const limitations = presentExplanationItems(explanation.explanation.limitations, ar ? "لا يمكن إصدار حكم تجاري كامل قبل اكتمال البيانات." : "A full commercial conclusion is not possible until the data is complete.", ar);
  const steps = presentExplanationItems(explanation.explanation.reviewSteps, ar ? "راجع اكتمال بيانات المبيعات للفترة المحددة ثم أعد تشغيل الفحص." : "Review sales-data completeness for the selected period, then run the check again.", ar);
  return <div className="decision-evidence__content decision-basira-explanation"><small>{explanation.disposition === "REUSED" ? (ar ? "تفسير محفوظ — لم يُجرَ طلب ذكاء جديد." : "Saved interpretation — no new AI request was made.") : (ar ? "تفسير جديد محفوظ للدليل الحالي." : "A new interpretation was saved for this evidence.")}</small><section className="decision-basira-explanation__summary"><h4>{ar ? "الخلاصة" : "Summary"}</h4><p>{cleanExplanationText(explanation.explanation.summary, ar)}</p></section><div><h4>{ar ? "ما يعنيه التنبيه" : "What this alert means"}</h4><ul className="decision-list">{evidence.map((item, index) => <li key={`evidence-${index}`}>{item}</li>)}</ul></div><div><h4>{ar ? "ما يجب الانتباه له" : "What to keep in mind"}</h4><ul className="decision-list">{limitations.map((item, index) => <li key={`limit-${index}`}>{item}</li>)}</ul></div><div><h4>{ar ? "الخطوة التالية" : "Next step"}</h4><ol className="decision-list">{steps.map((item, index) => <li key={`step-${index}`}>{item}</li>)}</ol></div><details className="decision-basira-explanation__details"><summary>{ar ? "تفاصيل التدقيق" : "Audit details"}</summary><p>{ar ? "لرؤية معرفات الحزمة، صحة التحقق، وسجل التنبيه استخدم «عرض الأدلة» من بطاقة التنبيه. هذه التفاصيل لا تدخل في الخلاصة حتى تبقى سهلة القراءة." : "Use “View evidence” on the alert card for package identifiers, verification status, and the alert record. They are kept out of the summary to make it easy to read."}</p><small>{ar ? `النموذج: ${explanation.model} · تم الإنشاء: ${formatDateTime(explanation.createdAt, "ar", "Asia/Riyadh")}` : `Model: ${explanation.model} · Created: ${formatDateTime(explanation.createdAt, "en", "Asia/Riyadh")}`}</small></details><footer><BaseerButton type="button" variant="secondary" onClick={() => { window.location.hash = pageRouteHash("decision-interpretations"); }}>{ar ? "فتح في مركز التفسيرات" : "Open in interpretation centre"}</BaseerButton></footer><small>{ar ? "التفسير لا يغيّر حالة التنبيه أو أي سجل." : "This explanation does not change the alert or any record."}</small></div>;
}

function presentExplanationItems(items: readonly string[], fallback: string, ar: boolean) {
  const human = items.map((item) => cleanExplanationText(item, ar)).filter((item) => item.length >= 12 && !looksLikeAuditMetadata(item));
  return human.length ? human.slice(0, 3) : [fallback];
}

function cleanExplanationText(value: string, ar: boolean) {
  return value
    .replace(/\b(?:snapshotId|checksum|ruleCode|ruleVersion|status|id)\s*:\s*[^،.\n]+/gi, "")
    .replace(/\b(?:OPEN|CLOSED|ACKNOWLEDGED|SYSTEM_RECONCILED|READY)\b/g, ar ? "" : "")
    .replace(/\s{2,}/g, " ")
    .replace(/^\s*[-:،.]+|[-:،.]+\s*$/g, "")
    .trim();
}

function looksLikeAuditMetadata(value: string) {
  return /(?:\b(?:id|snapshot|checksum|finance\.)|[a-f0-9]{16,}|حالة التنبيه|قاعدة\s*:|تاريخ الإنشاء|صحة\s+checksum|معر[ّ]?ف اللقطة)/i.test(value);
}

function DecisionReadCoordinator({ session, language, section, period, permissionScope, canReadMetrics, canReadContext, canReadAlerts, canManageGlobalContext, canManagePolicies, migrationReviewLocked, onData, onLoading, onError, onRefetch }: { session: ActiveSession; language: Language; section: number; period: BaseerPeriodRange; permissionScope: readonly string[]; canReadMetrics: boolean; canReadContext: boolean; canReadAlerts: boolean; canManageGlobalContext: boolean; canManagePolicies: boolean; migrationReviewLocked: boolean; onData: (reads: DecisionReads) => void; onLoading: (loading: boolean) => void; onError: (reason: unknown) => void; onRefetch: (refetch: () => Promise<void>) => void }) {
  const scope = [language, String(section), period.preset, period.from, period.to, period.months.join(","), [...permissionScope].sort().join(","), String(migrationReviewLocked)];
  return <BaseerCompanyReadQuery session={session} resource="decision-intelligence.workspace" scope={scope} load={(current, signal) => loadDecisionReads(current, signal, { section, period, canReadMetrics, canReadContext, canReadAlerts, canManageGlobalContext, canManagePolicies, migrationReviewLocked })}>
...
function OverviewPanel({ language, metric, comparison, matchedWeekdayComparison, events, openAlerts }: { language: Language; metric: SalesMetric | null; comparison: SalesComparison | null; matchedWeekdayComparison: SalesComparison | null; events: TimelineEvent[]; openAlerts: Alert[] }) {
  const incompleteComparisons = [comparison, matchedWeekdayComparison].filter((item): item is SalesComparison => item !== null && item.dataQuality !== "READY");
  const readyComparisons = [comparison, matchedWeekdayComparison].filter((item): item is SalesComparison => item !== null && item.dataQuality === "READY");
  const hasReadingNotice = (metric !== null && metric.dataQuality !== "READY") || incompleteComparisons.length > 0;
  return <div className="decision-workspace__stack">
    {metric ? <BaseerSummaryMetricGrid ariaLabel={language === "ar" ? "ملخص الفترة" : "Period summary"}><BaseerSummaryMetric label={language === "ar" ? "صافي المبيعات المثبتة" : "Reconciled net sales"} value={formatMoney(metric.payload.netAmount)} icon={<BaseerSectionIcon glyph="wallet" />} accent="success" /><BaseerSummaryMetric label={language === "ar" ? "عدد العملاء" : "Customer count"} value={formatNumber(metric.payload.customerCount)} icon={<BaseerSectionIcon glyph="users" />} accent="info" /><BaseerSummaryMetric label={language === "ar" ? "تغطية الأيام" : "Day coverage"} value={`${metric.coverage.availableDays} / ${metric.coverage.requiredDays}`} icon={<BaseerSectionIcon glyph="calendar" />} accent="warning" /><BaseerSummaryMetric label={language === "ar" ? "جودة البيانات" : "Data quality"} value={<BaseerStatusBadge tone={qualityTone[metric.dataQuality]}>{qualityCopy[language][metric.dataQuality]}</BaseerStatusBadge>} icon={<BaseerSectionIcon glyph="shield" />} accent={metric.dataQuality === "READY" ? "success" : metric.dataQuality === "UNAVAILABLE" || metric.dataQuality === "CONFLICTED" ? "danger" : "warning"} /></BaseerSummaryMetricGrid> : <Empty language={language} message={language === "ar" ? "لا تملك صلاحية قراءة مؤشرات هذا المركز." : "You do not have access to the center metrics."} />}
    {hasReadingNotice ? <BaseerNoticeGroup ariaLabel={language === "ar" ? "تنبيهات جودة القراءة" : "Reading quality notices"}>{metric && metric.dataQuality !== "READY" ? <section className="decision-workspace__notice"><strong>{language === "ar" ? "لا تُفسّر هذه الفترة كتغير تجاري كامل." : "Do not interpret this period as a complete commercial change."}</strong><p>{language === "ar" ? `هناك ${metric.coverage.missingDays.length} يوم ناقص أو غير محسوم. تُعرض القيم المتاحة ولا تُحوّل الأيام الناقصة إلى صفر.` : `${metric.coverage.missingDays.length} days are missing or unsettled. Available values are shown; missing days are never zero-filled.`}</p></section> : null}{incompleteComparisons.map((item) => <SalesComparisonCard key={`${item.comparisonPolicyCode}-${item.current.period.fromBusinessDate}-${item.current.period.toBusinessDate}`} language={language} comparison={item} noticeOnly />)}</BaseerNoticeGroup> : null}
    {readyComparisons.map((item) => <SalesComparisonCard key={`${item.comparisonPolicyCode}-${item.current.period.fromBusinessDate}-${item.current.period.toBusinessDate}`} language={language} comparison={item} />)}
...
function SalesComparisonCard({ language, comparison, noticeOnly = false }: { language: Language; comparison: SalesComparison; noticeOnly?: boolean }) {
  // A comparison is intentionally incomplete until both periods are fully
  // reconciled.  Do not access its withheld (null) figures before this guard.
  const ready = comparison.dataQuality === "READY"
    && comparison.payload.currentNetAmount !== null
    && comparison.payload.comparisonNetAmount !== null
    && comparison.payload.differenceNetAmount !== null
    && comparison.payload.currentCustomerCount !== null
    && comparison.payload.comparisonCustomerCount !== null;
  const weekday = comparison.comparisonPolicyCode === "MATCHED_WEEKDAYS";
  const figures = ready ? comparison.payload : null;
  const differenceNetAmount = figures?.differenceNetAmount;
  const differenceHasValue = typeof differenceNetAmount === "string" && /[1-9]/.test(differenceNetAmount.replace(/^[+-]/, ""));
  const directionKey = !differenceHasValue ? "UNCHANGED" : differenceNetAmount.startsWith("-") ? "DECREASE" : "INCREASE";
  const direction = comparisonDirectionCopy[language][directionKey];
  const content = <><header className="decision-card__header"><div><h3>{weekday ? (language === "ar" ? "مقارنة اليوم نفسه من الأسبوع السابق" : "Same weekday, previous week") : (language === "ar" ? "مقارنة المبيعات بالفترة السابقة المماثلة" : "Sales compared with the previous equal period")}</h3><p>{weekday ? (language === "ar" ? "قراءة يومية فقط تقارن التاريخ المختار بالتاريخ نفسه قبل سبعة أيام. لا تثبت السبب." : "A daily-only read comparing the selected date with the same weekday seven days earlier. It does not prove cause.") : (language === "ar" ? "نفس عدد الأيام مباشرة قبل الفترة المحددة. هذه قراءة وصفية وليست إثباتاً للسبب." : "The same number of days immediately before the selected period. This is descriptive, not proof of cause.")}</p></div><BaseerStatusBadge tone={qualityTone[comparison.dataQuality]}>{qualityCopy[language][comparison.dataQuality]}</BaseerStatusBadge></header>{figures ? <BaseerSummaryMetricGrid ariaLabel={language === "ar" ? "مقارنة المبيعات" : "Sales comparison"}><BaseerSummaryMetric label={language === "ar" ? "الفترة الحالية" : "Current period"} value={formatMoney(figures.currentNetAmount)} /><BaseerSummaryMetric label={language === "ar" ? "الفترة السابقة" : "Previous period"} value={formatMoney(figures.comparisonNetAmount)} /><BaseerSummaryMetric label={language === "ar" ? `الفرق (${direction})` : `${direction} difference`} value={formatMoney(figures.differenceNetAmount)} /><BaseerSummaryMetric label={language === "ar" ? "نسبة التغير" : "Change rate"} value={formatPercent(figures.percentDifference, language)} /></BaseerSummaryMetricGrid> : <p>{language === "ar" ? "لا يصدر المركز حكماً عن التغير لأن إحدى الفترتين ناقصة أو غير متاحة. راجع جودة البيانات أولاً." : "The center does not judge the change because one period is incomplete or unavailable. Review data quality first."}</p>}</>;
  return noticeOnly ? <section className="decision-workspace__notice decision-sales-comparison">{content}</section> : <BaseerCard className="decision-sales-comparison">{content}</BaseerCard>;
}
...
function EvidenceDetails({ language, evidence }: { language: Language; evidence: AlertEvidence }) {
  ...
  return <div className="decision-evidence__content">
    ...
    {evidence.actions.length ? <div><h4>{copy.lifecycle}</h4><ul className="decision-list">{evidence.actions.map((action, index) => <li key={`${action.createdAt}-${index}`}><strong>{action.action === "ACKNOWLEDGED" ? copy.acknowledged : copy.closed}</strong><small>{action.reason} · {formatDateTime(action.createdAt, language, "Asia/Riyadh")}</small></li>)}</ul></div> : null}
  </div>;
}
...
function SalesChangePolicyCard({ language, policy, onSavePolicy, onEvaluate }: { language: Language; policy: SalesChangePolicy | null; onSavePolicy: (input: { enabled: boolean; decreaseThresholdBasisPoints: number | null; increaseThresholdBasisPoints: number | null; minimumBaselineAmount: string | null; minimumAbsoluteDifferenceAmount: string | null; cooldownHours: number | null }) => Promise<void>; onEvaluate: () => Promise<void> }) {
  ...
  return <BaseerCard className="decision-sales-policy"><BaseerValidatedFormField id="decision-sales-policy" className="decision-sales-policy__form" values={values} schemaFactory={({ z }) => z.object({ enabled: z.boolean(), decrease: z.string(), increase: z.string(), baseline: z.string(), difference: z.string(), cooldownHours: z.string() }).superRefine((value, context) => { if (!value.enabled) return; if (!exactBasisPoints(value.decrease)) context.addIssue({ code: "custom", path: ["decrease"], message: language === "ar" ? "أدخل نسبة بين 0.01 و100 بدقتين عشريتين كحد أقصى." : "Enter a percentage from 0.01 to 100 with up to two decimals." }); if (!exactBasisPoints(value.increase)) context.addIssue({ code: "custom", path: ["increase"], message: language === "ar" ? "أدخل نسبة بين 0.01 و100 بدقتين عشريتين كحد أقصى." : "Enter a percentage from 0.01 to 100 with up to two decimals." }); if (!exactPositiveDecimal(value.baseline)) context.addIssue({ code: "custom", path: ["baseline"], message: language === "ar" ? "أدخل مبلغاً موجباً حتى أربع خانات عشرية." : "Enter a positive amount with up to four decimals." }); if (!exactPositiveDecimal(value.difference)) context.addIssue({ code: "custom", path: ["difference"], message: language === "ar" ? "أدخل مبلغاً موجباً حتى أربع خانات عشرية." : "Enter a positive amount with up to four decimals." }); if (!exactHours(value.cooldownHours)) context.addIssue({ code: "custom", path: ["cooldownHours"], message: language === "ar" ? "أدخل عدداً صحيحاً من 1 إلى 720." : "Enter an integer from 1 to 720." }); })} onValid={(next) => void save(next)} errorSummaryLabel={language === "ar" ? "راجع قيم سياسة التنبيه." : "Review the alert-policy values."}>{({ errors }) => <><header className="decision-card__header"><div><h3>{copy.title}</h3><p>{copy.description}</p></div><BaseerStatusBadge tone={enabled ? "success" : "neutral"}>{enabled ? copy.active : copy.disabled}</BaseerStatusBadge></header><label className="decision-sales-policy__toggle"><BaseerCheckbox checked={enabled} disabled={busy} onChange={(event) => setEnabled(event.target.checked)} />{copy.enabled}</label><div className="decision-sales-policy__thresholds"><label>{copy.decrease}<BaseerNumericSuffixInput aria-invalid={Boolean(errors.decrease)} disabled={!enabled || busy} suffix={copy.percent} suffixDirection={language === "ar" ? "rtl" : "ltr"} value={decrease} onValueChange={setDecrease} />{errors.decrease ? <small role="alert">{errors.decrease.message}</small> : null}</label><label>{copy.increase}<BaseerNumericSuffixInput aria-invalid={Boolean(errors.increase)} disabled={!enabled || busy} suffix={copy.percent} suffixDirection={language === "ar" ? "rtl" : "ltr"} value={increase} onValueChange={setIncrease} />{errors.increase ? <small role="alert">{errors.increase.message}</small> : null}</label><label>{copy.baseline}<BaseerMoneyInput aria-invalid={Boolean(errors.baseline)} disabled={!enabled || busy} value={baseline} onValueChange={setBaseline} />{errors.baseline ? <small role="alert">{errors.baseline.message}</small> : null}</label><label>{copy.difference}<BaseerMoneyInput aria-invalid={Boolean(errors.difference)} disabled={!enabled || busy} value={difference} onValueChange={setDifference} />{errors.difference ? <small role="alert">{errors.difference.message}</small> : null}</label><label>{copy.cooldown}<BaseerNumericSuffixInput aria-invalid={Boolean(errors.cooldownHours)} disabled={!enabled || busy} kind="integer" suffix={copy.hours} suffixDirection={language === "ar" ? "rtl" : "ltr"} value={cooldownHours} onValueChange={setCooldownHours} />{errors.cooldownHours ? <small role="alert">{errors.cooldownHours.message}</small> : null}</label></div><footer><BaseerButton type="submit" variant="secondary" disabled={busy || invalidPolicy}>{copy.save}</BaseerButton><BaseerButton type="button" disabled={busy || invalidPolicy} onClick={() => void evaluate()}>{copy.run}</BaseerButton></footer></>}</BaseerValidatedFormField></BaseerCard>;
}
