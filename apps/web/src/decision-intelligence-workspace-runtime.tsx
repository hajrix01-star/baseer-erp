...
import { BaseerCheckbox, BaseerTextArea, BaseerTextInput } from "./baseer-form-fields";
...
import { formatDateTime, formatMoney, formatNumber, normalizeBaseerNumericInput } from "./number-format";
...
export function DecisionIntelligenceWorkspaceRuntime({ language, section, permissionCodes }: { language: Language; section: number; permissionCodes: readonly string[] | null }) {
...
function SalesChangePolicyCard({ language, policy, onSavePolicy, onEvaluate }: { language: Language; policy: SalesChangePolicy | null; onSavePolicy: (input: { enabled: boolean; decreaseThresholdBasisPoints: number | null; increaseThresholdBasisPoints: number | null; minimumBaselineAmount: string | null; minimumAbsoluteDifferenceAmount: string | null; cooldownHours: number | null }) => Promise<void>; onEvaluate: () => Promise<void> }) {
  ...
  useEffect(() => {
    const fieldSetters = [setDecrease, setIncrease, setBaseline, setDifference, setCooldownHours] as const;
    const normalisePolicyInput = (event: Event) => {
      const target = event.target;
      if (!(target instanceof HTMLInputElement) || !target.closest(".decision-sales-policy__thresholds")) return;
      target.dir = "ltr"; target.lang = "en";
      const normalised = normalizeBaseerNumericInput(target.value, { allowNegative: false });
      if (normalised === target.value) return;
      Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, "value")?.set?.call(target, normalised);
      const index = [...document.querySelectorAll<HTMLInputElement>(".decision-sales-policy__thresholds input")].indexOf(target);
      fieldSetters[index]?.(normalised);
    };
    document.querySelectorAll<HTMLInputElement>(".decision-sales-policy__thresholds input").forEach((input) => { input.dir = "ltr"; input.lang = "en"; });
    document.addEventListener("input", normalisePolicyInput, true);
    return () => document.removeEventListener("input", normalisePolicyInput, true);
  }, []);
  useEffect(() => {
