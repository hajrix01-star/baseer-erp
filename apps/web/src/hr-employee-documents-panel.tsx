...
export function HrEmployeeDocumentsPanel({ employeeId, language, onError, onChanged }: { employeeId: string; language: Language; onError: (message: string) => void; onChanged: () => Promise<void> }) {
  ...
  const create = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (draft.file && !isAcceptedFile(draft.file)) { onError(ar ? "المسموح PDF أو JPG أو PNG وبحد أقصى 5 ميجابايت." : "Use a PDF, JPG, or PNG file up to 5 MiB."); return; }
    setBusy(true);
    try {
  ...
  const replace = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault(); const session = activeSession(); if (!session || !selected || !replacement) return;
    if (!isAcceptedFile(replacement)) { onError(ar ? "المسموح PDF أو JPG أو PNG وبحد أقصى 5 ميجابايت." : "Use a PDF, JPG, or PNG file up to 5 MiB."); return; }
    setBusy(true);
  ...
  return <><div className="baseer-inline-actions"><label>{ar ? "النوع" : "Type"}<select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as "" | HrEmployeeDocumentType)}><option value="">{ar ? "كل الأنواع" : "All types"}</option>{(["NATIONAL_ID", "IQAMA", "PASSPORT", "EMPLOYMENT_CONTRACT", "MEDICAL_INSURANCE", "HEALTH_CERTIFICATE", "QUALIFICATION", "OTHER"] as const).map((type) => <option key={type} value={type}>{documentTypeLabel(type)}</option>)}</select></label><label>{ar ? "الحالة" : "Status"}<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as "" | HrEmployeeDocumentStatus)}><option value="">{ar ? "كل الحالات" : "All statuses"}</option><option value="ACTIVE">{statusLabel("ACTIVE")}</option><option value="REVOKED">{statusLabel("REVOKED")}</option></select></label><label>{ar ? "الانتهاء" : "Expiry"}<select value={expiryFilter} onChange={(event) => setExpiryFilter(event.target.value as FilterExpiry)}><option value="">{ar ? "الكل" : "All"}</option><option value="VALID">{complianceLabel("VALID")}</option><option value="EXPIRING">{complianceLabel("EXPIRING")}</option><option value="EXPIRED">{complianceLabel("EXPIRED")}</option><option value="NONE">{ar ? "دون تاريخ انتهاء" : "No expiry"}</option></select></label><BaseerButton type="button" onClick={() => { setDraft(emptyDraft()); setCreateOpen(true); }}>{ar ? "إضافة مستند" : "Add document"}</BaseerButton></div>{loading ? <BaseerCard>{ar ? "جارٍ تحميل المستندات…" : "Loading documents…"}</BaseerCard> : documents.length ? <DataTable ariaLabel={ar ? "مستندات الموظف" : "Employee documents"} caption={ar ? "مستندات الموظف" : "Employee documents"} rows={documents} columns={columns} rowKey={(row) => row.id} /> : <BaseerCard>{ar ? "لا توجد مستندات مطابقة." : "No matching documents."}</BaseerCard>}{cursor ? <BaseerButton type="button" variant="secondary" onClick={() => void load(cursor, true)}>{ar ? "تحميل المزيد" : "Load more"}</BaseerButton> : null}
    <BaseerDialog open={createOpen} title={ar ? "إضافة مستند" : "Add document"} language={language} busy={busy} onClose={() => setCreateOpen(false)} footer={<><BaseerButton type="button" variant="secondary" disabled={busy} onClick={() => setCreateOpen(false)}>{ar ? "إلغاء" : "Cancel"}</BaseerButton><BaseerButton type="submit" form="hr-employee-document-create" disabled={busy}>{ar ? "حفظ المستند" : "Save document"}</BaseerButton></>}><form id="hr-employee-document-create" className="administration-form" onSubmit={(event) => void create(event)}><label>{ar ? "النوع" : "Type"}<select value={draft.documentType} onChange={(event) => setDraft((value) => ({ ...value, documentType: event.target.value as HrEmployeeDocumentType }))}>{(["NATIONAL_ID", "IQAMA", "PASSPORT", "EMPLOYMENT_CONTRACT", "MEDICAL_INSURANCE", "HEALTH_CERTIFICATE", "QUALIFICATION", "OTHER"] as const).map((type) => <option key={type} value={type}>{documentTypeLabel(type)}</option>)}</select></label><label>{ar ? "العنوان" : "Title"}<input required value={draft.title} onChange={(event) => setDraft((value) => ({ ...value, title: event.target.value }))} /></label><label>{ar ? "المرجع" : "Reference"}<input value={draft.referenceNumber} onChange={(event) => setDraft((value) => ({ ...value, referenceNumber: event.target.value }))} /></label><BaseerDatePicker language={language} label={ar ? "تاريخ الإصدار" : "Issue date"} clearable value={draft.issueDate} onChange={(issueDate) => setDraft((value) => ({ ...value, issueDate }))} /><BaseerDatePicker language={language} label={ar ? "تاريخ الانتهاء" : "Expiry date"} clearable value={draft.expiryDate} onChange={(expiryDate) => setDraft((value) => ({ ...value, expiryDate }))} /><BaseerDatePicker language={language} label={ar ? "الاحتفاظ حتى" : "Retain until"} clearable value={draft.retentionUntil} onChange={(retentionUntil) => setDraft((value) => ({ ...value, retentionUntil }))} /><label>{ar ? "الملف (اختياري)" : "File (optional)"}<input accept={acceptedFileInput} type="file" onChange={(event) => setDraft((value) => ({ ...value, file: event.target.files?.[0] ?? null }))} /></label><label><input type="checkbox" checked={draft.legalHold} onChange={(event) => setDraft((value) => ({ ...value, legalHold: event.target.checked }))} /> {ar ? "حجز قانوني" : "Legal hold"}</label><label>{ar ? "ملاحظات" : "Notes"}<textarea value={draft.notes} onChange={(event) => setDraft((value) => ({ ...value, notes: event.target.value }))} /></label></form></BaseerDialog>
    <BaseerDialog open={Boolean(selected)} title={selected?.title ?? ""} language={language} busy={busy} onClose={() => setSelected(null)} footer={selected ? <>{selected.status === "ACTIVE" && selected.currentVersion?.blobStatus === "READY" ? <BaseerButton type="button" variant="secondary" disabled={busy} onClick={() => void download()}>{ar ? "تنزيل الملف" : "Download file"}</BaseerButton> : null}<BaseerButton type="button" variant="secondary" disabled={busy || selected.status !== "ACTIVE"} onClick={() => { setReplacement(null); setReplaceOpen(true); }}>{ar ? "استبدال الملف" : "Replace file"}</BaseerButton><BaseerButton type="button" variant="danger" disabled={busy || selected.status !== "ACTIVE"} onClick={() => { setReason(""); setRevokeOpen(true); }}>{ar ? "إلغاء المستند" : "Revoke document"}</BaseerButton></> : undefined}>{selected ? <dl className="administration-details"><div><dt>{ar ? "النوع" : "Type"}</dt><dd>{documentTypeLabel(selected.documentType)}</dd></div><div><dt>{ar ? "المرجع" : "Reference"}</dt><dd>{selected.referenceNumber ?? "—"}</dd></div><div><dt>{ar ? "التواريخ" : "Dates"}</dt><dd>{`${selected.issueDate ?? "—"} — ${selected.expiryDate ?? "—"}`}</dd></div><div><dt>{ar ? "الامتثال" : "Compliance"}</dt><dd>{complianceLabel(selected.complianceStatus)}</dd></div><div><dt>{ar ? "الملف" : "File"}</dt><dd>{selected.currentVersion ? `v${selected.currentVersion.version} · ${selected.currentVersion.mimeType} · ${selected.currentVersion.blobStatus}` : "—"}</dd></div>{downloadAvailability(selected) ? <div><dt>{ar ? "إتاحة التنزيل" : "Download availability"}</dt><dd>{downloadAvailability(selected)}</dd></div> : null}{selected.notes ? <div><dt>{ar ? "ملاحظات" : "Notes"}</dt><dd>{selected.notes}</dd></div> : null}</dl> : null}</BaseerDialog>
    <BaseerDialog open={replaceOpen} title={ar ? "استبدال ملف المستند" : "Replace document file"} language={language} busy={busy} onClose={() => setReplaceOpen(false)} footer={<><BaseerButton type="button" variant="secondary" disabled={busy} onClick={() => setReplaceOpen(false)}>{ar ? "إلغاء" : "Cancel"}</BaseerButton><BaseerButton type="submit" form="hr-employee-document-replace" disabled={busy}>{ar ? "استبدال" : "Replace"}</BaseerButton></>}><form id="hr-employee-document-replace" className="administration-form" onSubmit={(event) => void replace(event)}><label>{ar ? "الملف الجديد" : "New file"}<input required accept={acceptedFileInput} type="file" onChange={(event) => setReplacement(event.target.files?.[0] ?? null)} /></label></form></BaseerDialog>
    <BaseerDialog open={revokeOpen} title={ar ? "إلغاء المستند" : "Revoke document"} language={language} busy={busy} onClose={() => setRevokeOpen(false)} footer={<><BaseerButton type="button" variant="secondary" disabled={busy} onClick={() => setRevokeOpen(false)}>{ar ? "رجوع" : "Back"}</BaseerButton><BaseerButton type="submit" variant="danger" form="hr-employee-document-revoke" disabled={busy}>{ar ? "تأكيد الإلغاء" : "Confirm revocation"}</BaseerButton></>}><form id="hr-employee-document-revoke" className="administration-form" onSubmit={(event) => void revoke(event)}><label>{ar ? "سبب الإلغاء" : "Revocation reason"}<textarea required value={reason} onChange={(event) => setReason(event.target.value)} /></label></form></BaseerDialog>
  </>;
}
