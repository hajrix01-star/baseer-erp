...
export function HrEmployeeDocumentsPanel({ employeeId, language, onError, onChanged }: { employeeId: string; language: Language; onError: (message: string) => void; onChanged: () => Promise<void> }) {
  ...
  return <><section className="hr-document-filters" aria-label={ar ? "تصفية المستندات" : "Document filters"}><div className="hr-document-filters__fields"><label>{ar ? "النوع" : "Type"}<select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as "" | HrEmployeeDocumentType)}><option value="">{ar ? "كل الأنواع" : "All types"}</option>{(["NATIONAL_ID", "IQAMA", "PASSPORT", "EMPLOYMENT_CONTRACT", "MEDICAL_INSURANCE", "HEALTH_CERTIFICATE", "QUALIFICATION", "OTHER"] as const).map((type) => <option key={type} value={type}>{documentTypeLabel(type)}</option>)}</select></label><label>{ar ? "الحالة" : "Status"}<select value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as "" | HrEmployeeDocumentStatus)}><option value="">{ar ? "كل الحالات" : "All statuses"}</option><option value="ACTIVE">{statusLabel("ACTIVE")}</option><option value="REVOKED">{statusLabel("REVOKED")}</option></select></label><label>{ar ? "الانتهاء" : "Expiry"}<select value={expiryFilter} onChange={(event) => setExpiryFilter(event.target.value as FilterExpiry)}><option value="">{ar ? "الكل" : "All"}</option><option value="VALID">{complianceLabel("VALID")}</option><option value="EXPIRING">{complianceLabel("EXPIRING")}</option><option value="EXPIRED">{complianceLabel("EXPIRED")}</option><option value="NONE">{ar ? "دون تاريخ انتهاء" : "No expiry"}</option></select></label></div><BaseerButton type="button" onClick={() => { setDraft(emptyDraft()); setCreateOpen(true); }}>{ar ? "إضافة مستند" : "Add document"}</BaseerButton></section>{loading ? <BaseerCard>{ar ? "جارٍ تحميل المستندات…" : "Loading documents…"}</BaseerCard> : documents.length ? <DataTable ariaLabel={ar ? "مستندات الموظف" : "Employee documents"} caption={ar ? "مستندات الموظف" : "Employee documents"} rows={documents} columns={columns} rowKey={(row) => row.id} /> : <BaseerCard>{ar ? "لا توجد مستندات مطابقة." : "No matching documents."}</BaseerCard>}{cursor ? <BaseerButton type="button" variant="secondary" onClick={() => void load(cursor, true)}>{ar ? "تحميل المزيد" : "Load more"}</BaseerButton> : null}
    <BaseerFormDialog open={createOpen} title={ar ? "إضافة مستند" : "Add document"} language={language} busy={busy} size="standard" formId="hr-employee-document-create" submitLabel={ar ? "حفظ المستند" : "Save document"} onClose={() => setCreateOpen(false)}><form id="hr-employee-document-create" className="baseer-form" onSubmit={(event) => void create(event)}><BaseerFormSection title={ar ? "بيانات المستند" : "Document details"}><BaseerFormGrid><label>{ar ? "النوع" : "Type"}<select value={draft.documentType} onChange={(event) => setDraft((value) => ({ ...value, documentType: event.target.value as HrEmployeeDocumentType }))}>{(["NATIONAL_ID", "IQAMA", "PASSPORT", "EMPLOYMENT_CONTRACT", "MEDICAL_INSURANCE", "HEALTH_CERTIFICATE", "QUALIFICATION", "OTHER"] as const).map((type) => <option key={type} value={type}>{documentTypeLabel(type)}</option>)}</select></label><label>{ar ? "العنوان" : "Title"}<input required value={draft.title} onChange={(event) => setDraft((value) => ({ ...value, title: event.target.value }))} /></label><label className="baseer-form-field--full">{ar ? "المرجع" : "Reference"}<input value={draft.referenceNumber} onChange={(event) => setDraft((value) => ({ ...value, referenceNumber: event.target.value }))} /></label><BaseerDatePicker language={language} label={ar ? "تاريخ الإصدار" : "Issue date"} clearable value={draft.issueDate} onChange={(issueDate) => setDraft((value) => ({ ...value, issueDate }))} /><BaseerDatePicker language={language} label={ar ? "تاريخ الانتهاء" : "Expiry date"} clearable value={draft.expiryDate} onChange={(expiryDate) => setDraft((value) => ({ ...value, expiryDate }))} /><BaseerDatePicker language={language} label={ar ? "الاحتفاظ حتى" : "Retain until"} clearable value={draft.retentionUntil} onChange={(retentionUntil) => setDraft((value) => ({ ...value, retentionUntil }))} /><label className="baseer-form-field--full">{ar ? "الملف (اختياري)" : "File (optional)"}<input accept={acceptedFileInput} type="file" onChange={(event) => setDraft((value) => ({ ...value, file: event.target.files?.[0] ?? null }))} /></label><label className="baseer-form-checkbox"><input type="checkbox" checked={draft.legalHold} onChange={(event) => setDraft((value) => ({ ...value, legalHold: event.target.checked }))} /> {ar ? "حجز قانوني" : "Legal hold"}</label><label className="baseer-form-field--full">{ar ? "ملاحظات" : "Notes"}<BaseerTextArea compact value={draft.notes} onValueChange={(notes) => setDraft((value) => ({ ...value, notes }))} /></label></BaseerFormGrid></BaseerFormSection></form></BaseerFormDialog>
    ...
    <BaseerFormDialog open={replaceOpen} title={ar ? "استبدال ملف المستند" : "Replace document file"} language={language} busy={busy} size="compact" formId="hr-employee-document-replace" submitLabel={ar ? "استبدال" : "Replace"} onClose={() => setReplaceOpen(false)}><form id="hr-employee-document-replace" className="baseer-form" onSubmit={(event) => void replace(event)}><BaseerFormSection title={ar ? "الملف الجديد" : "New file"}><BaseerFormGrid columns="one"><label>{ar ? "الملف الجديد" : "New file"}<input required accept={acceptedFileInput} type="file" onChange={(event) => setReplacement(event.target.files?.[0] ?? null)} /></label></BaseerFormGrid></BaseerFormSection></form></BaseerFormDialog>
