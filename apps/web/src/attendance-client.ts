...
export type AttendanceEmployeePortalProfile = { companyId: string; employeeId: string; employeeNumber: string; employeeNameAr: string; employeeNameEn: string | null; businessDate: string; state: "READY" | "IN_PROGRESS"; commitment: { score: number | null; plannedMinutes: number; shortageMinutes: number; evaluatedDays: number }; schedule: Array<{ businessDate: string; source: "ROSTER" | "EXCEPTION" | "WEEKLY_ADJUSTMENT" | "TEMPLATE" | "NONE"; kind: "FULL_REST" | "CUSTOM_PERIODS" | null; periods: Array<{ startTime: string; endTime: string; endsNextDay: boolean; minutes: number }> }> };
...
export function listAttendanceEmployeeSchedules(session: ActiveSession, options?: RequestInit) { return api<{ schedules: AttendanceEmployeeSchedule[] }>(session, "/attendance/employees/schedules", options); }
