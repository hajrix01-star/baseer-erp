...
export type AttendanceEmployeePortalPresentation = { companyNameAr: string; companyNameEn: string; hasCompanyLogo: boolean };
...
export function listAttendanceOpenSessionsForClose(session: ActiveSession, { cursor, pageSize = 30, ...options }: RequestInit & { cursor?: string; pageSize?: number } = {}) { const query = new URLSearchParams({ pageSize: String(pageSize) }); if (cursor) query.set("cursor", cursor); return api<AttendanceOpenSessionsReceipt>(session, `/attendance/sessions/open?${query}`, options); }
...
export function getAttendanceScheduleWorkspace(session: ActiveSession, date?: string, options?: RequestInit) { return api<{ templates: AttendanceScheduleTemplateReceipt[]; coverage: AttendanceCoverage; roster: AttendanceRoster; employeeSchedules: AttendanceEmployeeSchedulesReceipt }>(session, `/attendance/schedule-workspace${date ? `?date=${encodeURIComponent(date)}` : ""}`, options); }
...
export function getAttendanceEmployeePortalPresentation(tenantId: string, companyId: string) { const query = new URLSearchParams({ tenantId, companyId }); return employeePortalApi<AttendanceEmployeePortalPresentation>(`/attendance/portal/presentation?${query.toString()}`, { method: "GET" }); }
/** This is a presentation asset only. Scope is validated again by the API; the
 * URL never includes a storage reference or an employee/session credential. */
export function attendanceEmployeePortalCompanyLogoUrl(tenantId: string, companyId: string) { const query = new URLSearchParams({ tenantId, companyId }); return `${baseerApiBaseUrl}/attendance/portal/logo?${query.toString()}`; }
