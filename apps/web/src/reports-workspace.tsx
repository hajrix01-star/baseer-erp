...
function ReportTableRow({ row, depth, label, hasChildren, expanded, onToggle, onOpen }: { row: ReportRow; depth: number; label: string; hasChildren: boolean; expanded: boolean; onToggle: () => void; onOpen: () => void }) {
  ...
  const action = hasChildren
    ...
    : labelButton;
  return <tr className={`reports-prototype__row reports-prototype__row--${row.kind === 'SECTION' ? 'section' : 'line'} ${row.direction === 'INFLOW' ? 'is-inflow' : 'is-outflow'}`}><th scope="row" style={{ '--report-level': depth - 1 } as React.CSSProperties}>{action}</th><td><MoneyButton money={row.amount} label={label} onClick={onOpen} /></td></tr>;
}
...
function EvidenceDetail({ language, text, row, evidence, onOpenSource, onLoadMore }: { language: Language; text: typeof copy[Language]; row: ReportRow; evidence: EvidenceReceipt | null; onOpenSource: (eventId: string) => void; onLoadMore: () => void }) {
  ...
  return <div className="reports-prototype__detail" dir={language === 'ar' ? 'rtl' : 'ltr'}><div className="reports-prototype__drawer-total"><span>{label}</span><bdi dir="ltr">{row.amount.sign === 'negative' ? '−' : ''}{row.amount.display}</bdi></div><div className="reports-prototype__sales-share"><span>{text.salesShare}</span><bdi dir="ltr">{row.shareOfCollectedSalesPercent ? `${row.shareOfCollectedSalesPercent}%` : '—'}</bdi></div>{!evidence ? <p>{text.loading}</p> : evidence.items.length ? <><ul className="reports-prototype__source-list">{evidence.items.map((item) => <li key={item.eventId}><div><span>{language === 'ar' ? item.source.labelAr : item.source.labelEn}</span><strong dir="ltr">{item.source.reference}</strong><small>{item.businessDate}</small></div><div className="reports-prototype__source-value"><bdi dir="ltr">{item.amount.sign === 'negative' ? '−' : ''}{item.amount.display}</bdi><button type="button" onClick={() => onOpenSource(item.eventId)}>{text.openSource} <span aria-hidden="true">‹</span></button></div></li>)}</ul>{evidence.nextCursor ? <BaseerButton type="button" variant="secondary" onClick={onLoadMore}>{text.loadMore}</BaseerButton> : null}</> : <p>{text.noEvidence}</p>}</div>;
}
