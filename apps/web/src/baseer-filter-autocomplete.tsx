...
export function BaseerFilterAutocomplete({ id, label, placeholder, values, options, onChange }: Props) {
  ...
  return <div ref={rootRef} className="company-session-control">
    <label><span className="visually-hidden">{label}</span><input id={id} className="baseer-filter-bar__select" style={{ paddingInlineEnd: "2rem" }} role="combobox" aria-autocomplete="list" aria-expanded={open} aria-controls={`${id}-options`} autoComplete="off" value={displayValue} placeholder={placeholder} onFocus={() => { setOpen(true); setQuery(""); }} onChange={(event) => { setQuery(event.target.value); setOpen(true); }} onKeyDown={(event) => { if (event.key === "Escape") { setOpen(false); setQuery(""); } if (event.key === "Enter" && matches.length === 1) { event.preventDefault(); toggle(matches[0]!); } }} /></label><span aria-hidden="true" style={{ pointerEvents: "none", position: "absolute", insetInlineEnd: "0.65rem", insetBlockStart: "50%", transform: "translateY(-50%)", color: "var(--brand-deep)", fontSize: "0.8rem" }}>▾</span>
