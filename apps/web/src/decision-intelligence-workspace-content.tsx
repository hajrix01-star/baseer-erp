...
type AlertExplanation = { receiptId: string; alertId: string; skillKey: "decision.command_center_analyst"; model: string; explanation: { summary: string; evidence: string[]; limitations: string[]; reviewSteps: string[] }; createdAt: string; replayed: boolean };
...
export function DecisionIntelligenceWorkspaceContent({ language, section, permissionCodes }: { language: Language; section: number; permissionCodes: readonly string[] | null }) {
  ...
  }, [alerts, candidates, canGiveFeedback, canManageAlerts, canManageCompanyContext, canManageGlobalContext, canManagePolicies, canUseAi, companyEvents, comparison, events, globalEvents, language, load, matchedWeekdayComparison, metric, openAlerts, period.from, period.to, researchBusy, reviews, salesChangePolicy, section, sourceHealth]);
