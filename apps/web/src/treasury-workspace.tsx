...
export function TreasuryWorkspace({ language }: { language: Language }) {
  ...
  return <section className="daily-sales-workspace finance-setup-workspace" aria-label={text.vaults}>
    ...
    {!workspace ? <BaseerCard><p>{text.loadingVaults}</p></BaseerCard> : <>
      <div className="administration-role-cards treasury-summary-cards"><SummaryCard label={text.currentBalance} value={workspace.summary.balanceAsOf} /><SummaryCard label={text.incoming} value={workspace.summary.inflow} /><SummaryCard label={text.outgoing} value={workspace.summary.outflow} /></div>
...
function SummaryCard({ label, value }: { label: string; value: string }) { return <BaseerCard padding="compact" tone="muted"><span>{label}</span><strong>{formatMoney(value)}</strong></BaseerCard>; }
