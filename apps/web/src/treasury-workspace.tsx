...
function ActivityPanel({ activity, language, text, onMore, onReverseTransfer }: { activity: ActivityReceipt; language: Language; text: ReturnType<typeof financeText>; onMore: () => void; onReverseTransfer: (item: ActivityItem) => void }) { const columns: DataTableColumn<ActivityItem>[] = [{ id: "date", header: language === "ar" ? "التاريخ" : "Date", cell: (item) => item.businessDate }, { id: "reference", header: language === "ar" ? "المرجع" : "Reference", cell: (item) => item.description ?? item.sourceReference }, { id: "counterpart", header: language === "ar" ? "الطرف المقابل" : "Counterpart", cell: (item) => language === "ar" ? item.counterpartNameAr ?? "—" : item.counterpartNameEn ?? "—" }, { id: "in", header: text.incoming, numeric: true, align: "end", cell: (item) => formatMoney(item.inflow) }, { id: "out", header: text.outgoing, numeric: true, align: "end", cell: (item) => formatMoney(item.outflow) }, { id: "actions", header: text.operation, align: "end", cell: (item) => item.sourceType === "vault_transfer" ? <BaseerButton type="button" variant="secondary" onClick={() => onReverseTransfer(item)}>{text.reverseTransfer}</BaseerButton> : "—" }]; return <><div className="baseer-card-grid"><BaseerSummaryMetric tone="muted" label={text.currentBalance} value={formatMoney(activity.summary.balanceAsOf)} /><BaseerSummaryMetric tone="muted" label={text.incoming} value={formatMoney(activity.summary.inflow)} /><BaseerSummaryMetric tone="muted" label={text.outgoing} value={formatMoney(activity.summary.outflow)} /></div>{activity.items.length ? <DataTable ariaLabel={text.vaultActivity} caption={text.vaultActivity} columns={columns} rows={activity.items} rowKey={(item) => item.id} /> : <p className="empty-results">{text.noMovements}</p>}{activity.nextCursor ? <BaseerButton type="button" disabled={false} onClick={onMore}>{text.loadMore}</BaseerButton> : null}</>; }
