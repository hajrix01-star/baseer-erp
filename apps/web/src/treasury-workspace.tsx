...
export function TreasuryWorkspace({ language }: { language: Language }) {
  ...
  const transferSource = activeVaults.find((vault) => vault.id === transfer.fromVaultId) ?? null;
  const transferFromLabel = language === "ar" ? "تحويل من هذه الخزينة" : "Transfer from this vault";
  ...
  return <section className="daily-sales-workspace finance-setup-workspace" aria-label={text.vaults}>
    <header className="administration-section-heading"><div><p className="eyebrow">{text.finance}</p><h3>{text.vaults}</h3></div><div className="page-actions"><BaseerButton type="button" variant="secondary" onClick={openOrder}>{orderLabel}</BaseerButton><BaseerButton type="button" variant="secondary" onClick={openControl}>{text.treasuryControl}</BaseerButton><BaseerButton type="button" variant="primary" onClick={openAdd}>{text.addVault}</BaseerButton></div></header>
    ...
    <BaseerDialog open={showTransfer} language={language} busy={saving} title={text.transfer} onClose={() => setShowTransfer(false)} footer={<><BaseerButton type="button" onClick={() => setShowTransfer(false)}>{text.cancel}</BaseerButton><BaseerButton type="submit" form="vault-transfer" variant="primary" disabled={saving || !transferSource}>{saving ? text.saving : text.saveTransfer}</BaseerButton></>}>
      <form id="vault-transfer" className="administration-form" onSubmit={(event) => void saveTransfer(event)}>
        <label>{text.fromVault}<input readOnly value={transferSource ? displayName(language, transferSource) : ""} /></label>
        <label>{text.toVault}<select required value={transfer.toVaultId} onChange={(event) => setTransfer((value) => ({ ...value, toVaultId: event.target.value }))}><option value="">{text.selectVault}</option>{activeVaults.filter((vault) => vault.id !== transfer.fromVaultId).map((vault) => <option key={vault.id} value={vault.id}>{displayName(language, vault)}</option>)}</select></label>
        <label>{text.amount}<input required inputMode="decimal" value={transfer.amount} onChange={(event) => setTransfer((value) => ({ ...value, amount: event.target.value }))} /></label>
        <BaseerDatePicker language={language} label={text.transferDate} max={workspace?.businessDate ?? ""} value={transfer.businessDate} onChange={(businessDate) => setTransfer((value) => ({ ...value, businessDate }))} />
        <label>{text.optionalNotes}<input value={transfer.notes} onChange={(event) => setTransfer((value) => ({ ...value, notes: event.target.value }))} /></label>
      </form>
    </BaseerDialog>
...
function VaultCards({ vaults, language, text, canTransfer, transferLabel, onTransfer, onOpen, onEdit, onArchive, onRestore }: {
  vaults: Vault[];
  language: Language;
  ...
  return <div className="administration-role-cards treasury-vault-cards">
    {vaults.map((vault) => <BaseerCard key={vault.id} padding="compact" className={`baseer-metric-card${vault.status === "ARCHIVED" ? " baseer-metric-card--archived" : ""}`}>
      <header className="baseer-metric-card__header">
        <div className="baseer-metric-card__identity"><span className="baseer-metric-card__icon"><VaultGlyph type={vault.type} /></span><span className="baseer-metric-card__copy"><strong>{displayName(language, vault)}</strong><small>{language === "ar" ? vault.nameEn : vault.nameAr}</small></span></div>
        <details className="baseer-card-action-menu">
          <summary aria-label={text.vaultActivity}>⋮</summary>
          <div role="menu">
            <button type="button" role="menuitem" className="baseer-card-action-menu__item" onClick={() => onOpen(vault)}>{text.vaultActivity}</button>
            {canTransfer && vault.status === "ACTIVE" ? <button type="button" role="menuitem" className="baseer-card-action-menu__item" onClick={() => onTransfer(vault)}>{transferLabel}</button> : null}
            <button type="button" role="menuitem" className="baseer-card-action-menu__item" onClick={() => onEdit(vault)}>{text.edit}</button>
            <button type="button" role="menuitem" className="baseer-card-action-menu__item is-warning" onClick={() => vault.status === "ARCHIVED" ? onRestore(vault) : onArchive(vault)}>{vault.status === "ARCHIVED" ? text.restoreVault : text.archive}</button>
          </div>
        </details>
      </header>
      <div className="baseer-metric-card__balance"><small>{text.currentBalance}</small><strong className={Number(vault.balanceAsOf) < 0 ? "is-negative" : undefined}>{formatMoney(vault.balanceAsOf)}</strong></div>
      <div className="baseer-metric-card__flows"><span><small>↑ {text.incoming}</small><strong className="is-inflow">{formatMoney(vault.inflow)}</strong></span><span><small>↓ {text.outgoing}</small><strong>{formatMoney(vault.outflow)}</strong></span></div>
      <footer className="baseer-metric-card__footer">{vault.paymentMethods.map((method) => <span className="daily-sales-badge" key={method}>{paymentMethodLabel(text, method)}</span>)}{vault.isSalesChannel ? <span className="daily-sales-badge">{text.salesChannel}</span> : null}{vault.isPaymentDestination ? <span className="daily-sales-badge">{text.paymentDestination}</span> : null}</footer>
    </BaseerCard>)}
  </div>;
}
