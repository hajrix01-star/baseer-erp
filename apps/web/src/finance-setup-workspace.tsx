...
export function FinanceSetupWorkspace({ language, view = "setup" }: { language: "ar" | "en"; view?: "setup" | "suppliers" }) {
  ...
  return <FinanceSettingsHub language={language} text={text} configuration={configuration} saving={saving} onRefreshFoundation={refreshFoundation} />;
...
function FinanceSettingsHub({ language, text, configuration, saving, onRefreshFoundation }: {
  language: "ar" | "en";
  text: ReturnType<typeof financeText>;
  ...
  const openPeriod = configuration.periods.find((period) => period.status === "OPEN");
  const goTo = (hash: string) => { window.location.hash = hash; };
  const activeVaults = configuration.vaults.filter((vault) => vault.status === "ACTIVE").length;
  const activeCategories = configuration.categories.filter((category) => category.status === "ACTIVE").length;
  const activeSuppliers = configuration.suppliers.filter((supplier) => supplier.status === "ACTIVE").length;
  return <section className="daily-sales-workspace finance-setup-workspace" aria-label={text.setup}>
    <header className="administration-section-heading"><div><p className="eyebrow">{text.finance}</p><h3>{text.setup}</h3></div></header>
    <div className="baseer-card-grid finance-setup-readiness">
      <BaseerCard><span>{text.currentPeriod}</span><strong>{openPeriod ? displayName(language, openPeriod) : "—"}</strong></BaseerCard>
      <BaseerCard><span>{text.activeVaults}</span><strong>{activeVaults}</strong></BaseerCard>
      <BaseerCard><span>{text.activeCategories}</span><strong>{activeCategories}</strong></BaseerCard>
      <BaseerCard><span>{text.activeSuppliers}</span><strong>{activeSuppliers}</strong></BaseerCard>
    </div>
    <BaseerCard>
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(12rem, 1fr))", gap: "1rem" }}>
        <section><h3>{text.vaults}</h3><p>{text.vaultsAndPaymentChannels}</p><BaseerButton type="button" variant="secondary" onClick={() => goTo("#module=finance&section=2")}>{text.vaults}</BaseerButton></section>
        <section><h3>{text.categories}</h3><p>{text.categories}</p><BaseerButton type="button" variant="secondary" onClick={() => goTo("#module=finance&section=4")}>{text.categories}</BaseerButton></section>
        <section><h3>{text.suppliers}</h3><p>{text.standardSuppliersDescription}</p><BaseerButton type="button" variant="secondary" onClick={() => goTo("#module=operations&section=4")}>{text.suppliers}</BaseerButton></section>
        <section><h3>{text.tax}</h3><p>{configuration.profile?.vatAccountingEnabled ? text.taxOn : text.taxOff}</p><BaseerButton type="button" variant="secondary" onClick={() => goTo("#module=administration&section=1")}>{text.tax}</BaseerButton></section>
      </div>
    </BaseerCard>
    {configuration.profile && configuration.profile.baseSeedVersion < BASE_FINANCE_SEED_VERSION ? <BaseerCard><h3>{text.seedUpdate}</h3><BaseerButton type="button" variant="secondary" disabled={saving} onClick={() => void onRefreshFoundation()}>{text.updateFinanceSeed}</BaseerButton></BaseerCard> : null}
  </section>;
}
