...
export function FinanceSetupWorkspace({ language, view = "setup" }: { language: "ar" | "en"; view?: "setup" | "suppliers" }) {
  ...
  return <FinanceSettingsHub language={language} text={text} readiness={readiness} saving={saving} onRefreshFoundation={refreshFoundation} />;
...
function FinanceSettingsHub({ language, text, readiness, saving, onRefreshFoundation }: {
  language: "ar" | "en";
  text: ReturnType<typeof financeText>;
  ...
  return <section className="daily-sales-workspace finance-setup-workspace" aria-label={text.setup}>
    ...
    <BaseerCard><div style={{ display: "flex", gap: "0.75rem", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap" }}><div><h3>{text.seedUpdate}</h3><p>{text.seedUpdateDescription}</p></div><BaseerButton type="button" variant="secondary" disabled={saving || profile.baseSeedVersion >= BASE_FINANCE_SEED_VERSION} onClick={() => void onRefreshFoundation()}>{profile.baseSeedVersion >= BASE_FINANCE_SEED_VERSION ? (language === "ar" ? "محدّثة" : "Up to date") : text.updateFinanceSeed}</BaseerButton></div></BaseerCard>
  </section>;
}
