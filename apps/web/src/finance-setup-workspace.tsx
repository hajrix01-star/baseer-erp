...
export function FinanceSetupWorkspace({ language, view = "setup" }: { language: "ar" | "en"; view?: "setup" | "suppliers" }) {
  ...
  if (view === "suppliers") return <SuppliersWorkspacePanel language={language} configuration={configuration} loading={!configuration} saving={saving} message={message} form={supplierForm} editingSupplier={editingSupplier} dialogOpen={supplierDialogOpen} archiveTarget={archiveTarget} onOpen={openSupplierDialog} onClose={() => { setSupplierDialogOpen(false); setEditingSupplier(null); }} onChange={setSupplierForm} onSave={saveSupplier} onArchiveRequest={setArchiveTarget} onArchiveCancel={() => setArchiveTarget(null)} onArchive={archiveSupplier} />;
...
onOpen: (supplier?: SupplierRecord) => void;
  ...
  const columns: readonly DataTableColumn<SupplierRecord>[] = [
    { id: "supplier", header: text.supplier, cell: (supplier) => <strong>{displayName(language, supplier)}</strong> },
    { id: "type", header: text.invoiceType, cell: (supplier) => supplier.supplierType === "PURCHASE" ? text.purchaseInvoice : text.expenseInvoice, align: "center" },
    { id: "category", header: text.defaultCategory, cell: (supplier) => supplier.categoryId ? categoryNames.get(supplier.categoryId) ?? "—" : "—" },
    { id: "tax", header: text.taxNumber, cell: (supplier) => supplier.taxNumber ?? "—" },
    { id: "phone", header: text.phone, cell: (supplier) => supplier.phone ?? "—" },
    { id: "status", header: text.status, cell: (supplier) => supplier.status === "ACTIVE" ? text.active : text.archive, align: "center" },
    { id: "actions", header: "", cell: (supplier) => <div className="baseer-table-actions"><BaseerButton type="button" variant="secondary" onClick={() => onOpen(supplier)}>{text.edit}</BaseerButton>{supplier.status === "ACTIVE" ? <BaseerButton type="button" variant="danger" onClick={() => onArchiveRequest(supplier)}>{text.archive}</BaseerButton> : null}</div>, align: "end" },
  ];
