...
export const updateAdministrationCompany = (session: ActiveSession, companyId: string, body: { nameAr: string; nameEn: string; businessTimezone: string; logoFileMetadataId: string | null; vatRateBasisPoints?: number; contextLocationCode: string | null; contextLocationLabelAr: string | null; contextLatitude: number | null; contextLongitude: number | null }) => api(session, `/administration/companies/${companyId}/settings`, { method: "PUT", headers: jsonHeaders(), body: JSON.stringify(body) });
...
export const updateAdministrationCompanyMigrationReviewLock = (session: ActiveSession, companyId: string, locked: boolean, reason: string) => api(session, `/administration/companies/${companyId}/migration-review-lock`, { method: "PUT", headers: jsonHeaders(), body: JSON.stringify({ locked, reason }) });
export const createAdministrationUser = (session: ActiveSession, body: { login: string; nameAr: string; nameEn: string; password: string; preferredLanguage: "ar" | "en"; avatarKind: "INITIALS" | "MALE" | "FEMALE"; companyIds: string[]; roleId: string }) => api(session, "/administration/users", { method: "POST", headers: jsonHeaders(), body: JSON.stringify(body) });
export const createAdministrationRole = (session: ActiveSession, body: { nameAr: string; nameEn: string; permissionCodes: string[] }) => api(session, "/administration/roles", { method: "POST", headers: jsonHeaders(), body: JSON.stringify(body) });
...
export const replaceAdministrationUserAccess = (session: ActiveSession, userId: string, body: { roleId: string; companyIds: string[]; reason: string }) => api(session, `/administration/users/${userId}/access`, { method: "PUT", headers: jsonHeaders(), body: JSON.stringify(body) });
export const updateAdministrationUserLogin = (session: ActiveSession, userId: string, body: { login: string; reason: string }) => api(session, `/administration/users/${userId}/login`, { method: "PUT", headers: jsonHeaders(), body: JSON.stringify(body) });
export const updateAdministrationUserDisplayName = (session: ActiveSession, userId: string, body: { nameAr: string; nameEn: string; reason: string }) => api(session, `/administration/users/${userId}/display-name`, { method: "PUT", headers: jsonHeaders(), body: JSON.stringify(body) });
