...
export function OwnerDailyBriefWorkspaceRuntime({ language }: { language: Language }) {
  ...
  return <BaseerWorkspace className="owner-daily-brief">
    <div className="owner-daily-brief__paper" data-paper-tone={paperTone} dir={ar ? "rtl" : "ltr"}>
      <header className="owner-daily-brief__notebook-intro"><div><span>{ar ? "مركز القيادة · للمالك فقط" : "Command center · owner only"}</span><p>{ar ? "لقطة محفوظة تجمع قراءة الشركات والتسويق والبريد، وكل القيم المالية شاملة الضريبة." : "A saved snapshot of companies, marketing, and mail. All financial values include VAT."}</p></div><details className="owner-daily-brief__paper-tone"><summary aria-label={ar ? "تغيير لون الدفتر" : "Change notebook color"}><span aria-hidden="true">◐</span><span>{ar ? "لون الدفتر" : "Notebook color"}</span></summary><div role="group" aria-label={ar ? "ألوان الدفتر" : "Notebook colors"}>{([{ id: "notebook-yellow", ar: "أصفر الدفتر", en: "Notebook yellow" }, { id: "pure-white", ar: "أبيض ناصع", en: "Pure white" }, { id: "soft-yellow", ar: "أصفر فاتح", en: "Soft yellow" }] as const).map((tone) => <button key={tone.id} type="button" aria-pressed={paperTone === tone.id} onClick={(event) => { setPaperTone(tone.id); event.currentTarget.closest("details")?.removeAttribute("open"); }}><span className={`owner-daily-brief__paper-tone-swatch is-${tone.id}`} /><span>{ar ? tone.ar : tone.en}</span></button>)}</div></details></header>
...
function CompanyCard({ company, language }: { company: OwnerDailyBriefCompany; language: Language }) {
  ...
  return <BaseerCard className="owner-daily-brief__company" padding="compact">
    ...
    <div className="owner-daily-brief__company-sales"><span className="owner-daily-brief__company-sales-label">{ar ? "مبيعات أمس" : "Yesterday's sales"}</span><bdi dir="ltr">{moneyValue(company.yesterdaySales)}</bdi><span className={change === null ? "is-unknown" : change >= 0 ? "is-up" : "is-down"}>{ar ? "التغير اليومي: " : "Daily change: "}<strong dir="ltr">{changeValue}</strong></span></div>
