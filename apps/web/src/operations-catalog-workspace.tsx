...
type Catalog = {
  units: Unit[];
  sections: Array<{ id: string; nameAr: string; nameEn: string | null; isActive: boolean }>;
...
export function OperationsCatalogWorkspace({ language }: { language: Language }) {
  ...
  const reload = useCallback(async (options: { cursor?: string; append?: boolean } = {}): Promise<Catalog | null> => {
    ...
    const scope = `${current.companyId}:${tab}:${search}:${options.cursor ?? "first"}`;
  ...
  const rawColumns: readonly BaseerDataGridColumn<Item>[] = [
    { id: "material", header: t.material, cell: (item) => <button type="button" className="operations-row-link" onClick={() => openItem(item)}><strong>{itemName(item)}</strong><small>{item.code}</small></button> },
  ...
  const menuColumns: readonly BaseerDataGridColumn<Item>[] = [
    ...
    { id: "recipe-cost", header: t.recipeCost, cell: (item) => item.liveRecipeCostStatus === "AVAILABLE" && item.liveRecipeUnitCost !== null ? <span><strong><bdi>{item.liveRecipeUnitCost} {isArabic ? "ر.س" : "SAR"}</bdi></strong><small>{t.recipeCostLive}</small></span> : <small>{item.liveRecipeCostStatus === "NO_RECIPE" ? t.noRecipe : t.incompleteRecipeCost}</small> },
    { id: "price", header: t.salePrice, cell: (item) => item.itemUnits.find((line) => line.isBase)?.menuSaleUnitPrice ?? t.none },
  ];
  ...
  return <>
    <section className="operations-catalog" aria-label={t.title}>
      ...
      {loading ? <BaseerCard><p>{t.loading}</p></BaseerCard> : <>
        <div className="operations-catalog__metrics"><BaseerCard><strong>{t.materials}</strong><p>{catalog?.metrics.activeRawMaterialCount ?? 0}</p></BaseerCard><BaseerCard><strong>{t.needsConversion}</strong><p>{catalog?.metrics.needsConversionCount ?? 0}</p></BaseerCard><BaseerCard><strong>{t.noPrice}</strong><p>{catalog?.metrics.missingPurchasePriceCount ?? 0}</p></BaseerCard></div>
        <nav className="operations-catalog__tabs" aria-label={t.title}>{tabs.map((entry) => <button key={entry.id} type="button" className={tab === entry.id ? "is-active" : ""} onClick={() => { setQuery(""); setTab(entry.id); }}>{entry.label}</button>)}</nav>
