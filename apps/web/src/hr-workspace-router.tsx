...
export function HrWorkspaceRouter({ language, section }: { language: Language; section: number }) {
  ...
  if (section === 2) return <Suspense fallback={<BaseerCard>{ar ? "جارٍ تحميل الإجازات والعودة…" : "Loading leave & return…"}</BaseerCard>}><HrLeaveWorkspace language={language} /></Suspense>;
  if (section === 3) return <Suspense fallback={<BaseerCard>{ar ? "جارٍ تحميل مسير الرواتب…" : "Loading payroll…"}</BaseerCard>}><HrPayrollWorkspace language={language} /></Suspense>;
  if (section === 5) return <Suspense fallback={<BaseerCard>{ar ? "جارٍ تحميل خدمات الموظفين…" : "Loading employee services…"}</BaseerCard>}><HrServicesWorkspace language={language} /></Suspense>;
  ...
  return <Suspense fallback={<BaseerCard>{ar ? "جارٍ تحميل سجل الموارد البشرية…" : "Loading HR register…"}</BaseerCard>}><HrWorkspaceCore language={language} section={section} /></Suspense>;
}
