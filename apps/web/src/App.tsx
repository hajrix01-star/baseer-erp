...
function ThemePicker({ language, theme, onTheme, background, onBackground }: { language: Language; theme: Theme; onTheme: (theme: Theme) => void; background?: LauncherBackground; onBackground?: (background: LauncherBackground) => void }) {
  ...
  const backgrounds: ReadonlyArray<{ id: LauncherBackground; ar: string; en: string }> = [{ id: 'emerald-light', ar: 'أخضر هادئ', en: 'Calm green' }, { id: 'emerald-dark', ar: 'أخضر داكن', en: 'Executive dark' }, { id: 'architectural-light', ar: 'معماري مضيء', en: 'Architectural light' }, { id: 'desert-night', ar: 'ليل تنفيذي', en: 'Executive night' }, { id: 'saudi-heritage', ar: 'تراث سعودي', en: 'Saudi heritage' }, { id: 'saudi-heritage-burned', ar: 'تراث بنقش بصير', en: 'Heritage engraving' }, { id: 'emerald-glass', ar: 'زجاج زمردي', en: 'Emerald glass' }];
...
function ModuleWorkspaceContents({ route, language, theme, background, onLanguage, onTheme, onBackground, onModules, onSection, onStage, onSignOut, permissionCodes }: { route: ResolvedRoute; language: Language; theme: Theme; background: LauncherBackground; onLanguage: () => void; onTheme: (theme: Theme) => void; onBackground: (background: LauncherBackground) => void; onModules: () => void; onSection: (section: number) => void; onStage: (stage: string) => void; onSignOut: () => void; permissionCodes: readonly string[] | null }) {
  ...
  if (route.moduleId === "operations" && route.section === 7) return <><AppHeader language={language} theme={theme} background={background} onLanguage={onLanguage} onTheme={onTheme} onBackground={onBackground} onModules={onModules} onSignOut={onSignOut} /><main className="workspace"><section className="module-page"><div className="page-breadcrumb">Baseer ERP / {module.title[language]}</div><div className="page-heading"><div><h1>{sectionTitle}</h1></div></div><Suspense fallback={<section className="module-page__placeholder">{text.loading}</section>}><OperationsInternalRegistrationWorkspace language={language} /></Suspense></section></main></>;
  return <>
...
export function App() {
  ...
  const [background, setBackground] = useState<LauncherBackground>(() => {
    ...
    return stored === 'emerald-dark' || stored === 'architectural-light' || stored === 'desert-night' || stored === 'saudi-heritage' || stored === 'saudi-heritage-burned' || stored === 'emerald-glass' || stored === 'emerald-light' ? stored : 'emerald-light';
  });
