...
function ModuleWorkspaceContents({ route, language, theme, background, onLanguage, onTheme, onBackground, onModules, onOpenModule, onSection, onStage, onQuickAdvance, onSignOut, permissionCodes }: { route: ResolvedRoute; language: Language; theme: Theme; background: LauncherBackground; onLanguage: () => void; onTheme: (theme: Theme) => void; onBackground: (background: LauncherBackground) => void; onModules: () => void; onOpenModule: (moduleId: ModuleId) => void; onSection: (section: number) => void; onStage: (stage: string) => void; onQuickAdvance: () => void; onSignOut: () => void; permissionCodes: readonly string[] | null }) {
  ...
  if (route.moduleId === "operations" && route.section === 7) return <><AppHeader language={language} theme={theme} background={background} activeModuleId={route.moduleId} permissionCodes={permissionCodes} onLanguage={onLanguage} onTheme={onTheme} onBackground={onBackground} onModules={onModules} onOpenModule={onOpenModule} onQuickAdvance={onQuickAdvance} onSignOut={onSignOut} /><main className="workspace"><section className="module-page"><div className="page-breadcrumb">Baseer ERP / {module.title[language]}</div><div className="page-heading"><div><h1>{sectionTitle}</h1></div></div><Suspense fallback={<section className="module-page__placeholder">{text.loading}</section>}><OperationsInternalRegistrationWorkspace language={language} /></Suspense></section></main></>;
