...
export function HrCompensationAgreementDialog({ open, language, employees, fixedEmployeeId, profile, onClose, onSaved, onError }: { open: boolean; language: Language; employees: readonly HrEmployee[]; fixedEmployeeId?: string; profile?: HrCompensationProfile | null; onClose: () => void; onSaved: () => Promise<void>; onError: (message: string) => void }) {
  ...
  const canBackdate = hasActivePermission("hr.compensation.backdate");
    ...
    <form id="hr-salary-editor" className="baseer-form hr-salary-manager" data-baseer-rhf-form="true" noValidate onSubmit={salaryForm.handleSubmit(() => void submit())}>
      <BaseerFormSection title={selectedEmployee ? (ar ? `راتب ${selectedEmployee.nameAr}` : `${selectedEmployee.nameEn ?? selectedEmployee.nameAr} salary`) : (ar ? "بيانات الراتب" : "Salary details")}>
        ...
        <BaseerFormGrid className="hr-salary-manager__fields">
          ...
          <label className="baseer-form-field">{ar ? "شهر التطبيق" : "Effective month"}<BaseerMonthPicker required min={canBackdate && !existingSalary ? undefined : month(existingSalary ? 1 : 0)} value={draft.effectiveFrom.slice(0, 7)} onChange={(event) => setDraft((value) => ({ ...value, effectiveFrom: `${event.target.value}-01` }))} /></label>
          ...
          {fullEdit ? <details className="baseer-form-advanced" open={requiresOvertimeSchedule}><summary>{ar ? "البدلات وطريقة الاحتساب" : "Allowances & calculation"}</summary><div><label>{ar ? "طريقة الاحتساب" : "Calculation method"}<BaseerStaticSelect label={ar ? "طريقة الاحتساب" : "Calculation method"} value={draft.compensationMethod} onChange={(event) => setDraft((value) => ({ ...value, compensationMethod: event.target.value as HrCompensationMethod }))}><option value="FIXED_MONTHLY">{ar ? "راتب شهري ثابت" : "Fixed monthly salary"}</option><option value="INCLUSIVE_OVERTIME">{ar ? "شامل الأوفر تايم" : "Inclusive overtime"}</option></BaseerStaticSelect></label><label>{ar ? "بدل الأكل" : "Food allowance"}<BaseerMoneyInput value={draft.foodAllowance} onValueChange={(foodAllowance) => setDraft((value) => ({ ...value, foodAllowance }))} /></label><label>{ar ? "بدل السكن" : "Housing allowance"}<BaseerMoneyInput value={draft.housingAllowance} onValueChange={(housingAllowance) => setDraft((value) => ({ ...value, housingAllowance }))} /></label><label>{ar ? "بدل المواصلات" : "Transport allowance"}<BaseerMoneyInput value={draft.transportAllowance} onValueChange={(transportAllowance) => setDraft((value) => ({ ...value, transportAllowance }))} /></label><label>{ar ? "بدلات أخرى" : "Other allowances"}<BaseerMoneyInput value={draft.otherAllowance} onValueChange={(otherAllowance) => setDraft((value) => ({ ...value, otherAllowance }))} /></label>{requiresOvertimeSchedule ? <><label>{ar ? "ساعات الدوام اليومية" : "Daily hours"}<BaseerIntegerInput required min="9" max="12" value={draft.scheduledHoursPerDay} onValueChange={(scheduledHoursPerDay) => setDraft((value) => ({ ...value, scheduledHoursPerDay }))} /></label><label>{ar ? "أيام العمل الشهرية" : "Monthly working days"}<BaseerIntegerInput required min="1" max="31" value={draft.scheduledWorkDays} onValueChange={(scheduledWorkDays) => setDraft((value) => ({ ...value, scheduledWorkDays }))} /></label></> : null}</div></details> : null}
          <label className="baseer-form-field baseer-form-field--full">{ar ? "السبب أو الملاحظة (اختياري)" : "Reason or note (optional)"}<BaseerTextArea compact value={draft.notes} onValueChange={(notes) => setDraft((value) => ({ ...value, notes }))} /></label>
        </BaseerFormGrid>
      </BaseerFormSection>
    </form>
  </BaseerFormDialog>;
