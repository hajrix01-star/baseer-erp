...
export function HrLeaveWorkspace({ language }: { language: Language }) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [leaveReceipt, employeeReceipt] = await Promise.all([listHrEmployeeLeaves(current), listHrEmployees(current)]);
      setLeaves(leaveReceipt.leaves); setEmployees(employeeReceipt.employees);
