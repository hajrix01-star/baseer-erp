...
function HrWorkspaceCore({ language, section }: { language: Language; section: number }) {
  ...
  return <BaseerWorkspace aria-label={text.title}>
    ...
    {compensationTarget ? <Suspense fallback={null}><HrCompensationAgreementDialog open language={language} employees={employees} fixedEmployeeId={compensationTarget.employee.id} profile={compensationTarget.profile} onClose={() => setCompensationTarget(null)} onSaved={async () => { setMessage(language === "ar" ? "تم حفظ الراتب؛ سيطبق من شهر التعديل دون تغيير المسيرات السابقة." : "Salary saved; it applies from the change month without altering past payroll."); await load(); }} onError={setMessage} /></Suspense> : null}
