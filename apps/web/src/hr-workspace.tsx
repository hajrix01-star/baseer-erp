...
export function HrWorkspaceCore({ language, section, stage }: { language: Language; section: number; stage?: string | null }) {
  ...
  const saveAdvanceReversal = async (event: React.FormEvent) => { event.preventDefault(); const current = activeSession(); if (!current || saving) return; setSaving(true); try { await reverseHrEmployeeAdvanceIssue(current, { ...advanceReversalForm, idempotencyKey: requestId() }); setAdvanceReversalOpen(false); setAdvanceDetail(null); showSuccess(language === "ar" ? "تم إلغاء إصدار السلفة." : "The advance issue was cancelled."); await load(); } catch (error) { showError(presentBaseerApiError(error, language, language === "ar" ? "إلغاء إصدار السلفة" : "Cancelling advance issue")); } finally { setSaving(false); } };
  ...
  return <BaseerWorkspace aria-label={text.title}>
    ...
    {section === 0 ? (
      ...
      <>
        <BaseerFilterBar language={language} search={search} searchLabel={sectionTitle} searchPlaceholder={sectionTitle} onSearchChange={setSearch} controls={isAdvance ? <BaseerCombobox label={text.selectEmployee} value={employeeFilter} placeholder={text.selectEmployee} options={activeEmployees.map((item) => ({ id: item.id, label: `${item.employeeNumber} · ${language === "ar" ? item.nameAr : (item.nameEn ?? item.nameAr)}` }))} remoteSearch={searchEmployeeOptions} scopeKey={session?.companyId ?? "signed-out"} onChange={setEmployeeFilter} /> : undefined} appliedFilters={[...(search ? [{ id: "q", label: search, onRemove: () => setSearch("") }] : []), ...(employeeFilter ? [{ id: "employee", label: activeEmployees.find((item) => item.id === employeeFilter)?.nameAr ?? employeeFilter, onRemove: () => setEmployeeFilter("") }] : [])]} onClear={() => { setSearch(""); setEmployeeFilter(""); }} />
    ...
    <BaseerFormDialog open={employeeOpen} title={text.editEmployee} language={language} busy={saving} size="standard" className="hr-employee-edit-dialog" formId="hr-employee-form" submitLabel={text.save} onClose={() => { setEmployeeOpen(false); setEditingEmployeeId(null); setAdditionalEmployeeInfoOpen(false); }}>
      <form id="hr-employee-form" className="baseer-form hr-employee-edit" onSubmit={(event) => void saveEmployee(event)}>
        ...
        <BaseerFormGrid className="hr-employee-edit__core">
          ...
          {employeeForm.status === "TERMINATED" ? <label className="baseer-form-field">{text.terminationDate}<BaseerDatePicker language={language} label={text.terminationDate} min={employeeForm.hireDate} max={today()} value={employeeForm.terminatedAt} onChange={(terminatedAt) => setEmployeeForm((value) => ({ ...value, terminatedAt }))} /></label> : null}
        </BaseerFormGrid>
