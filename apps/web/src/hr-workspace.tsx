...
export function HrWorkspaceCore({ language, section, stage }: { language: Language; section: number; stage?: string | null }) {
  ...
  const load = useCallback(async (): Promise<boolean> => {
    ...
    try {
      ...
      const employeeReceipt = await listHrEmployees(current, { search: section === 1 ? employeeSearch || undefined : undefined });
  ...
  const loadMoreRegister = async (kind: "employees" | "advances" | "deductions") => {
    ...
    try {
      if (kind === "employees" && nextEmployeeCursor) {
        const receipt = await listHrEmployees(current, { search: section === 1 ? employeeSearch || undefined : undefined, cursor: nextEmployeeCursor });
  ...
  const saveAdvanceReversal = async (event: React.FormEvent) => { event.preventDefault(); const current = activeSession(); if (!current || saving) return; setSaving(true); try { await reverseHrEmployeeAdvanceIssue(current, { ...advanceReversalForm, idempotencyKey: requestId() }); setAdvanceReversalOpen(false); setAdvanceDetail(null); showSuccess(language === "ar" ? "تم عكس إصدار السلفة." : "The advance issue was reversed."); await load(); } catch (error) { showError(presentBaseerApiError(error, language, language === "ar" ? "عكس إصدار السلفة" : "Reversing advance issue")); } finally { setSaving(false); } };
  ...
  return <BaseerWorkspace aria-label={text.title}>
    ...
    <BaseerDialog open={advanceDetail !== null} title={advanceDetail?.advance.advanceNumber ?? text.advances} language={language} onClose={() => setAdvanceDetail(null)} footer={advanceDetail ? <>{hasActivePermission("hr.advances.reverse") && advanceDetail.advance.status === "ISSUED" && advanceDetail.settlements.length === 0 && !advanceDetail.hasMoreSettlements ? <BaseerButton type="button" variant="danger" onClick={() => { setAdvanceReversalForm({ advanceId: advanceDetail.advance.id, businessDate: today(), reason: "" }); setAdvanceReversalOpen(true); }}>{language === "ar" ? "عكس إصدار السلفة" : "Reverse advance issue"}</BaseerButton> : null}<BaseerButton type="button" variant="secondary" onClick={() => setAdvanceDetail(null)}>{text.cancel}</BaseerButton></> : undefined}>{advanceDetail ? <div className="administration-list"><article><strong>{language === "ar" ? advanceDetail.advance.employeeNameAr : advanceDetail.advance.employeeNameEn ?? advanceDetail.advance.employeeNameAr}</strong><span>{text.originalAmount}: <BaseerMoney value={advanceDetail.advance.originalAmount} language={language} /></span><span>{text.remainingAmount}: <BaseerMoney value={advanceDetail.advance.remainingAmount} language={language} /></span><span>{text.status}: {hrEnumLabel(language, advanceDetail.advance.status)}</span></article><article><strong>{text.settleAdvance}</strong>{advanceDetail.settlements.length ? advanceDetail.settlements.map((item) => <span key={item.id}>{item.businessDate} · <BaseerMoney value={item.amount} language={language} /> · {item.source === "PAYROLL" ? text.payrollPayment : text.settleAdvance} · {item.sourceReference ?? "—"}</span>) : <span>—</span>}{advanceDetail.nextSettlementCursor ? <BaseerButton type="button" variant="secondary" onClick={() => void loadMoreAdvanceSettlements()}>{text.loadMore}</BaseerButton> : null}</article><article><strong>{text.deferAdvance}</strong>{advanceDetail.deferrals.length ? advanceDetail.deferrals.map((item) => <span key={item.id}>{item.businessDate} · {item.deferredUntil} · {item.reason}</span>) : <span>—</span>}{advanceDetail.nextDeferralCursor ? <BaseerButton type="button" variant="secondary" onClick={() => void loadMoreAdvanceDeferrals()}>{text.loadMore}</BaseerButton> : null}</article></div> : null}</BaseerDialog>
    ...
    <BaseerFormDialog open={advanceOpen} title={text.addAdvance} language={language} busy={saving} size="standard" formId="hr-advance-form" submitLabel={text.save} onClose={() => setAdvanceOpen(false)}>
      <form id="hr-advance-form" className="baseer-form" onSubmit={(event) => void saveAdvance(event)}>
        <BaseerFormSection title={language === "ar" ? "بيانات السلفة" : "Advance details"}>
          <BaseerFormGrid>
            <label className="baseer-form-field baseer-form-field--full">{text.selectEmployee}<BaseerSearchSelect required label={text.selectEmployee} value={advanceForm.employeeId} placeholder={text.selectEmployee} options={activeEmployees.map((item) => ({ id: item.id, label: `${item.employeeNumber} · ${language === "ar" ? item.nameAr : item.nameEn ?? item.nameAr}` }))} remoteSearch={searchEmployeeOptions} onChange={(employeeId) => setAdvanceForm((value) => ({ ...value, employeeId }))} /></label>
    ...
    <BaseerFormDialog open={advanceReversalOpen} title={language === "ar" ? "عكس إصدار السلفة" : "Reverse advance issue"} language={language} busy={saving} size="compact" formId="hr-advance-reversal-form" submitLabel={language === "ar" ? "تأكيد العكس" : "Confirm reversal"} onClose={() => setAdvanceReversalOpen(false)}><form id="hr-advance-reversal-form" className="baseer-form" onSubmit={(event) => void saveAdvanceReversal(event)}><BaseerFormSection title={language === "ar" ? "بيانات العكس" : "Reversal details"}><BaseerFormGrid columns="one"><BaseerDatePicker language={language} label={text.date} max={today()} value={advanceReversalForm.businessDate} onChange={(businessDate) => setAdvanceReversalForm((value) => ({ ...value, businessDate }))} /><label>{language === "ar" ? "سبب العكس" : "Reversal reason"}<BaseerTextArea compact required value={advanceReversalForm.reason} onValueChange={(reason) => setAdvanceReversalForm((value) => ({ ...value, reason }))} /></label></BaseerFormGrid></BaseerFormSection></form></BaseerFormDialog>
