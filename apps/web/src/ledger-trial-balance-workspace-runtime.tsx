...
function LedgerTrialBalanceContent({ language, session, period, setPeriod, includeZeroRows, setIncludeZeroRows, report, loading, loadError, refetch }: { language: Language; session: ActiveSession; period: BaseerPeriodRange; setPeriod: (value: BaseerPeriodRange) => void; includeZeroRows: boolean; setIncludeZeroRows: (value: boolean) => void; report: Report | null; loading: boolean; loadError: unknown; refetch: () => Promise<void> }) {
  ...
  useEffect(() => {
    readEpoch.current += 1; reportRunId.current = ''; readAbort.current?.abort(); setSelected(null); setEvidence(null); setSource(null); setBusy(false); setMessage('');
  ...
  const openEvidence = async (row: Row, scope: Scope, cursor?: string) => {
    ...
    try {
      ...
      const next = await withFreshReportRun((nextRunId) => api<Evidence>(session, `/reports/ledger-trial-balance/${nextRunId}/evidence?${query}`, { signal: controller.signal }), () => reportRunId.current, async () => { const refreshed = await createReportRun(); reportRunId.current = refreshed; return refreshed; });
    ...
    } catch (error) { if (!controller.signal.aborted) setMessage(error instanceof ReportSnapshotRefreshError ? language === 'ar' ? 'انتهت صلاحية لقطة التقرير، حدّث التقرير ثم أعد فتح التفاصيل.' : 'The report snapshot expired. Refresh the report and open the details again.' : presentBaseerApiError(error, language, copy.details)); }
  ...
  const openSource = async (lineId: string) => {
    ...
    setBusy(true);
    try { const next = await withFreshReportRun((nextRunId) => api<Source>(session, `/reports/ledger-trial-balance/${nextRunId}/evidence/${lineId}/source?${new URLSearchParams({ accountId: selected.row.accountId, scope: selected.scope })}`, { signal: controller.signal }), () => reportRunId.current, async () => { const refreshed = await createReportRun(); reportRunId.current = refreshed; return refreshed; }); if (!controller.signal.aborted && epoch === readEpoch.current && activeSession()?.companyId === session.companyId && activeSession()?.sessionExpiresAt === session.sessionExpiresAt) setSource(next); }
    catch (error) { if (!controller.signal.aborted) setMessage(error instanceof ReportSnapshotRefreshError ? language === 'ar' ? 'انتهت صلاحية لقطة التقرير، حدّث التقرير ثم أعد فتح التفاصيل.' : 'The report snapshot expired. Refresh the report and open the details again.' : presentBaseerApiError(error, language, copy.source)); }
    finally { if (epoch === readEpoch.current) setBusy(false); }
  };
  ...
  return <section className="reports-prototype reports-trial-balance" aria-label={copy.title} dir={language === 'ar' ? 'rtl' : 'ltr'}>
    ...
    <section className="reports-prototype__canvas is-wide">
      ...
      {message || rootMessage ? <><p className="reports-prototype__notice is-error">{message || rootMessage}</p><BaseerButton type="button" variant="secondary" onClick={() => { setMessage(''); void refetch().catch(() => undefined); }}>{language === 'ar' ? 'إعادة المحاولة' : 'Retry'}</BaseerButton></> : null}
...
function SourceView({ language, source, onBack, label }: { language: Language; source: Source; onBack: () => void; label: string }) { const ar = language === 'ar'; return <div className="reports-prototype__source-journal"><BaseerButton type="button" variant="secondary" onClick={onBack}>{label}</BaseerButton><p>{source.journalEntry.businessDate} · {ar ? source.journalEntry.labelAr : source.journalEntry.labelEn} · {source.journalEntry.sourceReference}{source.journalEntry.description ? ` · ${source.journalEntry.description}` : ''}</p><table data-baseer-report-table="detail"><thead><tr><th>#</th><th>{ar ? 'الحساب' : 'Account'}</th><th>{ar ? 'مدين' : 'Debit'}</th><th>{ar ? 'دائن' : 'Credit'}</th></tr></thead><tbody>{source.journalEntry.lines.map((line) => <tr key={line.id}><td>{line.lineNumber}</td><td>{line.accountCode} · {ar ? line.accountNameAr : line.accountNameEn}</td><td dir="ltr">{line.debit.display}</td><td dir="ltr">{line.credit.display}</td></tr>)}</tbody></table></div>; }
