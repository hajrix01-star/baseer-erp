...
const copy = {
  ...
  en: {
    ...
    share: "of sales", financialChart: "Financial movement by item", categoryChart: "Purchase and expense breakdown by category", shareBasis: "Share basis", shareOfSpend: "Of total spend", shareOfCollectedSales: "Of total collected sales", total: "Total", details: "Operation details", sourceJournal: "Original operation", openSource: "Open original operation", openLocation: "Open in its section", back: "Back to operations", loadingOperations: "Loading operations…", noOperations: "There are no operations for this item.", close: "Close", debit: "Debit", credit: "Credit",
...
function FinancialRead({ language, session, period, report, previousReport }: { language: Language; session: ActiveSession; period: BaseerPeriodRange; report: Extract<FinancialReport, { state: "READY" }>; previousReport: Extract<FinancialReport, { state: "READY" }> | null }) {
  ...
  const [categoryShareBasis, setCategoryShareBasis] = useState<"spend" | "sales">("spend");
  ...
  const collectedSalesTotal = Math.abs(Number(report.rows.find((row) => row.code === "sales")?.amount.raw ?? 0));
  const shareDenominator = categoryShareBasis === "spend" ? categorySpendTotal : collectedSalesTotal;
  const inlineShareLabel = categoryShareBasis === "spend" ? text.shareOfSpend : text.shareOfCollectedSales;
  const chartPoints = categoryRows
    .map((row, index) => {
      ...
      rank: index + 1,
      shareOfTotalPercent: shareDenominator > 0 ? formatNumberFixed((value / shareDenominator) * 100, 2) : undefined,
    };
    });
