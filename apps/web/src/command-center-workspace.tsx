...
function AnimatedMoney({ money }: { money: MoneyDisplay }) {
  const target = Math.abs(Number(money.raw)); const [value, setValue] = useState(0);
  const decimals = money.display.match(/\.(\d+)/)?.[1]?.length ?? 0;
  useEffect(() => {
  ...
  const display = value === target ? money.display : new Intl.NumberFormat("en-US", { minimumFractionDigits: decimals, maximumFractionDigits: decimals }).format(value);
  return <>{money.sign === "negative" ? "−" : ""}{display}</>;
}
function MoneyValue({ money, onClick, label }: { money: MoneyDisplay; onClick?: () => void; label?: string }) {
  const value = <bdi className={`command-center__money command-center__money--count-up ${moneyClass(money)}`} dir="ltr"><AnimatedMoney money={money} /></bdi>;
...
function FinancialRead({ language, session, period, report, previousReport }: { language: Language; session: ActiveSession; period: BaseerPeriodRange; report: Extract<FinancialReport, { state: "READY" }>; previousReport: Extract<FinancialReport, { state: "READY" }> | null }) {
  ...
  const chartPoints = categoryRows
    .map((row, index) => {
      ...
      displayValue: row.amount.display.replace(/^[−-]\s*/, ""),
      rank: index + 1,
      shareOfTotalPercent: categorySpendTotal > 0 ? ((value / categorySpendTotal) * 100).toFixed(2) : "0.00",
    };
    });
...
function SourceJournalView({ language, source, reference, onBack }: { language: Language; source: SourceJournal; reference: string | null; onBack: () => void }) {
  ...
  return <div className="command-center__source-journal" dir={language === "ar" ? "rtl" : "ltr"}><BaseerButton type="button" variant="secondary" onClick={onBack}>{text.back}</BaseerButton><p><strong dir="ltr">{reference ?? entry.sourceReference}</strong> · {entry.businessDate}{entry.description ? ` · ${entry.description}` : ""}</p><table><thead><tr><th>#</th><th>{language === "ar" ? "الحساب" : "Account"}</th><th>{text.debit}</th><th>{text.credit}</th></tr></thead><tbody>{entry.lines.map((line) => <tr key={line.id}><td>{line.lineNumber}</td><td>{line.accountCode} · {language === "ar" ? line.accountNameAr : line.accountNameEn}</td><td dir="ltr">{line.debitAmount}</td><td dir="ltr">{line.creditAmount}</td></tr>)}</tbody></table></div>;
}
...
function MarketingPanel({ language, session, period }: { language: Language; session: ActiveSession; period: BaseerPeriodRange }) {
  ...
  return <BaseerCompanyReadQuery session={session} resource="command-center.marketing" scope={[period.from, period.to]} load={(current, signal) => api<MarketingRead>(current, `/marketing/calendar?${query.toString()}`, { signal })}>{({ data, loading, error, refetch }) => <section className="command-center__section" aria-busy={loading}>
    <header className="command-center__section-header"><div><h2>{text.marketing}</h2><p>{text.marketingDescription}</p></div><button type="button" className="command-center__link" onClick={() => { window.location.hash = "#module=marketing&section=1"; }}>{text.openMarketing}</button></header>
    {loading ? <BaseerCard className="command-center__loading">{text.loading}</BaseerCard> : null}
...
function MarketingCalendarView({ language, session, month, data, canManageTarget, onMonthChange, onSaved }: { language: Language; session: ActiveSession; month: string; data: MarketingRead; canManageTarget: boolean; onMonthChange: (month: string) => void; onSaved: () => void }) {
  ...
  return <BaseerCard className="command-center__calendar" padding="compact">
    ...
    <div className="command-center__target"><span>{text.salesTarget}</span>{canManageTarget ? <div><input inputMode="decimal" value={targetAmount} onChange={(event) => setTargetAmount(event.target.value)} placeholder="0.00" aria-label={text.salesTarget} /><BaseerButton type="button" variant="secondary" disabled={busy || !targetAmount} onClick={() => void saveTarget()}>{busy ? text.saving : text.saveTarget}</BaseerButton></div> : <bdi dir="ltr">{storedTarget || text.noTarget}</bdi>}</div>
    {monthEvents.length ? <section className="command-center__calendar-events" aria-label={text.monthEvents}><strong>{text.monthEvents}</strong><div>{monthEvents.map((event) => <button type="button" key={event.id} onClick={() => { window.location.hash = "#module=decision&section=1"; }}><span>{event.titleAr}</span><small dir="ltr">{event.startsOn} — {event.endsOn}</small></button>)}</div></section> : null}
