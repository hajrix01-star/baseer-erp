...
import { BaseerMetricActionCard, type BaseerMetricActionTone } from "./baseer-metric-action-card";
...
function HrOverviewContent({ language, data, loading, error, refetch }: { language: Language; data: HrOverviewReceipt | undefined; loading: boolean; error: unknown; refetch: () => void }) {
  ...
  return <BaseerWorkspace className="hr-overview">
    ...
    <section className="hr-overview__metrics" aria-label={ar ? "ملخص الموارد البشرية" : "Human resources summary"}>
      <BaseerMetricActionCard title={ar ? "الموظفون النشطون" : "Active employees"} value={countLabel(workforce?.activeEmployees)} detail={workforce ? (ar ? "فتح سجل الموظفين" : "Open employees") : unavailable} icon="users" tone="success" disabled={!workforce} onClick={() => routeTo(1)} />
      <BaseerMetricActionCard title={ar ? "في إجازة حالياً" : "Currently on leave"} value={countLabel(workforce?.employeesOnLeave)} detail={workforce ? (ar ? "متابعة الإجازات والعودة" : "Review leave and return") : unavailable} icon="calendar" tone="info" disabled={!workforce} onClick={() => routeTo(2)} />
    ...
    <section className={`hr-overview__operations${workforceChartPoints.length ? "" : " hr-overview__operations--without-chart"}`} aria-label={ar ? "اللقطة التشغيلية والمتابعة" : "Operational snapshot and follow-up"}>
      ...
      {task(5, serviceAttentionCount ? "danger" : "brand", "badge", ar ? "خدمات وإقامات قريبة الانتهاء" : "Services nearing expiry", services?.expiredCount ? (ar ? `${formatNumber(services.expiredCount)} منتهية وتحتاج معالجة.` : `${formatNumber(services.expiredCount)} are already expired.`) : (ar ? "تستحق المتابعة خلال 30 يوماً." : "Need review within 30 days."), serviceAttentionCount)}
      {task(2, leaves?.openCount ? "info" : "brand", "calendar", ar ? "إجازات مفتوحة" : "Open leave records", leaves?.openCount ? (ar ? "موظفون في إجازة بانتظار تسجيل العودة." : "Employees are away pending return registration.") : (ar ? "لا توجد عودة معلقة." : "No returns are pending."), leaves?.openCount ?? null)}
      {task(1, finalSettlements?.openCount ? "warning" : "brand", "ledger", ar ? "مخالصات نهاية الخدمة" : "Final settlements", finalSettlements?.openCount ? (ar ? "افتح ملف الموظف لإكمال التحقق أو الاعتماد أو السداد." : "Open the employee file to verify, approve, or pay.") : (ar ? "لا توجد مخالصات مفتوحة." : "No open settlements."), finalSettlements?.openCount ?? null)}
      </div></section>
    </section>
