...
export function LedgerTrialBalanceWorkspace({ language }: { language: Language }) {
  ...
  return <BaseerCompanyReadQuery session={session} resource="reports.ledger-trial-balance" scope={[language, period.from, period.to, String(includeZeroRows)]} mode="snapshot" load={(current, signal) => api<Report>(current, `/reports/ledger-trial-balance?from=${encodeURIComponent(period.from)}&to=${encodeURIComponent(period.to)}&includeZeroRows=${includeZeroRows}`, { signal })}>
...
function LedgerTrialBalanceContent({ language, session, period, setPeriod, includeZeroRows, setIncludeZeroRows, report, loading, loadError, refetch }: { language: Language; session: ActiveSession; period: BaseerPeriodRange; setPeriod: (value: BaseerPeriodRange) => void; includeZeroRows: boolean; setIncludeZeroRows: (value: boolean) => void; report: Report | null; loading: boolean; loadError: unknown; refetch: () => Promise<void> }) {
  ...
  useEffect(() => {
    readEpoch.current += 1; readAbort.current?.abort(); setSelected(null); setEvidence(null); setSource(null); setBusy(false); setMessage('');
    return () => readAbort.current?.abort();
  }, [session.companyId, session.sessionExpiresAt, period.from, period.to, includeZeroRows, runId]);
  const openEvidence = async (row: Row, scope: Scope, cursor?: string) => {
    ...
    try {
      ...
      const next = await api<Evidence>(session, `/reports/ledger-trial-balance/${report.reportRunId}/evidence?${query}`, { signal: controller.signal });
  ...
  const openSource = async (lineId: string) => {
    ...
    setBusy(true);
    try { const next = await api<Source>(session, `/reports/ledger-trial-balance/${report.reportRunId}/evidence/${lineId}/source?${new URLSearchParams({ accountId: selected.row.accountId, scope: selected.scope })}`, { signal: controller.signal }); if (!controller.signal.aborted && epoch === readEpoch.current && activeSession()?.companyId === session.companyId && activeSession()?.sessionExpiresAt === session.sessionExpiresAt) setSource(next); }
  ...
  return <section className="reports-prototype reports-trial-balance" aria-label={copy.title} dir={language === 'ar' ? 'rtl' : 'ltr'}>
    ...
    <section className="reports-prototype__canvas is-wide">
      <div className="reports-prototype__toolbar reports-prototype__toolbar--with-actions"><BaseerFilterBar language={language} controls={<><BaseerPeriodFilter language={language} value={period} onChange={(next) => setPeriod(trialBalancePeriod(next))} presets={['DAY', 'MONTH', 'QUARTER', 'YEAR', 'RANGE']} allowNonContiguousMonths={false} /><BaseerFilterToggle label={copy.zero} checked={includeZeroRows} onChange={setIncludeZeroRows} /></>} />{!loading && report?.state === 'READY' ? <ReportDocumentActions session={session} reportRunId={report.reportRunId} language={language} /> : null}</div>
