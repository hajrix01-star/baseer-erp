    ...
    <BaseerBatchPanel id={`hr-salary-tools-panel-${activeTab}`} labelledBy={`hr-salary-tools-${activeTab}`}>
      ...
      <section className="hr-salary-tools__calculator">
        <section className="hr-salary-tools__inputs" aria-labelledby="salary-tool-inputs-title"><header><h3 id="salary-tool-inputs-title">{ar ? "مدخلات الحاسبة" : "Calculator inputs"}</h3></header><div className="hr-salary-tools__input-grid">
          ...
          {draft.compensationMethod === "INCLUSIVE_OVERTIME" ? <><label>{ar ? "ساعات الدوام المتفق عليها يومياً" : "Agreed daily hours"}<input inputMode="numeric" dir="ltr" min="9" max="12" value={draft.scheduledHoursPerDay} onChange={(event) => setField("scheduledHoursPerDay", normalizeBaseerAmount(event.target.value).replace(".", ""))} /></label><label>{ar ? "أيام العمل المتفق عليها شهرياً" : "Agreed monthly working days"}<input inputMode="numeric" dir="ltr" min="1" max="31" value={draft.scheduledWorkDays} onChange={(event) => setField("scheduledWorkDays", normalizeBaseerAmount(event.target.value).replace(".", ""))} /></label></> : null}
        </div></section>
