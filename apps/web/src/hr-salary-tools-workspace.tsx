...
import { BaseerIntegerInput, BaseerMoneyInput } from "./baseer-form-fields";
    ...
    <BaseerBatchPanel id={`hr-salary-tools-panel-${activeTab}`} labelledBy={`hr-salary-tools-${activeTab}`}>
      ...
      <section className="hr-salary-tools__calculator">
        <section className="hr-salary-tools__inputs" aria-labelledby="salary-tool-inputs-title"><header><h3 id="salary-tool-inputs-title">{ar ? "مدخلات الحاسبة" : "Calculator inputs"}</h3></header><div className="hr-salary-tools__input-grid">
          ...
          <label>{ar ? "طريقة الاحتساب" : "Calculation method"}<BaseerStaticSelect label={ar ? "طريقة الاحتساب" : "Calculation method"} value={draft.compensationMethod} onChange={(event) => setField("compensationMethod", event.target.value as HrCompensationMethod)}><option value="FIXED_MONTHLY">{ar ? "راتب شهري ثابت" : "Fixed monthly salary"}</option><option value="INCLUSIVE_OVERTIME">{ar ? "إجمالي شامل الأوفر تايم" : "Inclusive overtime total"}</option></BaseerStaticSelect></label>
          ...
          {draft.compensationMethod === "INCLUSIVE_OVERTIME" ? <><label>{ar ? "ساعات الدوام المتفق عليها يومياً" : "Agreed daily hours"}<BaseerIntegerInput min="9" max="12" value={draft.scheduledHoursPerDay} onValueChange={(scheduledHoursPerDay) => setField("scheduledHoursPerDay", scheduledHoursPerDay)} /></label><label>{ar ? "أيام العمل المتفق عليها شهرياً" : "Agreed monthly working days"}<BaseerIntegerInput min="1" max="31" value={draft.scheduledWorkDays} onValueChange={(scheduledWorkDays) => setField("scheduledWorkDays", scheduledWorkDays)} /></label></> : null}
        </div></section>
