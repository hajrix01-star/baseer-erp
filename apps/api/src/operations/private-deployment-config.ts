...
function requireValue(name: string, value: string | undefined): string {
  if (!value?.trim()) {
    throw new Error(`${name} must be configured for private online deployment.`);
  }
  return value.trim();
}
...
export function validatePrivateDeploymentConfiguration(
  ...
  if (host !== '0.0.0.0') {
    throw new Error('Private online deployment must bind inside the Docker network.');
  }
  if (environment.BASEER_ALLOW_PUBLIC_SIGNUP !== 'false') {
    throw new Error('Public self-registration must remain disabled.');
  }

  const domain = requireValue('BASEER_PUBLIC_DOMAIN', environment.BASEER_PUBLIC_DOMAIN);
  if (!domainPattern.test(domain)) {
    throw new Error('BASEER_PUBLIC_DOMAIN must be a plain valid domain name.');
  }
  requireValue('BASEER_BACKUP_TARGET_LABEL', environment.BASEER_BACKUP_TARGET_LABEL);

  const jwtSecret = requireValue('IDENTITY_JWT_SECRET', environment.IDENTITY_JWT_SECRET);
