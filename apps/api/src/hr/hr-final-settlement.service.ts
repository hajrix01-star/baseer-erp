...
export class HrFinalSettlementService {
  ...
  async create(context: TrustedCompanyActorContext, input: CreateHrFinalSettlementRequest) {
    ...
    return this.database.inTenantTransaction(context.tenantId, async (tx) => {
      ...
      if (begun.kind === 'replay') return begun.response.body as Receipt;
  ...
  async approve(context: TrustedCompanyActorContext, input: Omit<ApproveHrFinalSettlementRequest, 'idempotencyKey'>, idempotencyKey: string) {
    return this.database.inTenantTransaction(context.tenantId, async (tx) => {
      ...
      const receipt = { id: settlement.id, settlementNumber: settlement.settlementNumber, replayed: false };
      await this.complete(tx, context, begun.receiptId, 200, receipt); await this.audit(tx, context, 'hr.final_settlement.approved', settlement.id, { ...receipt, journalEntryId: journal.journalEntryId }); return receipt;
    });
  }
  ...
  async reverse(context: TrustedCompanyActorContext, input: Omit<ReverseHrFinalSettlementRequest, 'idempotencyKey'>, idempotencyKey: string) {
    return this.database.inTenantTransaction(context.tenantId, async (tx) => {
      ...
      await this.lock(tx, context, input.settlementId);
      const settlement = await this.requireSettlement(tx, context, input.settlementId, true);
      if (settlement.status !== HrFinalSettlementStatus.APPROVED || !settlement.accrualJournalEntryId) throw new ConflictException('Only an unpaid approved final settlement can be reversed.');
      ...
      await this.journals.reverseInTransaction(tx, { tenantId: context.tenantId, companyId: context.companyId, actorUserId: context.actorUserId, requestId: `hr-final-settlement-reversal:${settlement.id}`, journalEntryId: settlement.accrualJournalEntryId, businessDate: input.businessDate, reason: input.reason });
      await this.reverseRecoveries(tx, context, settlement, input.businessDate);
      ...
      const receipt = { id: settlement.id, settlementNumber: settlement.settlementNumber, replayed: false };
      await this.complete(tx, context, begun.receiptId, 200, receipt); await this.audit(tx, context, 'hr.final_settlement.reversed', settlement.id, { ...receipt, reason: input.reason }); return receipt;
    });
  }
  ...
  private async reverseRecoveries(tx: Prisma.TransactionClient, context: TrustedCompanyActorContext, settlement: any, businessDate: Date) {
    for (const recovery of settlement.recoveries) { if (recovery.recoveryType === 'ADVANCE') { const advance = await tx.hrEmployeeAdvance.findFirst({ where: { id: recovery.sourceId, tenantId: context.tenantId, companyId: context.companyId }, select: { id: true, settledAmount: true, remainingAmount: true } }); if (!advance || advance.settledAmount.lt(recovery.amount)) throw new ConflictException('The employee advance cannot be restored safely.'); await tx.hrEmployeeAdvance.update({ where: { id: advance.id }, data: { settledAmount: advance.settledAmount.minus(recovery.amount), remainingAmount: advance.remainingAmount.plus(recovery.amount), status: HrEmployeeAdvanceStatus.PARTIALLY_SETTLED } }); }
      else { const deduction = await tx.hrEmployeeAdministrativeDeduction.findFirst({ where: { id: recovery.sourceId, tenantId: context.tenantId, companyId: context.companyId }, select: { id: true, appliedAmount: true, remainingAmount: true } }); if (!deduction || deduction.appliedAmount.lt(recovery.amount)) throw new ConflictException('The administrative deduction cannot be restored safely.'); await tx.hrEmployeeAdministrativeDeduction.update({ where: { id: deduction.id }, data: { appliedAmount: deduction.appliedAmount.minus(recovery.amount), remainingAmount: deduction.remainingAmount.plus(recovery.amount), status: HrEmployeeAdministrativeDeductionStatus.PARTIALLY_APPLIED } }); await tx.hrEmployeeAdministrativeDeductionAction.create({ data: { id: randomUUID(), tenantId: context.tenantId, companyId: context.companyId, deductionId: deduction.id, actionType: HrEmployeeAdministrativeDeductionActionType.REVERSED, businessDate, amount: recovery.amount, reason: `Final settlement ${settlement.settlementNumber} reversed`, createdByUserId: context.actorUserId } }); } }
  }
...
function ymd(value: Date) { return value.toISOString().slice(0, 10) as `${number}-${number}-${number}`; } function json(value: unknown): never { return JSON.parse(canonicalJson(value)) as never; } function hash(value: unknown) { return createHash('sha256').update(canonicalJson(value), 'utf8').digest('hex'); }
