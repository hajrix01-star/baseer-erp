...
export class HrFinalSettlementService {
  ...
  private async applyRecoveries(tx: Prisma.TransactionClient, context: TrustedCompanyActorContext, settlement: any, businessDate: Date, journalEntryId: string) {
    for (const recovery of settlement.recoveries) {
      if (recovery.recoveryType === 'ADVANCE') {
        const advance = await tx.hrEmployeeAdvance.findFirst({ where: { id: recovery.sourceId, tenantId: context.tenantId, companyId: context.companyId }, select: { id: true, businessDate: true, settledAmount: true, remainingAmount: true } });
        if (!advance || recovery.amount.gt(advance.remainingAmount)) throw new ConflictException('The employee-advance balance changed. Refresh and try again.');
        if (!isHrDateOnOrAfter(businessDate, advance.businessDate)) throw new BadRequestException('A final-settlement advance recovery cannot predate the employee-advance issue.');
        const remainingAmount = advance.remainingAmount.minus(recovery.amount);
        await tx.hrEmployeeAdvance.update({ where: { id: advance.id }, data: { remainingAmount, settledAmount: advance.settledAmount.plus(recovery.amount), status: remainingAmount.eq(0) ? HrEmployeeAdvanceStatus.SETTLED : HrEmployeeAdvanceStatus.PARTIALLY_SETTLED } });
        await tx.hrEmployeeAdvanceSettlement.create({ data: { id: randomUUID(), tenantId: context.tenantId, companyId: context.companyId, advanceId: advance.id, source: HrEmployeeAdvanceSettlementSource.FINAL_SETTLEMENT, businessDate, amount: recovery.amount, journalEntryId } });
      } else {
        const deduction = await tx.hrEmployeeAdministrativeDeduction.findFirst({ where: { id: recovery.sourceId, tenantId: context.tenantId, companyId: context.companyId }, select: { id: true, businessDate: true, appliedAmount: true, remainingAmount: true } });
        if (!deduction || recovery.amount.gt(deduction.remainingAmount)) throw new ConflictException('The administrative-deduction balance changed. Refresh and try again.');
        if (!isHrDateOnOrAfter(businessDate, deduction.businessDate)) throw new BadRequestException('A final-settlement administrative deduction cannot predate its creation date.');
        const remainingAmount = deduction.remainingAmount.minus(recovery.amount);
        await tx.hrEmployeeAdministrativeDeduction.update({ where: { id: deduction.id }, data: { remainingAmount, appliedAmount: deduction.appliedAmount.plus(recovery.amount), status: remainingAmount.eq(0) ? HrEmployeeAdministrativeDeductionStatus.APPLIED : HrEmployeeAdministrativeDeductionStatus.PARTIALLY_APPLIED } });
    ...
    const advanceRecoveryAmount = settlement.recoveries.filter((recovery: { recoveryType: string }) => recovery.recoveryType === 'ADVANCE').reduce((total: Prisma.Decimal, recovery: { amount: Prisma.Decimal }) => total.plus(recovery.amount), new Prisma.Decimal(0));
    if (advanceRecoveryAmount.gt(0)) await tx.hrEmployeeFinancialMovement.create({ data: { id: randomUUID(), tenantId: context.tenantId, companyId: context.companyId, employeeId: settlement.employeeId, journalEntryId, movementType: HrEmployeeFinancialMovementType.ADVANCE_SETTLEMENT, businessDate, amount: advanceRecoveryAmount, sourceReference: settlement.settlementNumber, description: 'Employee advances recovered through final settlement' } });
  }
