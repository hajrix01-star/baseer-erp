...
function employeeCreateInput(value: EmployeeCreateInput) {
  return { nameAr: value.nameAr.trim(), nameEn: nullable(value.nameEn), jobTitle: nullable(value.jobTitle), phone: nullable(value.phone), email: nullable(value.email), workSchedule: nullable(value.workSchedule), hireDate: value.hireDate, notes: nullable(value.notes) };
}
function employeeUpdateInput(value: EmployeeUpdateInput) {
  return { nameAr: value.nameAr.trim(), ...(value.nameEn !== undefined ? { nameEn: nullable(value.nameEn) } : {}), ...(value.jobTitle !== undefined ? { jobTitle: nullable(value.jobTitle) } : {}), ...(value.phone !== undefined ? { phone: nullable(value.phone) } : {}), ...(value.email !== undefined ? { email: nullable(value.email) } : {}), ...(value.workSchedule !== undefined ? { workSchedule: nullable(value.workSchedule) } : {}), status: value.status, ...(value.terminatedAt !== undefined ? { terminatedAt: value.terminatedAt } : {}), ...(value.notes !== undefined ? { notes: nullable(value.notes) } : {}) };
}
...
function mapEmployee(value: { id: string; employeeNumber: string; nameAr: string; nameEn: string | null; jobTitle: string | null; phone: string | null; email: string | null; workSchedule: string | null; hireDate: Date; status: HrEmployeeStatus; terminatedAt: Date | null; notes: string | null }) { return { id: value.id, employeeNumber: value.employeeNumber, nameAr: value.nameAr, nameEn: value.nameEn, jobTitle: value.jobTitle, phone: value.phone, email: value.email, workSchedule: value.workSchedule, hireDate: day(value.hireDate)!, status: value.status, terminatedAt: day(value.terminatedAt), notes: value.notes }; }
