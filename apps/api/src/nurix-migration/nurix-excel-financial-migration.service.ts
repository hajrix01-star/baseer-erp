    ...
    return this.database.inTenantTransaction(context.tenantId, async (tx) => {
      ...
      for (const receipt of itemRows) {
        ...
        await Promise.all([
          ...
          ...(item.sourceLedgerNotes ? [upsertSourceAnnotation(tx, { tenantId: context.tenantId, companyId, sourceCompanyId: plan.sourceCompanyId, sourceEntity: 'LedgerEntry', sourceId: item.sourceLedgerId, sourceChecksum: item.sourceLedgerChecksum, targetEntity: 'FinanceJournalEntry', targetId: journal.journalEntryId, field: 'notes', exactText: item.sourceLedgerNotes })] : []),
        ]);
...
function sourceNotes(item: NurixHistoricalFinancePlan['items'][number]) {
  if (item.sourceNotes && item.sourceLedgerNotes) return `${item.sourceNotes}\n[NOORIX_LEDGER_NOTE]\n${item.sourceLedgerNotes}`.slice(0, 2_000);
  return item.sourceNotes ?? item.sourceLedgerNotes ?? 'مستند تاريخي مستورد من نوركس؛ القيد من دليل نوركس المصدر.';
}
