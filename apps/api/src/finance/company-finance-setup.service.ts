  ...
  async initialize(context: TrustedCompanyActorContext, input: CompanyFinanceSetupRequest, idempotencyKey?: string): Promise<CompanyFinanceSetupReceipt> {
    ...
    return this.database.inTenantTransaction(context.tenantId, async (transaction) => {
      ...
      for (const supplier of selectedSuppliers) {
        ...
        const prior = await transaction.financeSupplier.findFirst({ where: { tenantId: context.tenantId, companyId: context.companyId, nameAr: supplier.nameAr }, select: { id: true } });
        if (prior) { supplierIds.push(prior.id); continue; }
        const id = randomUUID();
        await transaction.financeSupplier.create({ data: { id, tenantId: context.tenantId, companyId: context.companyId, categoryId, nameAr: supplier.nameAr, nameEn: supplier.nameEn, isTaxRegistered: false } });
        supplierIds.push(id);
      }
      const receipt: CompanyFinanceSetupReceipt = { periodId: period.id, vaultIds, supplierIds };
      await transaction.auditEvent.create({ data: { id: randomUUID(), tenantId: context.tenantId, companyId: context.companyId, actorUserId: context.actorUserId, action: 'finance.company_setup.initialized', entityType: 'Company', entityId: context.companyId, requestId: RequestContext.correlationId() ?? randomUUID(), afterJson: { periodId: period.id, selectedVaults: choices, selectedStandardSupplierKeys: standardSupplierKeys, supplierIds } as Prisma.InputJsonValue } });
...
function requiredText(value: string, maximumLength: number): string { const text = value?.trim(); if (!text || text.length > maximumLength) throw new BadRequestException('A fiscal-period name is required.'); return text; }function normalizeStandardSupplierKeys(value: readonly StandardSupplierKey[]): StandardSupplierKey[] { const unique = [...new Set(value)]; if (unique.some((item) => !STANDARD_SUPPLIER_KEYS.includes(item))) throw new BadRequestException("An unsupported standard supplier was selected."); return unique; }