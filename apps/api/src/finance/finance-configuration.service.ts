  ...
  async read(input: { accessToken: string; companyId: string }) {
    ...
    return this.database.inTenantTransaction(authorized.principal.tenantId, async (transaction) => {
      ...
      const [profile, periods, vaults, accounts, categories, suppliers] = await Promise.all([
        ...
        transaction.financeSupplier.findMany({ where: { tenantId: authorized.principal.tenantId, companyId }, orderBy: { nameAr: 'asc' }, take: 1_000, select: { id: true, nameAr: true, nameEn: true, phone: true, taxNumber: true, isTaxRegistered: true, supplierType: true, status: true, categoryId: true } }),
      ]);
