...
export const tenantCodeSchema = z.string().trim().min(1).max(64).regex(/^[A-Za-z0-9._-]+$/);
...
export const tokenPrincipalSchema = sessionPrincipalSchema.extend({
  exp: jwtExpirationSchema,
}).strict();

export const accessTokenPrincipalSchema = tokenPrincipalSchema.extend({
  tokenType: z.literal('access'),
}).strict();

export const refreshTokenPrincipalSchema = tokenPrincipalSchema.extend({
...
export const signOutReceiptSchema = z.object({
...
export const activeCompanyReceiptSchema = z.object({
  id: companyIdSchema,
  nameAr: z.string().min(1).max(160),
  nameEn: z.string().min(1).max(160),
